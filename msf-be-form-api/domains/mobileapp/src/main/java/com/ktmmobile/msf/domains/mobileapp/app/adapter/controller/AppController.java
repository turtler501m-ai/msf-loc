...
public class AppController {
    ...
    public CommonResponse<IntroResponse> login(@RequestBody @Valid IntroRequest request) {
        return ResponseUtils.ok(appIntroReader.intro(request));
    }

    @PostMapping("/api/app/login/init")
    public CommonResponse<AppInitResponse> init(@RequestBody @Valid AppInitRequest request) {
        return ResponseUtils.ok(appIntroReader.initLogin(request));
    }

    @PostMapping("/api/app/model/register")
    public CommonResponse<Integer> modelRegist(@RequestBody @Valid AppRegistRequest request) {
        return ResponseUtils.ok(appIntroReader.registModel(request));
    }

    @PostMapping("/api/app/model/remove")
    public CommonResponse<Integer> modelRemove(@RequestBody @Valid AppInitRequest request) {
        return ResponseUtils.ok(appIntroReader.removeModel(request));
    }

    @PostMapping("/api/app/settingbio/modify")
    public CommonResponse<Integer> bioModify(@RequestBody @Valid AppRegistRequest request) {
