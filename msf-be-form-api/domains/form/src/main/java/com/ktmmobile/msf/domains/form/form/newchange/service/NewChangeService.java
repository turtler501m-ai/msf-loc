...
public class NewChangeService {
    ...
    private final NewChangeCheckService newChangeCheckService;
    private final NewChangeSelectService newChangeSelectService;
    ...
    public MsfRequestRecord getNewChangeInfo(NewChangeRequest request) {
        ...
        return new MsfRequestRecord(
            newChangeSelectService.getMsfRequestInfo(request), //MSF_REQUEST
            newChangeSelectService.getMsfRequestCstmrInfo(request), //MSF_REQUEST_CSTMR
            newChangeSelectService.getMsfRequestAgentInfo(request), //MSF_REQUEST_AGENT
            newChangeSelectService.getMsfRequestSaleInfo(request), //MSF_REQUEST_SALE
            newChangeSelectService.getMsfRequestBillReqInfo(request), //MSF_REQUEST_BILL_REQ
            newChangeSelectService.getMsfRequestMoveInfo(request), //MSF_REQUEST_MOVE
            newChangeSelectService.getMsfRequestDvcChgInfo(request), //MSF_REQUEST_DVC_CHG
            newChangeSelectService.getMsfRequestAdditionInfo(request) //MSF_REQUEST_ADDITION
        );
    }
    ...
    public NewChangeDefaultResponse getNewChangeDefaultInfo(NewChangeRequest request) {
        ...
        if (request.getRequestKey() == null) {
            ...
            //3. 요금제 조회
            List<RateInfoResponse> rateInfoDtoList = productInfoService.getRateList(productInfoRequest);
    ...
    public FormResponse<NewChangeResponse> saveNewChangeFormInfo(NewChangeInfoRequest request) {
        ...
        //0. 입력 데이타 유효성체크
        boolean isValidLength = newChangeCheckService.checkInputLength(request);
        if (!isValidLength) {
