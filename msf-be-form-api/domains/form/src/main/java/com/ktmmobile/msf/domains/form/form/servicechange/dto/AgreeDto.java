package com.ktmmobile.msf.domains.form.form.servicechange.dto;

public class AgreeDto {
    private String agrYn;
    private String personalInfoCollectAgree;
    private String othersTrnsAgree;
    private String othersTrnsKtAgree;
    private String othersAdReceiveAgree;
    private String indvLocaPrvAgree;

    private String agrYnTime;
    private String personalInfoCollectAgreeTime;
    private String othersTrnsAgreeTime;
    private String othersTrnsKtAgreeTime;
    private String othersAdReceiveAgreeTime;
    private String indvLocaPrvAgreeTime;

    public String getAgrYn() {
        return agrYn;
    }

    public void setAgrYn(String agrYn) {
        this.agrYn = agrYn;
    }

    public String getPersonalInfoCollectAgree() {
        return personalInfoCollectAgree;
    }

    public void setPersonalInfoCollectAgree(String personalInfoCollectAgree) {
        this.personalInfoCollectAgree = personalInfoCollectAgree;
    }

    public String getOthersTrnsAgree() {
        return othersTrnsAgree;
    }

    public void setOthersTrnsAgree(String othersTrnsAgree) {
        this.othersTrnsAgree = othersTrnsAgree;
    }

    public String getOthersTrnsKtAgree() {
        return othersTrnsKtAgree;
    }

    public void setOthersTrnsKtAgree(String othersTrnsKtAgree) {
        this.othersTrnsKtAgree = othersTrnsKtAgree;
    }

    public String getOthersAdReceiveAgree() {
        return othersAdReceiveAgree;
    }

    public void setOthersAdReceiveAgree(String othersAdReceiveAgree) {
        this.othersAdReceiveAgree = othersAdReceiveAgree;
    }

    public String getIndvLocaPrvAgree() {
        return indvLocaPrvAgree;
    }

    public void setIndvLocaPrvAgree(String indvLocaPrvAgree) {
        this.indvLocaPrvAgree = indvLocaPrvAgree;
    }

    public String getAgrYnTime() {
        return agrYnTime;
    }

    public void setAgrYnTime(String agrYnTime) {
        this.agrYnTime = agrYnTime;
    }

    public String getPersonalInfoCollectAgreeTime() {
        return personalInfoCollectAgreeTime;
    }

    public void setPersonalInfoCollectAgreeTime(String personalInfoCollectAgreeTime) {
        this.personalInfoCollectAgreeTime = personalInfoCollectAgreeTime;
    }

    public String getOthersTrnsAgreeTime() {
        return othersTrnsAgreeTime;
    }

    public void setOthersTrnsAgreeTime(String othersTrnsAgreeTime) {
        this.othersTrnsAgreeTime = othersTrnsAgreeTime;
    }

    public String getOthersTrnsKtAgreeTime() {
        return othersTrnsKtAgreeTime;
    }

    public void setOthersTrnsKtAgreeTime(String othersTrnsKtAgreeTime) {
        this.othersTrnsKtAgreeTime = othersTrnsKtAgreeTime;
    }

    public String getOthersAdReceiveAgreeTime() {
        return othersAdReceiveAgreeTime;
    }

    public void setOthersAdReceiveAgreeTime(String othersAdReceiveAgreeTime) {
        this.othersAdReceiveAgreeTime = othersAdReceiveAgreeTime;
    }

    public String getIndvLocaPrvAgreeTime() {
        return indvLocaPrvAgreeTime;
    }

    public void setIndvLocaPrvAgreeTime(String indvLocaPrvAgreeTime) {
        this.indvLocaPrvAgreeTime = indvLocaPrvAgreeTime;
    }
}
