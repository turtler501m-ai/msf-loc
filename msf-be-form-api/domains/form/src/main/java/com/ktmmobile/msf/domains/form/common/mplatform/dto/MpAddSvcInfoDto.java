...
public class MpAddSvcInfoDto extends CommonXmlVO {
    ...
    private static final Logger logger = LoggerFactory.getLogger(MpAddSvcInfoDto.class);
	 private List<MpSocVO> list;
	    ...
	    public void parse()  {
	        ...
	        for(Element item : itemList){
				...
				String strSoc = XmlParse.getChildValue(item, "soc");
				...
				//무로 부가서비스 제외
	            if(XmlParse.getChildValue(item, "socRateValue").equals("Free") && !"PL249Q800".equals(strSoc)){
	                freeCnt++;
	                vo.setSocRateVat(0);
	            } else if ("PL249Q800".equals(strSoc)) {
					vo.setSocRateVat(0);
				} else {
	                ...
	                try {
	                    ...
	                    logger.warn("[MpAddSvcInfoDto][parse] SOC={} 비과세여부 캐시 조회 실패 — 과세로 처리함. cause={}({})",
