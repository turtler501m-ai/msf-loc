...
public class ScanSvcImpl implements ScanSvc {
    ...
    private static Logger logger = LoggerFactory.getLogger(ScanSvcImpl.class);
    ...
    public void prodSendScan(long requestKey, String cretId) {
        ...
        //1. 서식지 정보를 조회한
        Map<String, String> requestData = appformDao.getAppFormData(requestKey);
        ...
        try {
            ...
            /**
             * SRM25052295051 가입신청서 부가서비스 선택 변경
             * 기본 부가 서비스 제외 {"스팸차단서비스", "발신번호표시", "통합사서함", "정보제공사업자번호차단"}
             */
            String reqAddition = requestData.get("REQ_ADDITION");
            if (reqAddition != null && !reqAddition.isEmpty()) {
                String[] excludeAdditionList = {"스팸차단서비스", "발신번호표시", "통합사서함", "정보제공사업자번호차단"};
                String[] reqAdditionList = reqAddition.split(",");

                List<String> filteredList = Arrays.stream(reqAdditionList)
                    .filter(item -> Arrays.stream(excludeAdditionList).noneMatch(item::equals))
                    .collect(Collectors.toList());

                requestData.put("REQ_ADDITION", String.join(",", filteredList));
            } else {
                requestData.put("REQ_ADDITION", "");
            }
        ...
        //5. XML 파일 생성
        createXml(requestKey, cretId, requestData);

        //6. XML 파일 전송 및 삭제 처리
        xmlFileSend(requestKey);

        //7. 전송상태 상태값 UPDATE 처리
        appformDao.updateAppFormXmlYn(requestKey);
    ...
    public void settingSsnData(Map<String, String> reqMapData) {
        ...
        if (reqMapData.containsKey("CSTMR_NATIVE_RRN")
            && StringUtils.hasText(reqMapData.get("CSTMR_NATIVE_RRN"))
        ...
        if (reqMapData.containsKey("MINOR_AGENT_RRN")
            && StringUtils.hasText(reqMapData.get("MINOR_AGENT_RRN"))
        ...
        if (reqMapData.containsKey("ENTRUST_RES_RRN")
            && StringUtils.hasText(reqMapData.get("ENTRUST_RES_RRN"))
        ...
        if (reqMapData.containsKey("NAME_CHANGE_RRN")
            && StringUtils.hasText(reqMapData.get("NAME_CHANGE_RRN"))
        ...
        if (reqMapData.containsKey("OTHERS_PAYMENT_RRN")
            && StringUtils.hasText(reqMapData.get("OTHERS_PAYMENT_RRN"))
        ...
        if (reqMapData.containsKey("CSTMR_FOREIGNER_RRN")
            && StringUtils.hasText(reqMapData.get("CSTMR_FOREIGNER_RRN"))
        ...
        if (!"".equals(custSSN)) {
            ...
            if (Integer.parseInt(custSSN.substring(6, 7)) == 1 || Integer.parseInt(custSSN.substring(6,
                7)) == 2 || Integer.parseInt(custSSN.substring(6, 7)) == 5 || Integer.parseInt(custSSN.substring(6, 7)) == 6) {
                birYear = 1900 + birYear;
            } else if (Integer.parseInt(custSSN.substring(6, 7)) == 3 || Integer.parseInt(custSSN.substring(6,
                7)) == 4 || Integer.parseInt(custSSN.substring(6, 7)) == 7 || Integer.parseInt(custSSN.substring(6, 7)) == 8) {
    ...
    public void settingData(Map<String, String> reqMapData) {
        ...
        if (reqMapData.containsKey("REQ_PAY_TYPE")
            && StringUtils.hasText(reqMapData.get("REQ_PAY_TYPE"))) {
        ...
        if (reqMapData.containsKey("REQ_BUY_TYPE")
            && StringUtils.hasText(reqMapData.get("REQ_BUY_TYPE"))) {
        ...
        if (reqMapData.containsKey("CSTMR_TYPE_ORG")
            && StringUtils.hasText(reqMapData.get("CSTMR_TYPE_ORG"))) {
        ...
        if (reqMapData.containsKey("APP_CD")
            && StringUtils.hasText(reqMapData.get("APP_CD"))) {
     ...
     * @return
     */
    public String InitializaionXml(long requestKey, String userId, int totalPage) {
        ...
        xml.append("<?xml version='1.0' encoding='utf-8' ?> "
            + "<ONLINE_INFORMATION>"
            + "<REGISTER_DATA>");
        Map<String, String> requestData = appformDao.getAppFormUserData(requestKey);
        ...
        String resNo = String.valueOf(StringUtil.NVL(requestData.get("RES_NO"), ""));
    ...
    public void createXml(long requestKey, String userId, Map<String, String> requestData) {
        ...
        String xmlFileName = "online_information" + "_" + requestKey + ".xml";
        ...
        for (int i = 0; i < appPointInfoList.size(); i++) {
            ...
            if (deleteColumnYn.equals("KT_COUNSEL_AGREE")) {
                String deleteTempValue = String.valueOf(requestData.get("KT_COUNSEL_AGREE"));
                if (deleteTempValue.equals("N")) {
                    continue;
                }
            }
        ...
        //자급제 보상서비스 관련
        if ("Y".equals(requestData.get("CLAUSE_RWD_FLAG"))) {
            xmlCreate = true;

            List<HashMap<String, String>> rwdProdAppPointInfoList = appformDao.getAppFormPointList("F001");

            for (int i = 0; i < rwdProdAppPointInfoList.size(); i++) {
                HashMap<String, String> appPointInfo = rwdProdAppPointInfoList.get(i);
                ...
                //메타 데이터 여부 (데이터와 메타 정보가 동일한 경우 체크표시(v)로 표시)
                if (codeDataYn.equals("Y")) {
                    //데이터와 메타 정보가 동일 한지 확인한다.
                    if (appFormInsertData.equals(codeData)) { // 메타데이터를 V로 변환한다.
                        xmlCreate = true;
        ...
        StringBuffer registerDataXml = new StringBuffer(InitializaionXml(requestKey, userId, totalPage));
    ...
    public void xmlFileSend(long requestKey) {
        ...
        if (userSessionDto != null) {
            ...
            if (isExceptionId != null && "Y".equals(isExceptionId)) {
                ...
                mcpIpStatisticDto.setPrcsSbst("REQUEST_KET[" + requestKey + "]CALL_URL[" + requestUrl + "]");
                mcpIpStatisticDto.setParameter(requestKey + "");
    ...
    private String getGroupCode(Map<String, String> requestData) {
        ...
        boolean isInsurance = false; //단체보험 가입여부

        String socCode = String.valueOf(StringUtil.NVL(requestData.get("SOC_CODE"), ""));  //가입 요금제 코드
        String clauseRentalService = String.valueOf(StringUtil.NVL(requestData.get("CLAUSE_RENTAL_SERVICE"), ""));  //렌탈 여부
        String clauseInsuranceFlag = String.valueOf(StringUtil.NVL(requestData.get("CLAUSE_INSURANCE_FLAG"), ""));  //단체보험 가입여부 여부
        ...
        if (clauseFinanceRatesCDList != null) {
            for (NmcpCdDtlDto nmcpCdDtlDto: clauseFinanceRatesCDList) {
                if (socCode.equals(nmcpCdDtlDto.getDtlCd())) {
                    isFinaceRate = true;
                    break;
                }
            }
        ...
        //렌탈 상품 체크
        if (clauseRentalService.equals("Y")) {
            isRental = true;
        }

        //단체보험 가입여부
        if ("Y".equals(clauseInsuranceFlag)) {
            isInsurance = true;
        }
        ...
        if (isRental) {
            ...
            if (isFinaceRate) { //금융제휴
                if (isInsurance) {
                    groupCode = "A004";   //기본 + 금융제휴 요금제 + 단체보험 가입
                } else {
                    groupCode = "A002";  //기본 + 금융제휴 요금제 +
                }

            } else { //기본그룹
                if (isInsurance) {
                    groupCode = "A003";   //기본 + 단체보험 가입
                } else {
                    groupCode = "A001";  //기본
                }
