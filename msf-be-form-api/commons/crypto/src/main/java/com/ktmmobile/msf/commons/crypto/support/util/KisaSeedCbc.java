...
/**
 * MSP PRX 레거시 시스템과의 SEED-CBC 암복호화 호환을 위해 기존 구현을 이관한 클래스
 */
@SuppressWarnings({"all", "PMD", "checkstyle:all"})
...
public class KisaSeedCbc {
    ...
    private static byte GetB0(int A) { return (byte) (A & 0x0ff); }

    private static byte GetB1(int A) { return (byte) ((A >> 8) & 0x0ff); }

    private static byte GetB2(int A) { return (byte) ((A >> 16) & 0x0ff); }

    private static byte GetB3(int A) { return (byte) ((A >> 24) & 0x0ff); }
    ...
    private static int EndianChange(int dwS) { return ((/*ROTL(dwS,8)*/(((dwS) << (8)) | (((dwS) >> (32 - (8))) & 0x000000ff)) & 0x00ff00ff) | (/*ROTL(dwS,24)*/(((dwS) << (24)) | (((dwS) >> (32 - (24))) & 0x00ffffff)) & 0xff00ff00)); }
    ...
    private static void BLOCK_XOR_CBC(
        int[] OUT_VALUE,
        ...
        OUT_VALUE[out_value_offset + 0] = (in_value1_offset < IN_VALUE1.length
            ? IN_VALUE1[in_value1_offset + 0]
            : 0) ^ (in_value2_offset < IN_VALUE2.length ? IN_VALUE2[in_value2_offset + 0] : 0);
        OUT_VALUE[out_value_offset + 1] = (in_value1_offset + 1 < IN_VALUE1.length
            ? IN_VALUE1[in_value1_offset + 1]
            : 0) ^ (in_value2_offset + 1 < IN_VALUE2.length ? IN_VALUE2[in_value2_offset + 1] : 0);
        OUT_VALUE[out_value_offset + 2] = (in_value1_offset + 2 < IN_VALUE1.length
            ? IN_VALUE1[in_value1_offset + 2]
            : 0) ^ (in_value2_offset + 2 < IN_VALUE2.length ? IN_VALUE2[in_value2_offset + 2] : 0);
        OUT_VALUE[out_value_offset + 3] = (in_value1_offset + 3 < IN_VALUE1.length
            ? IN_VALUE1[in_value1_offset + 3]
            : 0) ^ (in_value2_offset + 3 < IN_VALUE2.length ? IN_VALUE2[in_value2_offset + 3] : 0);
    }
    ...
    private static void KISA_SEED_Encrypt_Block_forCBC(int[] in, int in_offset, int[] out, int out_offset, KISA_SEED_KEY ks) {
        int LR[] = new int[4];        // Input/output values at each round
    ...
    public static int SEED_CBC_init(KISA_SEED_INFO pInfo, KISA_ENC_DEC enc, byte[] pbszUserKey, byte[] pbszIV) {
        int ABCD[] = new int[4];            // Input/output values at each round
    ...
    public static class Common {
        ...
        // 1 block(128-bit) XOR
        public static void BLOCK_XOR_PROPOSAL(
            int[] OUT_VALUE,
            int out_value_offset,
            int[] IN_VALUE1,
            int in_value1_offset,
