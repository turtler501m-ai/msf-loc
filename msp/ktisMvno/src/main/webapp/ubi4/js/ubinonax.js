try{
if (typeof nexacro == 'undefined' || (typeof nexacro.Browser != 'undefined' && nexacro.Browser != "Runtime") || (typeof nexacro._Browser != 'undefined' && nexacro._Browser != "Runtime") || (navigator != 'undefined')) {
//====================================================================================================
...
var UbiJS_WsVersion = "4, 0, 2312, 801";				// 웹소켓 뷰어 버전
...
*/		
		if( parent.length > 0 ) {
			...
			if (parent[parent.length-1].RetrieveEnd) {
			    parent[parent.length-1].RetrieveEnd();
			}
		}
		else {
			if (parent.RetrieveEnd) {
...
*/			
			if( parent.length > 0 ) {
				...
				if (parent[parent.length-1].PrintEnd) {
				    parent[parent.length-1].PrintEnd(status);
				}
			}
			else {
				if (parent.PrintEnd) {
...
*/			
			if( parent.length > 0 ) {
				...
				if (parent[parent.length-1].ExportEnd) {
				    parent[parent.length-1].ExportEnd(filepath);
				}
			}
			else {
				if (parent.ExportEnd) {
...
*/		
		if( parent.length > 0 ) {
			...
			if (parent[parent.length-1].ViewerEnd) {
			    parent[parent.length-1].ViewerEnd();
			}
		}
		else {
			if (parent.ViewerEnd) {
...
function _enableHtmlViewer() {
	if(_ubihtmlviewer) {
		_ubihtmlviewer._enableViewer();
	}
}
/**
...
function UbiJS_ShowWsInstall(isUpdate) {
	...
	if( UbiJS_WINDOC.getElementById(UbiJS_Div_Install) == null ) {
		...
		str[i++] = "	<div style='position:relative; height:40px; margin-top:15px; text-align:center; overflow:hidden; '>";
		str[i++] = "		<button id='ubibtn_download' style='background-color: transparent; border: 0 none; cursor: pointer; vertical-align: middle;min-width:100px; font: bold 14px Dotum; color:#fff; padding:12px 0 12px 0; text-align:center; background:#2372ba;' onclick=\"location.href='{DOWNLOAD_URL}'\">" + _ubinonax_getMessage(UbiJS_Language, 'BTN_Download') + "</button>";
		str[i++] = "		<button id='ubibtn_close' style='background-color: transparent; border: 0 none; cursor: pointer; vertical-align: middle;min-width:100px; font: bold 14px Dotum; color:#fff; padding:12px 0 12px 0; text-align:center; background:#666666;' onclick=\"UbiJS_WINDOC.getElementById('{MSG_ID}').style.display='none';_enableHtmlViewer();\">" + _ubinonax_getMessage(UbiJS_Language, 'BTN_Cancel') + "</button>";
		str[i++] = "	</div>";
		str[i++] = "</div>";
...
} catch(e){ console.log('Object is undefined');}