...
<%
	 ...
	 * @Description : M2M사용자 관리 화면
	 * @
	 * @ 수정일	    수정자 수정내용
	 * @ ---------- ------ -----------------------------
	 * @ 2024.07.26 wooki 최초생성
	 */
%>
...
<script type="text/javascript">
	 var DHX = {
			 ...
			 items : [
					{type:"settings", position:"label-left", labelAlign:"left", labelWidth:55, blockOffset:0},
						{type: 'block', list: [
							...
							{type: 'newcolumn'},
							{type: 'select', name: 'usgYn', label: '사용자상태' ,width:70, labelWidth:60, offsetLeft:20, options:[{value:"", text:"- 전체 -"}, {value:"Y", text:"사용"}, {value:"S", text:"정지"}, {value:"E", text:"잠금"}]},
							{type: 'hidden', name: 'm2mYn', value: 'Y'}
						]
					},
				...
				onButtonClick : function(btnId) {
					...
					switch (btnId) {
						...
						case "btnOrgFind":
							mvno.ui.codefinder("M2MSHOP", function(result) {
		...
		GRID1 : {
			...
			title : "조회결과",
			header : "판매점ID,판매점명,사용자ID,사용자명,핸드폰번호,사용자상태,패스워드상태,최종로그인,usrId",
			columnIds : "orgnId,orgnNm,usrIdMsk,usrNm,mblphnNum,usgNm,passInit,lastLoginDt,usrId",
			...
			widths : "110,160,100,160,100,90,80,140,0",
			hiddenColumns:[8],
			paging: true,
			pageSize:20,
			buttons : {
				left : {
					...
					btnStatusErrorInit: { label : "잠금상태해제" },
			...
			checkSelectedButtons : ["btnStatusInit","btnStatusErrorInit", "btnPassInit","btnMod", "btnDel"],
			...
			onButtonClick : function(btnId, selectedData) {
				switch (btnId) {
					case "btnStatusInit" :
						...
						break;
					case "btnStatusErrorInit" :
						if(selectedData.usgYn == "A" || selectedData.usgYn == "E" || selectedData.usgYn == "L") {
							mvno.cmn.ajax4confirm("CCMN0019", ROOT + "/org/userinfomgmt/updateUserErrorReset.json", selectedData,
									function(resultData) {
										if (resultData.result.code == "OK") {
											mvno.ui.notify("NCMN0002");
											PAGE.GRID1.refresh();
							...
							mvno.ui.alert("ICMN0018");
						}
						break;
					case "btnPassInit" :
						if(selectedData.usgYn == "E") {
							mvno.ui.alert("ICMN0019");
						} else {
					...
					case "btnMod":
						...
						}else if(selectedData.m2mYn != "Y") {
							mvno.ui.alert("자동생성 아이디는 수정 불가합니다.");
						}else if(selectedData.usgYn == "S") {
							mvno.ui.alert("정지 아이디는 수정 불가합니다.");
						}else {
					...
					case "btnDel":
						...
						if(selectedData.m2mYn != "Y") {
							mvno.ui.alert("자동생성 아이디는 삭제 불가합니다.");
						}else {
							var pUsrId = PAGE.GRID1.getSelectedRowData().usrId;

							mvno.ui.prompt("삭제 사유", function (pDelRsn) {
								mvno.cmn.ajax(ROOT + "/org/userinfomgmt/deleteShopUserInfoMgmt.json", {
									usrId: pUsrId,
									delRsn: pDelRsn
								}, function (result) {
			...
			onButtonClick : function(btnId) {
				...
				switch (btnId) {
					...
					case "btnSave":
						...
						//사용자명체크
						if(form.getItemValue("usrNm").indexOf("*") == 1){
							mvno.ui.alert("사용자명을 바르게 입력해 주세요");
							return;
						}
