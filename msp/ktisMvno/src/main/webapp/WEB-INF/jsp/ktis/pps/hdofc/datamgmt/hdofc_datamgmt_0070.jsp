...
<%
  /**
   * @Class Name : hdofc_datamgmt_0010.jsp
   * @Description : 본사  데아타관리  실시간선불정산대상
	...
	function goCustDetail(contractNum){
		...
		var DHX = {
				...
				onButtonClick : function(btnId) {
					...
					switch (btnId) {
						...
						case "btnSearchPps":
							...
							PAGE.GRID1.list(ROOT + "/pps/hdofc/datamgmt/RealDataInfoMgmtList.json", form, {
								callback : function () {
			...
			GRID1 : {
				...
				title : "조회결과",
				header : "계약번호, 계약자, 국적, 식별구분, 요금제, 상태, 개통일, 정지일, 해지일,만료일, 모델명, 비자여부, 사용기간, 총시도수, 총시도액,조정금액,무료충전액,유료충전액,무료취소액,유료취소액,총충전액, 최근충전일,사용횟수,사용분,사용액,일차감,데이타량(M),데이타사용액,잔액,개통대리점,판매점,일차감건수,일차감액,국내음성(분),국내통화료,국제음성(분),국제통화료,국내데이터(M),국내데이타료,국내문자건수,국내문자료,국제문자건수,국제문자료,기타건수,기타금액,계약번호(히든)",
				columnIds : "contractNumStr,subLinkName,adNationNm,custIdntNoIndCdNm,serviceNm,subStatusNm,enterDate,statusStopDt,statusCancelDt,basicExpire,modelName,vizaFlag,useTerm,tryRcgCnt,tryRcgCharge,rcgEditCharge,rcgFreeCharge,rcgPayCharge,rcgFreeCancelCharge,rcgPayCancelCharge,totRcgCharge,lastBasicChgDt,useCnt,strUseDur,strUseCharge,strDayCharge,strUsePkt,strTotPktCharge,strBasicRemains,agentNm,agentSaleNm,dayCnt,strDayCharge,strVocNDur,strVocNCharge,strVocIDur,strVocICharge,strPktNDur,strPktNCharge,smsNCnt,strSmsNCharge,smsICnt,strSmsICharge,etcCnt,strEtcCharge,contractNum",
				widths : "70,120,80,80,100,60,80,80,80,80,120,80,60,70,60,70,70,70,70,70,80,80,60,60,60,60,80,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,100,0", //총 1500
				colAlign : "center,center,center,center,center,center,center,center,center,center,left,center,right,right,right,right,right,right,right,right,right,center,right,right,right,right,right,right,right,right,right,right,right,right,center,right,right,right,right,right,right,right,right,right,right,center",
				colTypes : "link,ro,ro,ro,ro,ro,roXd,roXd,roXd,roXd,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro",
				colSorting : "str,str,str,str,str,str,str,str,str,str,str,str,int,int,int,int,int,int,int,int,int,str,int,int,int,int,int,int,int,str,str,int,int,int,int,int,int,int,int,int,int,int,int,int,str",
				paging : true,
				pageSize :15,
				pagingStyle :2,
				buttons : {
				...
				onButtonClick : function(btnId) {
					...
					switch (btnId) {
						case "btnDnExcel":
							...
							var url = "/pps/hdofc/datamgmt/RealDataInfoMgmtListExcel.json";
							  var searchData =  PAGE.FORM1.getFormData(true);
							  mvno.cmn.download(url+"?menuId=PPS1600007",true,{postData:searchData});
							break;
					}
				}
			}
