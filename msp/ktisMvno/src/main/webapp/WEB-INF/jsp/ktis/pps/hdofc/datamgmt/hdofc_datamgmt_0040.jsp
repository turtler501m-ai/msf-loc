...
<%
  /**
   * @Class Name : hdofc_datamgmt_0040.jsp
   * @Description : 본사  데아타관리  우량정산대상
	...
	function goCustDetail(contractNum){
		...
		var DHX = {
			...
			FORM1 : {
				...
				items : [ 	
						 ...
						 {type:"block", list:[					 
							 ...
							 {type:"block", list:[
													{type:"settings", position:"label-left", labelAlign:"left", labelWidth:30, labelHeight:15, blockOffset:0},
													{type : "input",label : "실사용액",name : "searchRealCharge", width:100,maxlength:20,labelWidth:50, numberFormat:"0,000,000,000", offsetLeft:10, validate:'ValidInteger'} 
							 ]},
							 {type : "newcolumn", offset:10},
							 {type:"block", list:[
													{type:"settings", position:"label-left", labelAlign:"left", labelWidth:50, labelHeight:15, blockOffset:0},
							 						{type : "input",label : "무료충전액",name : "searchRcgFreeCharge", width:100,maxlength:20,labelWidth:60, numberFormat:"0,000,000,000", offsetLeft:10, validate:'ValidInteger'} 
							 ]},
				...
				onButtonClick : function(btnId) {
					...
					switch (btnId) {
						...
						case "btnSearch":
							...
							PAGE.GRID1.list(ROOT + "/pps/hdofc/datamgmt/DataInfoExcellentMgmtList.json", form, {
								callback : function () {
			...
			GRID1 : {
				...
				title : "조회결과",
				header : "계약자,요금제,상태,개통일,정지일,해지일,만료일,모델명,사용기간,총시도수,총충전액,무료충전액,유료충전액,충전건수,최근충전일,사용횟수,사용분,사용액,일차감,데이터량(M),데이터사용액,실사용액,우량값,잔액,개통대리점,판매점,계약번호(히든)",
				columnIds : "subLinkName,serviceNm,subStatusNm,enterDate,statusStopDt,statusCancelDt,basicExpire,modelName,useTerm,tryRcgCnt,totRcgCharge,rcgFreeCharge,rcgPayCharge,totRcgCnt,lastBasicChgDt,useCnt,strUseDur,strUseCharge,strDayCharge,strUsePkt,strTotPktCharge,realCharge,excellentVal,strBasicRemains,agentNm,agentSaleNm,contractNum",
				widths : "120,100,80,80,80,80,80,120,60,70,60,70,70,70,70,70,80,80,60,80,80,60,60,60,120,120,0", //총 1500
				colAlign : "left,center,center,center,center,center,center,left,right,right,right,right,right,right,center,right,right,right,right,center,right,right,right,right,left,left,left",
				colTypes : "ro,ro,ro,roXd,roXd,roXd,roXd,ro,ro,ro,ro,ro,ro,ro,roXd,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro",
				colSorting : "str,str,str,str,str,str,str,str,int,int,int,int,int,int,str,int,int,int,int,int,int,int,int,int,str,str,str",
				paging : true,
				pageSize :15,
				pagingStyle :2,
				buttons : {
				...
				onButtonClick : function(btnId) {
					...
					switch (btnId) {
						case "btnDnExcel":
							...
							var url = "/pps/hdofc/datamgmt/DataInfoExcellentMgmtListExcel.json";
							  var searchData =  PAGE.FORM1.getFormData(true);
							  mvno.cmn.download(url+"?menuId=PPS1600004",true,{postData:searchData});
							break;
					}
				}
			}
