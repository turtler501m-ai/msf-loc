...
<%
	...
	/**
	 * 서식지 파라미터
	 * VIEW_TYPE		화면유형 ( SCAN(스캔하기), SCANVIEW(스캔검색), URLVIEW(URL검색), MCPIVEW(신청서식지보기), MSPVIEW(개통서식지보기))
	 * USE_DEL			삭제권한
...
<script type = "text/javascript">
	...
	// 검색 조건
		...
		GRID1 : {
			css : {height : "160px"},
			title : "가입자관리",
			header : "상품구분,계약번호,고객명,생년월일,나이(만),휴대폰번호,요금제,할인유형,판매정책명,약정개월수,할부개월수,단말모델,단말일련번호,상태,해지사유,모집경로,유입경로,추천인구분,추천인정보,녹취여부,대리점,판매점명,판매자,판매자ID,기변유형,기변일자,기변모델명,기변대리점,프로모션명,평생할인 프로모션명,청구번호,단말 IMEI",
			columnIds : "prodTypeNm,contractNum,cstmrName,birth,age,subscriberNo,lstRateNm,sprtTpNm,salePlcyNm,enggMnthCnt,instMnthCnt,lstModelNm,lstModelSrlNum,subStatusNm,canRsn,onOffTypeNm,openMarketReferer,recommendFlagNm,recommendInfo,recYn,agentNm,shopNm,shopUsrNm,shopUsrId,dvcOperTypeNm,dvcChgDt,dvcModelNm,dvcAgntNm,promotionNm,disPrmtNm,ban,imei",
			widths : "80,80,100,60,60,100,200,80,200,80,80,120,120,60,120,60,60,100,70,60,150,150,80,100,80,80,80,100,200,320,100,140",
			colAlign : "center,center,left,center,right,center,left,center,left,right,right,left,center,center,left,center,center,center,center,center,left,left,left,left,center,center,left,left,left,left,center,center",
			colTypes : "ro,ro,ro,ro,ro,ro,ro,ro,ro,ron,ron,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,ro,roXd,ro,ro,ro,ro,ro,ro",
			colSorting : "str,str,str,str,str,str,str,str,str,int,int,str,str,sr,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str,str",
			hiddenColumns : "30",
			paging : true,
			buttons : {
				hright : {
                ...
                if ( requestKey == "" || resNo == null  ) {
                    ...
                    var uploader = PAGE.FORM25.getUploader("file_upload1");
                        ...
                        callViewer(data);
											...
											mvno.cmn.ajax(ROOT + "/voc/giftvoc/getCustInfo.json", {contractNum : sdata.contractNum}, function(resultData) {
												var vocCnt = resultData.result.vocCnt;
												if(vocCnt > 0){
													mvno.ui.confirm("해당 고객은 30일 내에 사은품 VOC 생성 기록이 있습니다. VOC를 추가 생성 하시겠습니까?", function(){
														openVocRegPop(resultData.result.custInfo);
													});
												} else {
													openVocRegPop(resultData.result.custInfo);
												}
                ...
                // requestKey, resNo 가 있으면...
                    ...
                    switch (btnId) {
                        ...
                        case "btnFaxSarch":
                            ...
                            if (cstmrName == "이관고객") {
                                mvno.ui.alert("해지이관 고객은 URL 검색을 할 수 없습니다.");
                                return;
                            }
			...
			items: [
				{type: "block", list: [
					...
					{type: "fieldset", label: "민원정보", inputWidth:750, list:[
						{type: 'settings', position: 'label-left', labelAlign: "left", labelWidth: 60}
						,{type:"block", list:[
							{type: 'input', label: '민원인', name: 'custNm', width:100, maxLength:20}
							,{type: "newcolumn"}
							,{type: 'input', label: '프로모션명', name: 'promNm', width:100, offsetLeft:15, maxLength:80}
							,{type: "newcolumn"}
							,{type: 'input', label: '상품권종류', name: 'giftType', width:100, offsetLeft:15, maxLength:20}
							,{type: "newcolumn"}
							,{type: 'input', label: '금액', name: 'giftAmt', width:100, offsetLeft:15, maxLength:7, validate:"ValidInteger", numberFormat: "0,000,000,000"}
						]}
						,{type:"block", list:[
							{type: 'select', label: '접수상태', name: 'vocStat', width:100, required:true, validate:"NotEmpty"}
							,{type: "newcolumn"}
							,{type: 'input', label: '인입CTN', name: 'vocCtn', width:100, offsetLeft:15, maxLength:11, validate:"ValidInteger"}
							,{type: "newcolumn"}
							,{type: 'input', label: '구분', name: 'divisn', width:100, offsetLeft:15, maxLength:30}
							,{type: "newcolumn"}
							,{type: 'input', label: '비고', name: 'rmk', width:100, offsetLeft:15, maxLength:100}
						]}
						,{type:"block", list:[
							{type: 'select', label: '담당부서', name: 'vocMngOrgn', width:100}
							,{type: "newcolumn"}
							,{type: 'select', label: 'VOC구분', name: 'vocType', width:100, offsetLeft:15}
							,{type: "newcolumn"}
							,{type: 'calendar', dateFormat: "%Y-%m-%d", serverDateFormat: "%Y%m%d", label: '신청일자', name: 'vocReqDt', width:100, offsetLeft:15}
							,{type: "newcolumn"}
							,{type: 'checkbox', label: '대외기관 언급여부', name: 'extnlOrgnRefYn', width:20, offsetLeft:15, checked:false, labelWidth: 100}
						]}
						,{type:"block", list:[
							{type: 'input', label: '회원ID', name: 'userId', width:100, maxLength:30}
							,{type: "newcolumn"}
							,{type: 'input', label: '추천인ID', name: 'recommId', width:100, offsetLeft:15, maxLength:30}
							,{type: "newcolumn"}
							,{type: 'input', label: '추천인 계약번호', name: 'recommContractNum', width:100, offsetLeft:15, labelWidth: 100, maxLength:11, validate:"ValidInteger"}
						]}
						,{type:"block", list:[
							{type: 'input', label: 'VOC내용', name: 'vocDesc', rows: '8', width: 600, maxLength:500}
						]}
					]},
				]}
			]
			...
			,onButtonClick: function(btnId){
				...
				switch(btnId) {
					case "btnSave" :
						...
						mvno.cmn.ajax(ROOT + "/voc/giftvoc/insertGiftVoc.json", idata, function(result) {
...
function ScanValidation(div){
	...
	if(canChk != null && canChk != "0") {
		//1:녹취청취, 2:스캔하기, 3:서식지보기, 4:스캔검색, 5:URL검색, 6:이전사용자이미지
		if(div == "1"){	
			...
			mvno.ui.alert("해지이관 고객은 URL검색을 할 수 없습니다.");
		}else if(div == "6"){
...
// VOC 등록 팝업 오픈
function openVocRegPop(custInfo) {
	mvno.ui.createForm("FORM10");
	PAGE.FORM10.clear();
	mvno.cmn.ajax4lov(ROOT + "/cmn/cmnCdMgmtService/getCommCombo.json", {grpId:"VOC0034",orderBy:"etc6"}, PAGE.FORM10, "vocStat"); // 접수상태
	mvno.cmn.ajax4lov(ROOT + "/cmn/cmnCdMgmtService/getCommCombo.json", {grpId:"VOC0030",orderBy:"etc6"}, PAGE.FORM10, "vocMngOrgn"); // 담당부서
	mvno.cmn.ajax4lov(ROOT + "/cmn/cmnCdMgmtService/getCommCombo.json", {grpId:"VOC0033",orderBy:"etc6"}, PAGE.FORM10, "vocType"); // VOC구분

	if (custInfo) {
		PAGE.FORM10.setItemValue("subLinkName", custInfo.subLinkName);
		PAGE.FORM10.setItemValue("contractNum", custInfo.contractNum);
		PAGE.FORM10.setItemValue("subscriberNo", custInfo.subscriberNo);
		PAGE.FORM10.setItemValue("lstComActvDate", custInfo.lstComActvDate);
		PAGE.FORM10.setItemValue("reqInDay", custInfo.reqInDay);
		PAGE.FORM10.setItemValue("fstRateNm", custInfo.fstRateNm);
		PAGE.FORM10.setItemValue("lstRateNm", custInfo.lstRateNm);
		PAGE.FORM10.setItemValue("subStatus", custInfo.subStatus);
		PAGE.FORM10.setItemValue("recommendInfo", custInfo.recommendInfo);
		PAGE.FORM10.setItemValue("onOffType", custInfo.onOffType);
		PAGE.FORM10.setItemValue("openMarketReferer", custInfo.openMarketReferer);

		PAGE.FORM10.setItemValue("vocReqDt", new Date().format("yyyymmdd"));
	}

	mvno.ui.popupWindowById("FORM10", "VOC등록", 800, 500, {
		onClose: function(win) {
