...
//                       case "btnDel":
                         ...
                         case "btnRead":
                               ...
                               mvno.cmn.ajax(ROOT + "/m2m/pscboard/getNoticeFile.json", PAGE.GRID1.getSelectedRowData(), function(resultData) {
...
//			header2 : "#rspan,#rspan,#rspan,본사,대리점,판매점,#rspan,#rspan,#rspan",
			...
			header : "번호,제목,첨부,게시일자,판매점,매니저,M2M USIM 유통,소속,작성자", //10
			columnIds : "NOTICE_ID,TITLE,FILE_YN,POST_STRT_DT,ORGN20,ORGN30,ORGN40,REGST_ORGN_NM,REGST_USR_NM",
			colAlign : "center,left,center,center,center,center,center,center,center",
			colTypes : "ro,ro,ch,roXd,ch,ch,ch,ro,ro",
			widths : "100,300,30,86,40,40,100,120,120",
...
//			checkSelectedButtons : [ "btnMod"],
			onButtonClick : function(btnId, selectedData) {
            		...
            		switch (btnId) {
                       ...
                       case "btnMod":
                           ...
                           mvno.cmn.ajax( ROOT + "/m2m/pscboard/checkNoticeAuth.json", PAGE.GRID2.getSelectedRowData(), function(result){
	...
	//                                 PAGE.FORM1.showItem("file_upload1");
	                                 ...
	                                 mvno.cmn.ajax(ROOT + "/m2m/pscboard/getNoticeFile.json", PAGE.GRID2.getSelectedRowData(), function(resultData) {
	      	                           var totCnt = resultData.result.fileTotalCnt;
	...
	//                                     PAGE.FORM1.hideItem("file_upload1");
                           ...
                           case "btnDel":
	   						...
	   						mvno.cmn.ajax4confirm("CCMN0003", ROOT + "/m2m/pscboard/noticeDelete.json"
		      ...
		      switch (btnId) {
		          ...
		          case "btnSearch":
		        	...
		        	if ( '${hqAuth}' == '10') // 본사 사용자인 경우
		  			{
		  				var url = ROOT + "/m2m/pscboard/noticeList.json";
...
//					,{type:"select", width:85, offsetLeft:83, label:"대상조직", name:"ORGN_TYPE_CD", userdata:{ lov: "CMN0003"}}
				...
				,{type:"block", list:[
					...
					,{type:"checkbox",width:5, label:"전체",labelWidth:30, name:"ORGN", offsetLeft:0, labelAlign:"center", checked:false}
						,{type:"newcolumn"}
						,{type:"checkbox",width:5, label:"판매점",name:"ORGN20", position:"label-left", labelAlign:"right",checked:false}
						,{type:"newcolumn"}
 						,{type:"checkbox",width:5, label:"매니저",name:"ORGN30", position:"label-left", labelAlign:"right",checked:false}
 						,{type:"newcolumn"}
						,{type:"checkbox",width:5, label:"M2M USIM 유통",name:"ORGN40", position:"label-left", labelAlign:"right",checked:false}
				...
				,{type: "block", list: [
//					{type: "upload", label:"파일업로드" ,name: "file_upload1", width:535, url: "/m2m/pscboard/fileUpLoad.do" ,autoStart: false, userdata: { limitCount : 3, limitsize: 1000, delUrl:"/org/common/deleteFile2.json"} }
					{type: "upload", label:"파일업로드" ,name: "file_upload1", width:535, url: "/m2m/pscboard/fileUpLoad.do" ,autoStart: false, userdata: { limitCount : 3, limitsize: 1000, delUrl:"/org/common/deleteFile2.json"} }
				]}
			],
					...
					case "btnSave":
						if(form.getItemValue('TITLE').indexOf("<") < 0 && form.getItemValue('TITLE').indexOf(">") < 0 && form.getItemValue('CONTENTS').indexOf("<") < 0 && form.getItemValue('CONTENTS').indexOf(">") < 0) { // 재목, 본문 특수문자(<, >) 체크
							if ( form.getItemValue('inputMod') == "R"){
	                            mvno.cmn.ajax(ROOT + "/m2m/pscboard/noticeInsert.json", form, function(result) {
	                            ...
	                            mvno.cmn.ajax(ROOT + "/m2m/pscboard/noticeUpdate.json", form, function(result) {
	                                mvno.ui.closeWindowById(form, true);  
						...
						break;	
					case "fileDown1" :
						mvno.cmn.download('/m2m/pscboard/downFile.json?fileId=' + PAGE.FORM1.getItemValue("fileId1"));
						break;
                        
					case "fileDel1" :
                        mvno.cmn.ajax(ROOT + "/m2m/pscboard/deleteFile.json",  {fileId:PAGE.FORM1.getItemValue("fileId1"),noticeId:PAGE.GRID2.getSelectedRowData().NOTICE_ID} , function(resultData) {
//						mvno.cmn.ajax(ROOT + "/m2m/pscboard/deleteFile.json",  form , function(resultData) {
					...
					case "fileDown2" :
						mvno.cmn.download('/m2m/pscboard/downFile.json?fileId=' + PAGE.FORM1.getItemValue("fileId2"));
						break;
                        
					case "fileDel2" :
                        mvno.cmn.ajax(ROOT + "/m2m/pscboard/deleteFile.json",  {fileId:PAGE.FORM1.getItemValue("fileId2"),noticeId:PAGE.GRID2.getSelectedRowData().NOTICE_ID} , function(resultData) {
//						mvno.cmn.ajax(ROOT + "/m2m/pscboard/deleteFile.json",  form , function(resultData) {
                           mvno.ui.notify("NCMN0014");
					...
					case "fileDown3" :
						mvno.cmn.download('/m2m/pscboard/downFile.json?fileId=' + PAGE.FORM1.getItemValue("fileId3"));
						break;
                        
					case "fileDel3" :
                        mvno.cmn.ajax(ROOT + "/m2m/pscboard/deleteFile.json",  {fileId:PAGE.FORM1.getItemValue("fileId3"),noticeId:PAGE.GRID2.getSelectedRowData().NOTICE_ID} , function(resultData) {
//						mvno.cmn.ajax(ROOT + "/m2m/pscboard/deleteFile.json",  form , function(resultData) {
                           mvno.ui.notify("NCMN0014");
			...
			,onChange : function(name, value){
        	   ...
        	   {
        		   if(ORGN20S==1 && ORGN30S==1 && ORGN40S==1)
        		   {
			...
			,onValidateMore : function (data){
				...
				// 체크박스 하나도 선택 안된 경우 
				if( (data.ORGN20 == '0') && (data.ORGN30 == '0') && (data.ORGN40 == '0') )
				{
...
//					,{type:"select", width:85, offsetLeft:83, label:"대상조직", name:"ORGN_TYPE_CD", userdata:{ lov: "CMN0003"}, disabled:true}
			...
			onButtonClick : function(btnId) {
				...
				switch (btnId) {
					...
					case "fileDown1" :
						mvno.cmn.download('/m2m/pscboard/downFile.json?fileId=' + PAGE.FORM2.getItemValue("fileId1"));
						break;
                           
					case "fileDown2" :
						mvno.cmn.download('/m2m/pscboard/downFile.json?fileId=' + PAGE.FORM2.getItemValue("fileId2"));
						break;
                           
					case "fileDown3" :
						mvno.cmn.download('/m2m/pscboard/downFile.json?fileId=' + PAGE.FORM2.getItemValue("fileId3"));
						break;
