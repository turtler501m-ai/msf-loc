...
/**
 * 서식지 파라미터
 * VIEW_TYPE		화면유형 ( SCAN(스캔하기), SCANVIEW(스캔검색), MCPIVEW(신청서식지보기), MSPVIEW(개통서식지보기))
 * USE_DEL			삭제권한
...
var DHX = {
	...
	,GRID1 : {
		...
		buttons : {
			hright : {
				btnPrevious: { label : "이전사용자" , auth:"A-PREVIOUS"},
			...
			,left: {
				btnSimple: { label : "직접개통" },
                btnDocRcv: { label : "서류접수문자발송" }
			}
		}
			...
			switch (btnId) {
				case "btnDnExcel" : //엑셀다운로드 버튼 클릭시
					...
					if(PAGE.GRID1.getRowsNum() == 0) {
						...
						var pSearchGbn = searchData.pSearchGbn;
						var pSearchName = searchData.pSearchName;
						if((searchStartDt != null && searchStartDt != '' && searchEndDt != null && searchEndDt != '') || (pSearchGbn != null && pSearchGbn != '' && pSearchName != null && pSearchName != '')){
                            mvno.cmn.downloadCallback(function(result) {
                                PAGE.FORM1.setItemValue("DWNLD_RSN",result);
                                mvno.cmn.ajax(ROOT + "/rcp/rcpMgmt/getRcpNewMgmtListExcelDownload.json?menuId=MSP1000015", PAGE.FORM1.getFormData(true), function(result){
				...
				case "btnDtl" : //Row 더블클릭시 Start
					...
					var trgtMenuId = "MSP1000015";
					var tabTitle = "신청등록";
					
					if((sdata.osstChk > 0 || sdata.knoteChk > 0) && (sdata.cstmrType == "NA" || sdata.cstmrType == "FN")){
						if(confirm("직접개통을 진행하시겠습니까?") == true){
					...
					if(maskingYn == "Y" || maskingYn == "1"){
						mvno.cmn.ajax(ROOT + "/org/userinfomgmt/insertSearchLog.json", {"requestKey":sdata.requestKey,"trgtMenuId":trgtMenuId}, function(result){});
					}
                ...
                case "btnDocRcv":
                    ...
                    if (!sdata.scanId) {
                        mvno.ui.alert("서식지 정보가 없습니다.");
                        return;
                    }

                    var url = "/org/workMgmt/docRcvRequestView.do";
                    var param = "?resNo=" + sdata.resNo + "&viewType=REQUEST";
