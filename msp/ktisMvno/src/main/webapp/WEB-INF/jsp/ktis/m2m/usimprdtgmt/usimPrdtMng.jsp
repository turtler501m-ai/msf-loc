	...
	var DHX = {
			FORM1 : {
				items : [
					...
					, {type:"block", list:[
										{type:"input", label:"대표제품ID", name:"rprsPrdtId", width:100, offsetLeft:10},
										{type:"newcolumn"},
										{type:"input", label:"제조사", name:"prdtCode", width:120, offsetLeft:20},
										{type:"newcolumn"},
				...
				onButtonClick : function(btnId) {
					...
					switch (btnId) {
						case "btnSearch" :
							...
							PAGE.GRID1.list(ROOT + "/m2m/usimPrdtMng/usimPrdtMngList.json", form, {callback : function() {
			...
			GRID1 : {
				...
				title : "M2M USIM 제품관리",
				header : "대표제품ID,제조사,제품명,단가(VAT별도),이미지파일명,사용여부,단가적용일,단가만료일,등록자",	//9
				columnIds : "rprsPrdtId,prdtCode,prdtNm,outUnitPric,orgImgFile,useYn,unitPricApplDttm,unitPricExprDttm,usrNm",
				widths : "100,100,190,80,100,80,100,100,150",
				colAlign : "center,center,center,right,center,center,center,center,center",
				colTypes : "ro,ro,ro,ron,ro,ro,roXd,roXd,ro",
				...
				onRowSelect : function (rowId, celInd) {
					...
					PAGE.GRID2.list(ROOT + "/m2m/usimPrdtMng/usimPrdtMngHist.json", this.getRowData(rowId));
				...
				onButtonClick : function(btnId, selectedData) {
					switch (btnId) {
						...
						case "btnMod":
							...
							mvno.cmn.ajax(ROOT + "/m2m/usimPrdtMng/getImageFile.json", data, function(resultData) {
								...
								if(!mvno.cmn.isEmpty(resultData.result.fileNm)){
									...
									var img = '<img id="img_usim" src="/m2m/usimPrdtMng/getImageFile.do?rprsPrdtId=' + data.rprsPrdtId + '" style="width:auto; height:auto: margin-left:-30px">';//width:auto; height:90px: margin-left:-30px
						...
						case "btnDnExcel":
							...
							mvno.cmn.download(ROOT + "/m2m/usimPrdtMng/usimPrdtMngListEx.json?menuId=M2M1000011", true, {postData:searchData});
							break;							
			...
			FORM2 : {
				...
				items : [
					 ...
					 {type:"block", blockOffset:0, list: [
						{type:'input', name:'rprsPrdtId', label:'대표제품ID', width:160, required: true, maxLength:10},
						{type:'input', name:'prdtCode', label:'제조사', width:160, required: true, maxLength:30},
						
						{type:'input', name:'prdtNm', label:'제품명', width:160, required: true, maxLength:50},

						{type:'input', name:'outUnitPric', label:'단가(VAT별도)', width:160, required: true, numberFormat:"0,000,000,000", validate:"ValidInteger", maxLength:15},
					 ]},
					 {type:"block", blockOffset:0, list: [
							{type:'select', name:'useYn', label:'사용여부', width:160, required: true},
							{type: 'calendar', dateFormat: '%Y-%m-%d', serverDateFormat: '%Y%m%d', name: 'unitPricApplDttm', label: '단가적용일', calendarPosition: 'bottom', width:100 },
							{type: 'calendar', dateFormat: '%Y-%m-%d', serverDateFormat: '%Y%m%d', name: 'unitPricExprDttm', label: '단가만료일', calendarPosition: 'bottom', width:100 },
					 ...
					 {type: "block", blockOffset:0, list: [
							{type: "upload", label:"파일업로드" ,name: "file_upload1", width:285, url:"/m2m/usimPrdtMng/fileUpLoad.do", autoStart: false, userdata: { limitCount : maxFileCnt, limitsize: maxFileSize, delUrl:"/org/common/deleteFile2.json"} }
					 ]}
				],
				buttons : {
					center : {
				...
				onButtonClick : function(btnId) {
					...
					switch (btnId) {
						case "btnSave":
							mvno.cmn.ajax(ROOT + "/m2m/usimPrdtMng/usimPrdtInsert.json", form, function(result) {
			...
			FORM3 : {
				...
				items : [
					 {type:'settings', position:'label-left', labelWidth:'100', inputWidth:'auto'},
					 {type:"block", blockOffset:0, list: [
						{type:'input', name:'rprsPrdtId', label:'대표제품ID', width:160, disabled:true, required: true, maxLength:10},
						{type:'input', name:'prdtCode', label:'제조사', width:160, required: true, maxLength:30},
...
// 							break;
					case "fileDel1" :
						mvno.cmn.ajax(ROOT + "/m2m/usimPrdtMng/deleteFile.json", form, function(resultData) {
				...
				onButtonClick : function(btnId) {
					...
					switch (btnId) {
						...
						case "btnImage" :
							...
							var rprsPrdtId = PAGE.FORM3.getItemValue("rprsPrdtId");
			...
			GRID2 : {
				...
				title : "M2M USIM 제품이력",
				header : "대표제품ID,제조사,제품명,단가(VAT별도),단가VAT포함),이미지파일명,사용여부,단가적용일,단가만료일,수정일시,수정자",	//11
				columnIds : "rprsPrdtId,prdtCode,prdtNm,outUnitPric,outUnitPricVat,orgImgFile,useYn,unitPricApplDttm,unitPricExprDttm,rvisnDttm,usrNm",
				widths : "100,100,190,80,80,100,80,100,100,100,100",
				colAlign : "center,center,center,right,right,center,center,center,center,center,center",
				colTypes : "ro,ro,ro,ron,ron,ro,ro,roXd,roXd,ro,ro",
