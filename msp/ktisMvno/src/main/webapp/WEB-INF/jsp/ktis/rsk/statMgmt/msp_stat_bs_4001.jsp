...
<%
  /**
  * @Class Name : msp_stat_bs_4001.jsp
  * @Description : 채널별영업실적
				...
				, onButtonClick : function(btnId) {
					...
					switch (btnId) {
						case "btnSearch" :
							...
							if(new Date(PAGE.FORM1.getItemValue("stdrDt")).format("yyyymmdd") < "20160601"){
								mvno.ui.alert("2016년 6월 1일 이전 자료는 조회할 수 없습니다.");
								return;
							}
							
							var url = ROOT + "/rsk/statMgmt/getChnlSaleSttcList.json";
...
// 							break;
			...
			GRID1 : {
				...
				, title : "조회결과"
				, header : "선후불,상품구분,채널유형,유지건수,월,#cspan,#cspan,#cspan,#cspan,#cspan,일,#cspan,#cspan,#cspan,#cspan,#cspan"
				, header2 : "#rspan,#rspan,#rspan,#rspan,개통,취소,신규,해지,해지복구,순증,개통,취소,신규,해지,해지복구,순증"
				, columnIds : "pppo,prodNm,chnlTpNm,keepCnt,mmOpenCnt,mmCanCnt,mmNewCnt,mmTmntCnt,mmTmntRclCnt,mmNetCnt,openCnt,canCnt,newCnt,tmntCnt,tmntRclCnt,netCnt"
				, widths : "80,80,80,80,80,80,80,80,80,80,80,80,80,80,80,80"
				, colAlign : "center,center,center,right,right,right,right,right,right,right,right,right,right,right,right,right"
				, colTypes : "ro,ro,ro,ron,ron,ron,ron,ron,ron,ron,ron,ron,ron,ron,ron,ron"
				, colSorting : "str,str,str,int,int,int,int,int,int,int,int,int,int,int,int,int"
				, paging : false
				...
				, onButtonClick : function(btnId){
					...
					switch(btnId){
						case "btnExcel" :
							...
							mvno.cmn.download(ROOT + "/rsk/statMgmt/getChnlSaleSttcListExcel.json?menuId=MSP5003003", true, {postData:searchData}); 
