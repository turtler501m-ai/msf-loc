...
<%
  /**
   * @Class Name : hdofc_sttcmgmt_0090.jsp
   * @Description : 본사  통계관리 사용현황2
	...
	function setCurrentDate(form, name)
	{
		...
		var DHX = {
			...
			FORM1 : {
				...
				items : [ 	
							{type:"block", list:[
							 ...
							 {type : "newcolumn"},
							 {type : "checkbox", label : "요금제별", width : 50, labelWidth:60,name:"searchSocType"},
							 {type : "newcolumn"}
							...
							onButtonClick : function(btnId) {
								...
								switch (btnId) {
									...
									case "btnSearch":
										...
										PAGE.GRID1.list(ROOT + "/pps/hdofc/sttcmgmt/SttcUse2MgmtList.json", form, {
											callback : function () {
												PAGE.FORM1.setItemValue("btnCount1", 0); //로딩 끝나면 초기화 - 조회 버튼
												PAGE.FORM1.setItemValue("btnExcelCount1", 0); //로딩 끝나면 초기화 - 엑셀다운로드버튼
											}
			...
			GRID1 : {
				...
				title : "조회결과",
				header : "기간, 요금제, 일차감건수, 일차감액, 국내음성(분), 국내통화료, 국제음성(분), 국제통화금액, 국내데이타(M), 국내데이타료, 국내문자건수, 국내문자료, 국제문자건수, 국제문자료, 기타건수, 기타금액",
				columnIds : "callDate,socTypeNm,dayMinusCnt,dayMinusAmount,inCallDuration,inCallAmount,outCallDuration,outCallAmount,inDataPktDuration,inDataPktAmount,inSmsCnt,inSmsAmount,outSmsCnt,outSmsAmount,etcCnt,etcAmount",
				widths : "80,80,90,90,90,90,90,90,120,90,90,90,90,90,90,90,*", //총 1500
				colAlign : "center,center,right,right,right,right,right,right,right,right,right,right,right,right,right,right",
				colTypes : "ro,ro,ron,ron,ron,ron,ron,ron,ron,ron,ron,ron,ron,ron,ron,ron",
				colSorting : "str,str,int,int,int,int,int,int,int,int,int,int,int,int,int,int",
				paging: true,
				pageSize :15,
				pagingStyle :2,
				buttons : {
				...
				onButtonClick : function(btnId, selectedData) {
					...
					switch (btnId) {
						...
						case "btnDnExcel":
							...
							var url = ROOT + "/pps/hdofc/sttcmgmt/SttcUse2MgmtListExcel.json";
							
							  var searchData =  PAGE.FORM1.getFormData(true);
							  console.log("searchData",searchData);
								
							  mvno.cmn.download(url+"?menuId=PPS1500009",true,{postData:searchData});
							break;
