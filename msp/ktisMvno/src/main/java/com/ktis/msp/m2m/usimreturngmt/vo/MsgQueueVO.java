/**
 * 
 */
package com.ktis.msp.m2m.usimreturngmt.vo;

import java.io.Serializable;

import com.ktis.msp.base.mvc.BaseVo;

public class MsgQueueVO extends BaseVo implements Serializable {

	private static final long serialVersionUID = 1L;

	private String mseq;
	private String msgType;
	private String sendType;
	private String dkey;
	private String dcnt;
	private String dstaddr;
	private String callback;
	private String stat;
	private String subject;
	private String textType;
	private String text;
	private String html;
	private String expiretime;
	private String filecnt;
	private String fileloc1;
	private String filesize1;
	private String fileloc2;
	private String filesize2;
	private String fileloc3;
	private String filesize3;
	private String fileloc4;
	private String filesize4;
	private String fileloc5;
	private String filesize5;
	private String filecntCheckup;
	private String insertTime;
	private String requestTime;
	private String sendTime;
	private String reportTime;
	private String tcprecvTime;
	private String saveTime;
	private String telecom;
	private String readReply;
	private String result;
	private String repcnt;
	private String serverId;
	private String optId;
	private String optCmp;
	private String optPost;
	private String optName;
	private String extCol0;
	private String extCol1;
	private String extCol2;
	private String extCol3;
	/**
	 * @return the mseq
	 */
	public String getMseq() {
		return mseq;
	}
	/**
	 * @param mseq the mseq to set
	 */
	public void setMseq(String mseq) {
		this.mseq = mseq;
	}
	/**
	 * @return the msgType
	 */
	public String getMsgType() {
		return msgType;
	}
	/**
	 * @param msgType the msgType to set
	 */
	public void setMsgType(String msgType) {
		this.msgType = msgType;
	}
	/**
	 * @return the sendType
	 */
	public String getSendType() {
		return sendType;
	}
	/**
	 * @param sendType the sendType to set
	 */
	public void setSendType(String sendType) {
		this.sendType = sendType;
	}
	/**
	 * @return the dkey
	 */
	public String getDkey() {
		return dkey;
	}
	/**
	 * @param dkey the dkey to set
	 */
	public void setDkey(String dkey) {
		this.dkey = dkey;
	}
	/**
	 * @return the dcnt
	 */
	public String getDcnt() {
		return dcnt;
	}
	/**
	 * @param dcnt the dcnt to set
	 */
	public void setDcnt(String dcnt) {
		this.dcnt = dcnt;
	}
	/**
	 * @return the dstaddr
	 */
	public String getDstaddr() {
		return dstaddr;
	}
	/**
	 * @param dstaddr the dstaddr to set
	 */
	public void setDstaddr(String dstaddr) {
		this.dstaddr = dstaddr;
	}
	/**
	 * @return the callback
	 */
	public String getCallback() {
		return callback;
	}
	/**
	 * @param callback the callback to set
	 */
	public void setCallback(String callback) {
		this.callback = callback;
	}
	/**
	 * @return the stat
	 */
	public String getStat() {
		return stat;
	}
	/**
	 * @param stat the stat to set
	 */
	public void setStat(String stat) {
		this.stat = stat;
	}
	/**
	 * @return the subject
	 */
	public String getSubject() {
		return subject;
	}
	/**
	 * @param subject the subject to set
	 */
	public void setSubject(String subject) {
		this.subject = subject;
	}
	/**
	 * @return the textType
	 */
	public String getTextType() {
		return textType;
	}
	/**
	 * @param textType the textType to set
	 */
	public void setTextType(String textType) {
		this.textType = textType;
	}
	/**
	 * @return the text
	 */
	public String getText() {
		return text;
	}
	/**
	 * @param text the text to set
	 */
	public void setText(String text) {
		this.text = text;
	}
	/**
	 * @return the html
	 */
	public String getHtml() {
		return html;
	}
	/**
	 * @param html the html to set
	 */
	public void setHtml(String html) {
		this.html = html;
	}
	/**
	 * @return the expiretime
	 */
	public String getExpiretime() {
		return expiretime;
	}
	/**
	 * @param expiretime the expiretime to set
	 */
	public void setExpiretime(String expiretime) {
		this.expiretime = expiretime;
	}
	/**
	 * @return the filecnt
	 */
	public String getFilecnt() {
		return filecnt;
	}
	/**
	 * @param filecnt the filecnt to set
	 */
	public void setFilecnt(String filecnt) {
		this.filecnt = filecnt;
	}
	/**
	 * @return the fileloc1
	 */
	public String getFileloc1() {
		return fileloc1;
	}
	/**
	 * @param fileloc1 the fileloc1 to set
	 */
	public void setFileloc1(String fileloc1) {
		this.fileloc1 = fileloc1;
	}
	/**
	 * @return the filesize1
	 */
	public String getFilesize1() {
		return filesize1;
	}
	/**
	 * @param filesize1 the filesize1 to set
	 */
	public void setFilesize1(String filesize1) {
		this.filesize1 = filesize1;
	}
	/**
	 * @return the fileloc2
	 */
	public String getFileloc2() {
		return fileloc2;
	}
	/**
	 * @param fileloc2 the fileloc2 to set
	 */
	public void setFileloc2(String fileloc2) {
		this.fileloc2 = fileloc2;
	}
	/**
	 * @return the filesize2
	 */
	public String getFilesize2() {
		return filesize2;
	}
	/**
	 * @param filesize2 the filesize2 to set
	 */
	public void setFilesize2(String filesize2) {
		this.filesize2 = filesize2;
	}
	/**
	 * @return the fileloc3
	 */
	public String getFileloc3() {
		return fileloc3;
	}
	/**
	 * @param fileloc3 the fileloc3 to set
	 */
	public void setFileloc3(String fileloc3) {
		this.fileloc3 = fileloc3;
	}
	/**
	 * @return the filesize3
	 */
	public String getFilesize3() {
		return filesize3;
	}
	/**
	 * @param filesize3 the filesize3 to set
	 */
	public void setFilesize3(String filesize3) {
		this.filesize3 = filesize3;
	}
	/**
	 * @return the fileloc4
	 */
	public String getFileloc4() {
		return fileloc4;
	}
	/**
	 * @param fileloc4 the fileloc4 to set
	 */
	public void setFileloc4(String fileloc4) {
		this.fileloc4 = fileloc4;
	}
	/**
	 * @return the filesize4
	 */
	public String getFilesize4() {
		return filesize4;
	}
	/**
	 * @param filesize4 the filesize4 to set
	 */
	public void setFilesize4(String filesize4) {
		this.filesize4 = filesize4;
	}
	/**
	 * @return the fileloc5
	 */
	public String getFileloc5() {
		return fileloc5;
	}
	/**
	 * @param fileloc5 the fileloc5 to set
	 */
	public void setFileloc5(String fileloc5) {
		this.fileloc5 = fileloc5;
	}
	/**
	 * @return the filesize5
	 */
	public String getFilesize5() {
		return filesize5;
	}
	/**
	 * @param filesize5 the filesize5 to set
	 */
	public void setFilesize5(String filesize5) {
		this.filesize5 = filesize5;
	}
	/**
	 * @return the filecntCheckup
	 */
	public String getFilecntCheckup() {
		return filecntCheckup;
	}
	/**
	 * @param filecntCheckup the filecntCheckup to set
	 */
	public void setFilecntCheckup(String filecntCheckup) {
		this.filecntCheckup = filecntCheckup;
	}
	/**
	 * @return the insertTime
	 */
	public String getInsertTime() {
		return insertTime;
	}
	/**
	 * @param insertTime the insertTime to set
	 */
	public void setInsertTime(String insertTime) {
		this.insertTime = insertTime;
	}
	/**
	 * @return the requestTime
	 */
	public String getRequestTime() {
		return requestTime;
	}
	/**
	 * @param requestTime the requestTime to set
	 */
	public void setRequestTime(String requestTime) {
		this.requestTime = requestTime;
	}
	/**
	 * @return the sendTime
	 */
	public String getSendTime() {
		return sendTime;
	}
	/**
	 * @param sendTime the sendTime to set
	 */
	public void setSendTime(String sendTime) {
		this.sendTime = sendTime;
	}
	/**
	 * @return the reportTime
	 */
	public String getReportTime() {
		return reportTime;
	}
	/**
	 * @param reportTime the reportTime to set
	 */
	public void setReportTime(String reportTime) {
		this.reportTime = reportTime;
	}
	/**
	 * @return the tcprecvTime
	 */
	public String getTcprecvTime() {
		return tcprecvTime;
	}
	/**
	 * @param tcprecvTime the tcprecvTime to set
	 */
	public void setTcprecvTime(String tcprecvTime) {
		this.tcprecvTime = tcprecvTime;
	}
	/**
	 * @return the saveTime
	 */
	public String getSaveTime() {
		return saveTime;
	}
	/**
	 * @param saveTime the saveTime to set
	 */
	public void setSaveTime(String saveTime) {
		this.saveTime = saveTime;
	}
	/**
	 * @return the telecom
	 */
	public String getTelecom() {
		return telecom;
	}
	/**
	 * @param telecom the telecom to set
	 */
	public void setTelecom(String telecom) {
		this.telecom = telecom;
	}
	/**
	 * @return the readReply
	 */
	public String getReadReply() {
		return readReply;
	}
	/**
	 * @param readReply the readReply to set
	 */
	public void setReadReply(String readReply) {
		this.readReply = readReply;
	}
	/**
	 * @return the result
	 */
	public String getResult() {
		return result;
	}
	/**
	 * @param result the result to set
	 */
	public void setResult(String result) {
		this.result = result;
	}
	/**
	 * @return the repcnt
	 */
	public String getRepcnt() {
		return repcnt;
	}
	/**
	 * @param repcnt the repcnt to set
	 */
	public void setRepcnt(String repcnt) {
		this.repcnt = repcnt;
	}
	/**
	 * @return the serverId
	 */
	public String getServerId() {
		return serverId;
	}
	/**
	 * @param serverId the serverId to set
	 */
	public void setServerId(String serverId) {
		this.serverId = serverId;
	}
	/**
	 * @return the optId
	 */
	public String getOptId() {
		return optId;
	}
	/**
	 * @param optId the optId to set
	 */
	public void setOptId(String optId) {
		this.optId = optId;
	}
	/**
	 * @return the optCmp
	 */
	public String getOptCmp() {
		return optCmp;
	}
	/**
	 * @param optCmp the optCmp to set
	 */
	public void setOptCmp(String optCmp) {
		this.optCmp = optCmp;
	}
	/**
	 * @return the optPost
	 */
	public String getOptPost() {
		return optPost;
	}
	/**
	 * @param optPost the optPost to set
	 */
	public void setOptPost(String optPost) {
		this.optPost = optPost;
	}
	/**
	 * @return the optName
	 */
	public String getOptName() {
		return optName;
	}
	/**
	 * @param optName the optName to set
	 */
	public void setOptName(String optName) {
		this.optName = optName;
	}
	/**
	 * @return the extCol0
	 */
	public String getExtCol0() {
		return extCol0;
	}
	/**
	 * @param extCol0 the extCol0 to set
	 */
	public void setExtCol0(String extCol0) {
		this.extCol0 = extCol0;
	}
	/**
	 * @return the extCol1
	 */
	public String getExtCol1() {
		return extCol1;
	}
	/**
	 * @param extCol1 the extCol1 to set
	 */
	public void setExtCol1(String extCol1) {
		this.extCol1 = extCol1;
	}
	/**
	 * @return the extCol2
	 */
	public String getExtCol2() {
		return extCol2;
	}
	/**
	 * @param extCol2 the extCol2 to set
	 */
	public void setExtCol2(String extCol2) {
		this.extCol2 = extCol2;
	}
	/**
	 * @return the extCol3
	 */
	public String getExtCol3() {
		return extCol3;
	}
	/**
	 * @param extCol3 the extCol3 to set
	 */
	public void setExtCol3(String extCol3) {
		this.extCol3 = extCol3;
	}

}
