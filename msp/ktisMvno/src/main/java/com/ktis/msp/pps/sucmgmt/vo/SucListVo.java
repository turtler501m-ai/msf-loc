package com.ktis.msp.pps.sucmgmt.vo;

import java.io.Serializable;
import java.util.List;

import com.ktis.msp.base.mvc.BaseVo;

public class SucListVo extends BaseVo implements Serializable{
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	String sellUsimKey;  
	String usimNo;
	String frgnrName;     
	String frgnrRn;
	String fileName;
	String fileMask;
	String rip;
	String sysRdate;
	String pstate;
	String frgnrPn;
	String uip;
	String sys_udate;
	String mobileFn;
	String mobileMn;
	String mobileRn;
	String mail;
	String vdateStart;    
	String vdateEnd;
	String lang_type;
	String sstate;
	String agentId;
	String mobileNo;
	String searchGbn;
	String searchName;
	String cntpntShopId;
	String usrId;
	String agentCode;
	String etcFileName;
	String etcFileMask; 
	String fileOx;
	String etcFileOx;
	List<SucVo> items;
	
	public static long getSerialversionuid() {
		return serialVersionUID;
	}

	public String getSellUsimKey() {
		return sellUsimKey;
	}

	public String getUsimNo() {
		return usimNo;
	}

	public String getFrgnrName() {
		return frgnrName;
	}

	public String getFrgnrRn() {
		return frgnrRn;
	}

	public String getFileName() {
		return fileName;
	}

	public String getFileMask() {
		return fileMask;
	}

	public String getRip() {
		return rip;
	}

	public String getSysRdate() {
		return sysRdate;
	}

	public String getPstate() {
		return pstate;
	}

	public String getFrgnrPn() {
		return frgnrPn;
	}

	public String getUip() {
		return uip;
	}

	public String getSys_udate() {
		return sys_udate;
	}

	public String getMobileFn() {
		return mobileFn;
	}

	public String getMobileMn() {
		return mobileMn;
	}

	public String getMobileRn() {
		return mobileRn;
	}

	public String getMail() {
		return mail;
	}

	public String getVdateStart() {
		return vdateStart;
	}

	public String getVdateEnd() {
		return vdateEnd;
	}

	public String getLang_type() {
		return lang_type;
	}

	public String getSstate() {
		return sstate;
	}

	public String getAgentId() {
		return agentId;
	}

	public String getMobileNo() {
		return mobileNo;
	}

	public String getSearchGbn() {
		return searchGbn;
	}

	public String getSearchName() {
		return searchName;
	}

	public String getCntpntShopId() {
		return cntpntShopId;
	}

	public String getUsrId() {
		return usrId;
	}

	public String getAgentCode() {
		return agentCode;
	}

	public String getEtcFileName() {
		return etcFileName;
	}

	public String getEtcFileMask() {
		return etcFileMask;
	}

	public void setSellUsimKey(String sellUsimKey) {
		this.sellUsimKey = sellUsimKey;
	}

	public void setUsimNo(String usimNo) {
		this.usimNo = usimNo;
	}

	public void setFrgnrName(String frgnrName) {
		this.frgnrName = frgnrName;
	}

	public void setFrgnrRn(String frgnrRn) {
		this.frgnrRn = frgnrRn;
	}

	public void setFileName(String fileName) {
		this.fileName = fileName;
	}

	public void setFileMask(String fileMask) {
		this.fileMask = fileMask;
	}

	public void setRip(String rip) {
		this.rip = rip;
	}

	public void setSysRdate(String sysRdate) {
		this.sysRdate = sysRdate;
	}

	public void setPstate(String pstate) {
		this.pstate = pstate;
	}

	public void setFrgnrPn(String frgnrPn) {
		this.frgnrPn = frgnrPn;
	}

	public void setUip(String uip) {
		this.uip = uip;
	}

	public void setSys_udate(String sys_udate) {
		this.sys_udate = sys_udate;
	}

	public void setMobileFn(String mobileFn) {
		this.mobileFn = mobileFn;
	}

	public void setMobileMn(String mobileMn) {
		this.mobileMn = mobileMn;
	}

	public void setMobileRn(String mobileRn) {
		this.mobileRn = mobileRn;
	}

	public void setMail(String mail) {
		this.mail = mail;
	}

	public void setVdateStart(String vdateStart) {
		this.vdateStart = vdateStart;
	}

	public void setVdateEnd(String vdateEnd) {
		this.vdateEnd = vdateEnd;
	}

	public void setLang_type(String lang_type) {
		this.lang_type = lang_type;
	}

	public void setSstate(String sstate) {
		this.sstate = sstate;
	}

	public void setAgentId(String agentId) {
		this.agentId = agentId;
	}

	public void setMobileNo(String mobileNo) {
		this.mobileNo = mobileNo;
	}

	public void setSearchGbn(String searchGbn) {
		this.searchGbn = searchGbn;
	}

	public void setSearchName(String searchName) {
		this.searchName = searchName;
	}

	public void setCntpntShopId(String cntpntShopId) {
		this.cntpntShopId = cntpntShopId;
	}

	public void setUsrId(String usrId) {
		this.usrId = usrId;
	}

	public void setAgentCode(String agentCode) {
		this.agentCode = agentCode;
	}

	public void setEtcFileName(String etcFileName) {
		this.etcFileName = etcFileName;
	}

	public void setEtcFileMask(String etcFileMask) {
		this.etcFileMask = etcFileMask;
	}

	public String getFileOx() {
		return fileOx;
	}

	public String getEtcFileOx() {
		return etcFileOx;
	}

	public void setFileOx(String fileOx) {
		this.fileOx = fileOx;
	}

	public void setEtcFileOx(String etcFileOx) {
		this.etcFileOx = etcFileOx;
	}

	public List<SucVo> getItems() {
		return items;
	}

	public void setItems(List<SucVo> items) {
		this.items = items;
	}

	
}
