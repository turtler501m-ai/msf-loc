package com.ktis.msp.voc.giftvoc.vo;

import com.ktis.msp.base.mvc.BaseVo;

public class GiftVocVo extends BaseVo {

    private static final long serialVersionUID = 1L;

    private String giftVocSeq;
    private String contractNum;
    private String custNm;
    private String promNm;
    private String giftType;
    private String giftAmt;
    private String vocStat;
    private String vocCtn;
    private String divisn;
    private String rmk;
    private String vocMngOrgn;
    private String vocType;
    private String vocReqDt;
    private String userId;
    private String recommId;
    private String recommContractNum;
    private String extnlOrgnRefYn;
    private String vocDesc;
    private String regstDttm;
    private String regstId;
    private String regstNm;
    private String rvisnDttm;
    private String rvisnId;
    private String rvisnNm;
    private String ansProcDttm;
    private String ansProcId;
    private String ansProcNm;
    private String ansRmk;
    private String ansPayRsn;
    private String ansPayItem;
    private String ansPayAmt;
    private String ansPayPlanMon;
    private String ansStat;

    private String subLinkName;
    private String subscriberNo;
    private String lstComActvDate;
    private String reqInDay;
    private String fstRateNm;
    private String lstRateNm;
    private String subStatus;
    private String recommendInfo;
    private String onOffType;
    private String openMarketReferer;
    private String maskingYn;

    // 검색조건
    private String srchStrtDt;
    private String srchEndDt;
    private String procStrtDt;
    private String procEndDt;
    private String planStrtDt;
    private String planEndDt;
    private String searchGbn;
    private String searchName;

    public String getGiftVocSeq() {
        return giftVocSeq;
    }

    public void setGiftVocSeq(String giftVocSeq) {
        this.giftVocSeq = giftVocSeq;
    }

    public String getContractNum() {
        return contractNum;
    }

    public void setContractNum(String contractNum) {
        this.contractNum = contractNum;
    }

    public String getCustNm() {
        return custNm;
    }

    public void setCustNm(String custNm) {
        this.custNm = custNm;
    }

    public String getPromNm() {
        return promNm;
    }

    public void setPromNm(String promNm) {
        this.promNm = promNm;
    }

    public String getGiftType() {
        return giftType;
    }

    public void setGiftType(String giftType) {
        this.giftType = giftType;
    }

    public String getGiftAmt() {
        return giftAmt;
    }

    public void setGiftAmt(String giftAmt) {
        this.giftAmt = giftAmt;
    }

    public String getVocStat() {
        return vocStat;
    }

    public void setVocStat(String vocStat) {
        this.vocStat = vocStat;
    }

    public String getVocCtn() {
        return vocCtn;
    }

    public void setVocCtn(String vocCtn) {
        this.vocCtn = vocCtn;
    }

    public String getDivisn() {
        return divisn;
    }

    public void setDivisn(String divisn) {
        this.divisn = divisn;
    }

    public String getRmk() {
        return rmk;
    }

    public void setRmk(String rmk) {
        this.rmk = rmk;
    }

    public String getVocMngOrgn() {
        return vocMngOrgn;
    }

    public void setVocMngOrgn(String vocMngOrgn) {
        this.vocMngOrgn = vocMngOrgn;
    }

    public String getVocType() {
        return vocType;
    }

    public void setVocType(String vocType) {
        this.vocType = vocType;
    }

    public String getVocReqDt() {
        return vocReqDt;
    }

    public void setVocReqDt(String vocReqDt) {
        this.vocReqDt = vocReqDt;
    }

    public String getUserId() {
        return userId;
    }

    public void setUserId(String userId) {
        this.userId = userId;
    }

    public String getRecommId() {
        return recommId;
    }

    public void setRecommId(String recommId) {
        this.recommId = recommId;
    }

    public String getRecommContractNum() {
        return recommContractNum;
    }

    public void setRecommContractNum(String recommContractNum) {
        this.recommContractNum = recommContractNum;
    }

    public String getExtnlOrgnRefYn() {
        return extnlOrgnRefYn;
    }

    public void setExtnlOrgnRefYn(String extnlOrgnRefYn) {
        this.extnlOrgnRefYn = extnlOrgnRefYn;
    }

    public String getVocDesc() {
        return vocDesc;
    }

    public void setVocDesc(String vocDesc) {
        this.vocDesc = vocDesc;
    }

    public String getRegstDttm() {
        return regstDttm;
    }

    public void setRegstDttm(String regstDttm) {
        this.regstDttm = regstDttm;
    }

    public String getRegstId() {
        return regstId;
    }

    public void setRegstId(String regstId) {
        this.regstId = regstId;
    }

    public String getRvisnDttm() {
        return rvisnDttm;
    }

    public void setRvisnDttm(String rvisnDttm) {
        this.rvisnDttm = rvisnDttm;
    }

    public String getRvisnId() {
        return rvisnId;
    }

    public void setRvisnId(String rvisnId) {
        this.rvisnId = rvisnId;
    }

    public String getAnsProcDttm() {
        return ansProcDttm;
    }

    public void setAnsProcDttm(String ansProcDttm) {
        this.ansProcDttm = ansProcDttm;
    }

    public String getAnsProcId() {
        return ansProcId;
    }

    public void setAnsProcId(String ansProcId) {
        this.ansProcId = ansProcId;
    }

    public String getAnsRmk() {
        return ansRmk;
    }

    public void setAnsRmk(String ansRmk) {
        this.ansRmk = ansRmk;
    }

    public String getAnsPayRsn() {
        return ansPayRsn;
    }

    public void setAnsPayRsn(String ansPayRsn) {
        this.ansPayRsn = ansPayRsn;
    }

    public String getAnsPayItem() {
        return ansPayItem;
    }

    public void setAnsPayItem(String ansPayItem) {
        this.ansPayItem = ansPayItem;
    }

    public String getAnsPayAmt() {
        return ansPayAmt;
    }

    public void setAnsPayAmt(String ansPayAmt) {
        this.ansPayAmt = ansPayAmt;
    }

    public String getAnsPayPlanMon() {
        return ansPayPlanMon;
    }

    public void setAnsPayPlanMon(String ansPayPlanMon) {
        this.ansPayPlanMon = ansPayPlanMon;
    }

    public String getAnsStat() {
        return ansStat;
    }

    public void setAnsStat(String ansStat) {
        this.ansStat = ansStat;
    }

    public String getSrchStrtDt() {
        return srchStrtDt;
    }

    public void setSrchStrtDt(String srchStrtDt) {
        this.srchStrtDt = srchStrtDt;
    }

    public String getSrchEndDt() {
        return srchEndDt;
    }

    public void setSrchEndDt(String srchEndDt) {
        this.srchEndDt = srchEndDt;
    }

    public String getProcStrtDt() {
        return procStrtDt;
    }

    public void setProcStrtDt(String procStrtDt) {
        this.procStrtDt = procStrtDt;
    }

    public String getProcEndDt() {
        return procEndDt;
    }

    public void setProcEndDt(String procEndDt) {
        this.procEndDt = procEndDt;
    }

    public String getPlanStrtDt() {
        return planStrtDt;
    }

    public void setPlanStrtDt(String planStrtDt) {
        this.planStrtDt = planStrtDt;
    }

    public String getPlanEndDt() {
        return planEndDt;
    }

    public void setPlanEndDt(String planEndDt) {
        this.planEndDt = planEndDt;
    }

    public String getSearchGbn() {
        return searchGbn;
    }

    public void setSearchGbn(String searchGbn) {
        this.searchGbn = searchGbn;
    }

    public String getSearchName() {
        return searchName;
    }

    public void setSearchName(String searchName) {
        this.searchName = searchName;
    }

    public String getRegstNm() {
        return regstNm;
    }

    public void setRegstNm(String regstNm) {
        this.regstNm = regstNm;
    }

    public String getRvisnNm() {
        return rvisnNm;
    }

    public void setRvisnNm(String rvisnNm) {
        this.rvisnNm = rvisnNm;
    }

    public String getAnsProcNm() {
        return ansProcNm;
    }

    public void setAnsProcNm(String ansProcNm) {
        this.ansProcNm = ansProcNm;
    }

    public String getSubLinkName() {
        return subLinkName;
    }

    public String getReqInDay() {
        return reqInDay;
    }

    public void setReqInDay(String reqInDay) {
        this.reqInDay = reqInDay;
    }

    public void setSubLinkName(String subLinkName) {
        this.subLinkName = subLinkName;
    }

    public String getSubscriberNo() {
        return subscriberNo;
    }

    public void setSubscriberNo(String subscriberNo) {
        this.subscriberNo = subscriberNo;
    }

    public String getLstComActvDate() {
        return lstComActvDate;
    }

    public void setLstComActvDate(String lstComActvDate) {
        this.lstComActvDate = lstComActvDate;
    }

    public String getFstRateNm() {
        return fstRateNm;
    }

    public void setFstRateNm(String fstRateNm) {
        this.fstRateNm = fstRateNm;
    }

    public String getLstRateNm() {
        return lstRateNm;
    }

    public void setLstRateNm(String lstRateNm) {
        this.lstRateNm = lstRateNm;
    }

    public String getRecommendInfo() {
        return recommendInfo;
    }

    public void setRecommendInfo(String recommendInfo) {
        this.recommendInfo = recommendInfo;
    }

    public String getSubStatus() {
        return subStatus;
    }

    public void setSubStatus(String subStatus) {
        this.subStatus = subStatus;
    }

    public String getOnOffType() {
        return onOffType;
    }

    public void setOnOffType(String onOffType) {
        this.onOffType = onOffType;
    }

    public String getOpenMarketReferer() {
        return openMarketReferer;
    }

    public void setOpenMarketReferer(String openMarketReferer) {
        this.openMarketReferer = openMarketReferer;
    }

    public String getMaskingYn() {
        return maskingYn;
    }

    public void setMaskingYn(String maskingYn) {
        this.maskingYn = maskingYn;
    }
}
