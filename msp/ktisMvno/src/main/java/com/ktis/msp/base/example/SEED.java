...
// reorder for little endian	  
	...
	public byte[] Encrypt(String sPlain, String key){
		//byte pbCipher[]   = new byte[16];		//
		byte pbCipher[]   = new byte[key.length()];		//
		
		try{
			int pdwRoundKey[] = new int[32];
			
			this.SeedRoundKey(pdwRoundKey, key.getBytes());	//SeedRoundKey
			
			//System.out.print("ROUND KEY   byte[]=[");
			//for (int i=0; i<32; i++){
			for (int i=0; i<pdwRoundKey.length; i++){
				//System.out.print(Integer.toHexString(0xff&pdwRoundKey[i])+" ");
			}
			//System.out.print("]\n");
			
			
			this.SeedEncrypt(sPlain.getBytes(), pdwRoundKey, pbCipher);
	...
	public byte[] Decrypt(byte pbCipher[], String key){
		...
		try{
			...
			this.SeedRoundKey(pdwRoundKey, key.getBytes());	//SeedRoundKey 
			this.SeedDecrypt(pbCipher, pdwRoundKey, pbPlain);
	...
	// 16진수 바이트 변환 
	public byte[] GetStringHextoByte(String sHexCipher){
		...
		try{
			...
			for(int i= 0 ; i < bCipher.length;i++){
				String aHex = sHexCipher.substring(i==0?0:i*2,   i==0?2:(i*2)+2 );
				//System.out.print(aHex + " " );
				//--------------------------------------------------//
				// 
				bCipher[i] =(byte)(0xff&Integer.parseInt(aHex,16));
				//--------------------------------------------------//
			}//	System.out.println("]");
	...
	// 자릿수 채우기 Left Padding
	public String appendLPad(String str, int precision, String padChar) {
		String pramStr = str;
		if (str == null) {
			pramStr = "";
		}
		//String Paddingstr = pramStr;
		StringBuilder Paddingstr = new StringBuilder(pramStr);
		int strlen = pramStr.getBytes().length;
		
		if (strlen >= precision){
			strlen = precision;
			return Paddingstr.substring(0, precision).toString();
		}
		int newCount =0;
		
		while ((newCount + strlen) < precision) {
			Paddingstr =  new StringBuilder( padChar);
			Paddingstr.append(Paddingstr);
			newCount++;
		}
		String returnResult = Paddingstr.toString();
		//logger.info("appendPadding Paddingstr : len[" + Paddingstr.getBytes().length +"]["+ Paddingstr +"]");
		return returnResult;
	}
