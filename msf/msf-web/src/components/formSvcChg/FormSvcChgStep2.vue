        ...
        <section v-if="hasOption('RATE_CHANGE')" class="product-card md:col-span-2">
          ...
          <div class="space-y-4">
            ...
            <div class="product-form-row">
              ...
              <div class="product-row-input">
                <select v-model="productForm.recommendedRatePlan" class="product-select w-full max-w-xs">
            ...
            <div class="product-form-row">
              <span class="product-row-label"></span>
              <div class="product-row-input">
                <select v-model="productForm.ratePlanSearchResult" class="product-select w-full max-w-md">
...
async function onPauseApply() {
  ...
  pauseErrorMsg.value = ''
  try {
    const res = await applyPause({ ncn: cf.ncn, ctn: cf.phone, custId: cf.custId, password: pausePassword.value })
    if (res && res.RESULT_CODE === 'SUCCESS') {
...
watch(canCheckPause, (v) => {
  if (!v) { pauseInfo.value = null; pauseErrorMsg.value = '' }
})
onMounted(async () => {
...
const save = async () => {
  ...
  if (hasOption('RATE_CHANGE')) {
    ...
    if (!soc) return { valid: false, message: '요금상품 변경: 변경할 요금제를 선택해 주세요.' }
    if (!productForm.value.rateChangeSchedule) return { valid: false, message: '요금상품 변경: 변경일시(즉시/예약)를 선택해 주세요.' }
  }
