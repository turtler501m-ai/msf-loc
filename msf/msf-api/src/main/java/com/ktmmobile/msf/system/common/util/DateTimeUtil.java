...
public class DateTimeUtil {
  ...
  /**
   * 특정값이 시작일시와 종료일시 사이의 날짜인지 비교
   * @param startDateTime : 2022-01-31 00:00:00
   * @param endDateTime : 9999-12-31 23:59:59
   * @param middleDateTime : 2025-08-06 12:00:00
   * @return boolean
   */
  public static boolean isMiddleDateTime(String startDateTime, String endDateTime, String middleDateTime){
    ...
    LocalDateTime endDttm = LocalDateTime.parse(endDateTime, formatter);
    LocalDateTime middleDttm = LocalDateTime.parse(middleDateTime, formatter);

    return !(middleDttm.isBefore(startDttm) || middleDttm.isAfter(endDttm));
  }
    ...
    public static boolean isExceeded(LocalDateTime dateTime, ChronoUnit unit, long amount) {
        LocalDateTime now = LocalDateTime.now();
        long diff = unit.between(dateTime, now);
        return diff > amount;
    }

    /**
     * 기준날짜(openingDate) +  추가할날짜(moreDay)를 현재날짜와 비교하여 보다 이전이면 false
     * @param openingDate : 예시) 20250505
     * @param moreDay : 예시) 5
     * @return boolean
     */
