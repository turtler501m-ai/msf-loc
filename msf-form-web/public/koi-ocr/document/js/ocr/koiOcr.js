...
class KoiOcr extends EventTarget {
  ...
  _initEventHandler() {
    if (this.useWebCamera) {
      ...
      this._webCamera.addEventListener('imagecapturedonly', async (event) => {
        this.dispatchCaptureEvent(event)
        if (await this.processImage(event.detail)) {
          event.preventDefault()
        }
      ...
      this._webCamera.addEventListener('workerPass', async (event) => {
        ...
        eventType = await this.processOCRResult(
          result,
          eventDetail,
          cropData,
          rtcToken,
    ...
    window.addEventListener('imageProcessed', async (event) => {
      this.dispatchCaptureEvent(event)
      const { cropData, originData, ocrType, rtcToken, detectTime, resultCode, timeout } =
        event.detail
      let eventType = 'result'
      const eventDetail = {
        success: false,
      ...
      if (this._useDemo) {
        if (
          await this.sendToOcrWorkerDemo(
            cropData,
            eventDetail,
            ocrType,
            rtcToken,
  ...
  async processImage(data) {
    ...
    try {
      ...
      if (imageData && this._useCapOcr != 4) {
        eventType = await this.processOCR(
          imageData,
          eventDetail,
          this._ocrType,
          rtcToken,
  ...
  async processOCR(
    ...
    return await this.sendToOcrWorker(
      imageData,
      eventDetail,
      ocrType,
      rtcToken,
  ...
  async processOCRResult(
    ...
    if (this._ocrType == 16 || this._ocrType == 27) {
      return this._handleOcrType16Result(
        eventDetail,
        resultCode,
        detectTime,
        rtcToken,
