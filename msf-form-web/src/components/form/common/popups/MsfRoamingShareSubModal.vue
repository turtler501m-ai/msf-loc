  ...
  >
    ...
    <template #footer>
      <MsfButtonGroup>
        ...
        <MsfButton variant="primary" :disabled="isCheckingMainProduct" @click="onConfirm">확인</MsfButton>
      </MsfButtonGroup>
    </template>
  </MsfDialog>
</template>
...
const getSavedMainNumber = () => {
  ...
  return normalizePhone(isSub2.value ? parts[2] : parts[0]) || normalizePhone(props.settingData?.mainNumber)
}
const getSavedMainProductSeq = (phone) => {
  ...
  if (!savedPhone || savedPhone !== phone) return ''
  return String(props.settingData?.mainProductSeq || props.settingData?.shareMainProdHstSeq || '').replace(/\D/g, '')
}
...
const getMainProductCode = () => {
  const subCode = String(props.settingData?.rateCd || props.settingData?.value || props.settingData?.soc || '').toUpperCase()
...
const initializeFromSettingData = () => {
  ...
  const { ftrNewParam } = props.settingData
  if (ftrNewParam) {
  ...
  if (!formData.mainNumber) {
    formData.mainNumber = normalizePhone(props.settingData?.mainNumber)
  }
}
...
const validatePhone = () => {
  ...
  if (!isValidMobileNumber(phone)) return '유효한 휴대폰번호를 입력해 주세요.'
  return ''
}
