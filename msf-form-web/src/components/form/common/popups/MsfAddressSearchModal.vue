...
const onSearch = async () => {
  ...
  try {
    const { data } = await post('/addrlink/addrLinkApi', {
      params: {
    ...
    if (resultsData.common.errorCode === '0') {
      totalCount.value = parseInt(resultsData.common.totalCount)
      results.value = resultsData.juso.map((item) => ({
        zipNo: item.zipNo,
        roadAddr: item.roadAddr,
        jibunAddr: item.jibunAddr,
      }))
    } else {
      console.error('Juso API Error:', resultsData.common.errorMessage)
      results.value = []
      totalCount.value = 0
    }
  } catch (e) {
