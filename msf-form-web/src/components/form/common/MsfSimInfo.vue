...
const validate = () => {
  if (!formData.hasSim) return false
  if (!formData.usimKindsCd) return false
  if (!formData.reqUsimSn) return false
  if (!formData.simPurchaseMethod) return false
  if (!formData.prodNm) return false
  if (!formData.eid) return false
  if (!formData.imei1) return false
  if (!formData.imei2) return false
  return true
}
