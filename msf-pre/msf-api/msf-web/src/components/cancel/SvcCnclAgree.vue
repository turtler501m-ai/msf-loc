<template>
  <div class="page-step-panel space-y-6">
    ...
    <!-- 서명 (UI 설계서 S104030101: 가입자(대리인) 서명, eFormSign 연동 예정) -->
...
const validate = async () => {
  if (!benefitAgreed.value) return { valid: false, message: '혜택 소멸사항 동의에 체크해 주세요.' }
  if (!signAgreed.value) return { valid: false, message: '서명을 완료해 주세요.' }
  return { valid: true, message: '' }
}

/** 작성 완료 시 호출 (McpStepService에서 호출, UI 설계서 S104030101) */
const save = async () => {
