      ...
      <div v-if="openConfirmPopup" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50" @click.self="openConfirmPopup = false">
        <div class="ident-popup bg-white rounded-lg shadow-xl max-w-lg w-full mx-4 max-h-[90vh] overflow-auto">
          ...
          <div class="p-4 border-t border-gray-200 flex justify-end gap-2">
            ...
            <button type="button" class="ident-btn-primary" @click="confirmPopupOk">확인</button>
          </div>
        </div>
      </div>
    </Teleport>
...
onMounted(() => emit('update:complete', isComplete.value))

const validate = async () => {
  const g = guideChecks.value
  if (!g.recordRefuse || !g.nameLending || !g.fraudPrevent || !g.mSafer || !g.sellerConfirm) {
    return { valid: false, message: '고객 안내 사항을 모두 확인하고 동의해 주세요.' }
  }
  if (!signAgreed.value) return { valid: false, message: '서명을 완료해 주세요.' }
  return { valid: true, message: '' }
}

/** 작성 완료 시 호출 (McpStepService에서 호출, UI 설계서 S103030101) */
const save = async () => {
  ...
  try {
    ...
    const msg = e?.message || '신청서 등록이 실패하였습니다. 다시 시도해 주세요.'
    alert(msg)
  }
}
