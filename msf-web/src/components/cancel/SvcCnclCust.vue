<template>
  <div class="page-step-panel space-y-6">
    ...
    <!-- 20260310: 가입유형 선택(대리점) — 고객정보 하단 배치 -->
    <section class="mt-8 pt-6 border-t border-gray-200">
      <h4 class="text-lg font-semibold mb-3">가입유형 선택</h4>
      <p class="text-sm text-gray-600 mb-3">대리점을 선택한 후 다음 단계로 진행해 주세요.</p>
      <div>
...
async function doPhoneAuth() {
  ...
  try {
    ...
    if (data && data.success !== false) {
      ...
      // UI 설계서 S104010101: 인증 성공 메시지
      alert('휴대폰번호 인증이 완료되었습니다.')
    } else {
      // UI 설계서 S104010101: 인증 실패 메시지
...
const validate = async () => {
  if (!form.value.agencyCode) return { valid: false, message: '대리점을 선택해 주세요.' }
