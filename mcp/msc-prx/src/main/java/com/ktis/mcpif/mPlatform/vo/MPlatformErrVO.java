...
public class MPlatformErrVO implements Serializable {
  ...
  public MPlatformErrVO() {
  }

  public MPlatformErrVO(String resNo, String prgrStatCd) {
    this.resNo = resNo;
  ...
  public void setErrInfo(Exception exception){
    ...
    if(errType != null && 100 < errType.length()) errType = errType.substring(0, 100);
    this.errType = errType;

    String errMsg = exception.getMessage();
    if(errMsg != null && 1000 < errMsg.length()) errMsg = errMsg.substring(0, 1000);
    this.errMsg= errMsg;
  }
