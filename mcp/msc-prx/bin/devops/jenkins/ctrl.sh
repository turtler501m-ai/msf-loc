...
INST_NAME_LIST=("mcpadmin" "mcpapi" "mcpprx" "mcpwas" "mcpwaswidget" "prx")
ST_LIST=("0" "1")
ENV_FILE=env.sh
APP_NAME="ctrl.sh"
DATE=`date +%Y%m%d%H%M%S`

JAVA_HOME=/home/jboss/appsw/jdk1.6.0_45
...
#        sleep 3;
        echo "[ INFO ] ${SERVER_NAME} is shutdown now."
        kill -9 ${status};
    fi
    local status=$(func_check ${VAL1} ${VAL2} "${VAL4}" "=$SERVER_NAME " "")
    if [[ -z ${status} ]]; then
...
#        nohup $JBOSS_HOME/bin/standalone.sh -DSERVER=$SERVER_NAME -P=${PROPFILE} -c $CONFIG_FILE >> ${FILENM}  2>&1 &
        # jboss-as-7
        echo "[ INFO ] nohup $JBOSS_HOME/bin/standalone.sh -b $DOMAIN_IP -u $MULTICAST_IP >> ${FILENM} 2>&1 &"
        nohup $JBOSS_HOME/bin/standalone.sh -b $DOMAIN_IP -u $MULTICAST_IP >> ${FILENM} 2>&1 &
        sleep 3
    else
