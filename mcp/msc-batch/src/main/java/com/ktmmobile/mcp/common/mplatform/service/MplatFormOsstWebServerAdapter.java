...
public class MplatFormOsstWebServerAdapter {
	...
	public boolean callService(HashMap<String,String> param, CommonXmlVO vo, int timeout) throws SelfServiceException, SocketTimeoutException {
		...
		try {
			...
			if("LOCAL".equals(serverName)) {
				...
				if(EVENT_CODE_REPLACE_USIM_RCP_SEND.equals(appEventCd)){
					responseXml = this.getResXmlMessageT02();
				}else{
					vo.setSuccess(true);
					result= true;
					return result;
	...
	private String getResXmlMessageT02() {
		...
		selfStringBuffer.append("          <appEventCd>TO2</appEventCd>");
