...
/*        if($('#eventBranch').val() == 'E'){
    ...
    var setBannerTemplate = function(obj,targetObj,index) {
        ...
        if(gubun == 'E'){
            arr.push(`<a class='rate-banner__content' href='/m/appForm/eSimSelfView.do?rateCd=${rateAdsvcCd}' title='${obj.rateAdsvcGdncBas.rateAdsvcNm} 페이지 이동'>\n`);
       }else{
            arr.push(`<a class='rate-banner__content' href='/m/appForm/appFormDesignView.do?rateCd=${rateAdsvcCd}&onOffType=7' title='${obj.rateAdsvcGdncBas.rateAdsvcNm} 페이지 이동'>\n`);
       }
        ...
        if (obj.rateGdncEffPriceDtl?.useYn === "Y") {
            ...
            arr.push(`      <button class="perceived-price-btn" role="button" data-tpbox="#${tooltipId}" onclick="event.stopPropagation(); event.preventDefault();">`);
            arr.push(`        <b>체감가</b><i class="c-icon c-icon--tooltip" aria-hidden="true"></i>`);
            arr.push(`      </button>`);
            arr.push(`      <div class="c-tooltip-box" id="${tooltipId}" onclick="event.stopPropagation(); event.preventDefault();">`);
...
function goView(ntcartSeq, sbstCtg){
    var url = "/m/event/eventDetail.do?ntcartSeq=" + ntcartSeq + "&sbstCtg=" + sbstCtg;
    location.href = url;
}
        ...
        ajaxCommon.getItemNoLoading({
            ...
            ,function(jsonObj){
                if (jsonObj.rateAdsvcGdncBas !== null ) {
                    if(move == 'M'){
                        location.href = "/m/rate/rateList.do?eventRatePop=Y&rateAdsvcGdncSeq="+jsonObj.rateAdsvcGdncBas.rateAdsvcGdncSeq+"&rateAdsvcCd="+rateCode;
                    }else{
                        openPage('pullPopup', '/m/rate/rateLayer.do?rateAdsvcGdncSeq='+jsonObj.rateAdsvcGdncBas.rateAdsvcGdncSeq+'&rateAdsvcCd='+rateCode+'', '');
                    }
                }
            });
    }
