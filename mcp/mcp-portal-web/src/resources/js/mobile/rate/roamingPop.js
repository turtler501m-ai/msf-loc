...
// 신청번호 유효성
$(".roamPhone").on('input keyup',(function() {
    $("#checkBtn").prop("disabled", true);

    var roamDiv = $('.grpRoamNum');

    if(roamDiv.next().hasClass('c-form__text')) {
        roamDiv.next().remove();
    }
    roamDiv.removeClass("has-error");

    var roamPhone = $("#roamPhone1").val() + $("#roamPhone2").val() + $("#roamPhone3").val();
    var regPhone = /^01([0|1|6|7|8|9])-?([0-9]{3,4})-?([0-9]{4})$/;

    //신청번호 11자리 아니면 false
    if(roamPhone.length != 11) {
        return false;
    }
    //유효한 번호 형태가 아니면 false
    if(!regPhone.test(roamPhone)) {
        if(!roamDiv.next().hasClass('c-form__text')) {
            viewErrorMgs(roamDiv.find('input').first(), "유효한 번호가 아닙니다.<br>확인 후 다시 입력해주세요.");
            return false;
        }
    }
    btnChk();
...
$("#checkBtn").click(function (){
    ...
    roamingCom(ctn, paramPhone);
});

function roamingCom(ctn, paramPhone){
    var lineType = $("#lineType").val();

    KTM.Confirm("해외 로밍 상품을 신청하시겠습니까?", function () {
            ...
            , success: function (rtnObj) {
                if (rtnObj.rtnCd == "0000") {
                    ...
                    $.ajax({
                        ...
                        , success: function (jsonObj) {
                            if (jsonObj.returnCode == "00") {
                                MCP.alert("해외 로밍 신청이 완료되었습니다.", function () {
                                    $('.c-icon--close').trigger('click');
                                    location.href = '/m/rate/roamingList.do';
                                });
                            } else {
                                if(jsonObj.RESULT_MSG) {
                                    MCP.alert(jsonObj.RESULT_MSG);
                                }else if(jsonObj.message) {
                                    MCP.alert(jsonObj.message, function(){
                                        viewErrorMgs($('#' + returnCode[jsonObj.returnCode]), rtnObj.rtnMsg);
                                        $('#' + returnCode[jsonObj.returnCode]).focus();
                                    });
                                }
                            }
                    ...
                    MCP.alert(rtnObj.rtnMsg, function () {
                        if(rtnObj.rtnCd == '0014'){
                            viewErrorMgs($("#addPhone1_1"), rtnObj.rtnMsg);
                            $("#txtPhoneF").focus(); // 포커스
                        }else{
                            viewErrorMgs($("#roamPhone1"), rtnObj.rtnMsg);
                            $("#txtPhoneF").focus(); // 포커스
                        }
                    });
                }
