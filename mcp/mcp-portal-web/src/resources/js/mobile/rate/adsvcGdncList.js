...
var initXmlData = function() {
    return new Promise(function(resolve, reject) {
        $.ajax({
            type : 'GET'
            , url: '/m/rate/adsvcGdncListAjax.do'
            , dataType : 'json'
...
function mainTabAllClick(){
    ...
    var varData = ajaxCommon.getSerializedData({
     ...
     url = "/m/rate/adsvcAllListPanel.do";
    //페이지 호출
...
function mainTabClickCallback(resp){
    $('#listPanel').html(resp);
    $('#totalNum').html('전체 개수 : ' + $('#accordionB > li').length + '개');
}
    ...
    ,function(data){
        if(data.isLogin == false){
            KTM.Confirm('M모바일을 사용하시는<br>회원 전용 서비스 입니다.<br><br>로그인 후 이용 하시겠습니까?', function() {
                ...
                location.href = '/m/loginForm.do?redirectUrl=' + redirectUrl;
            });
        } else {
            if(data.userDivision == "00"){
                ...
                //부가서비스 신청 팝업
                openPage('pullPopup','/m/rate/adsvcJoinPop.do',parameterData, 1);
