...
$(document).ready(function (){
    ...
    window.onpopstate = function (event){
        ...
        history.go("/m/mypage/chargeView05.do");
    }
    ...
    $("#reqPayType").change(function(){
        var val = $("#reqPayType option:selected").val();
        if(val=="D"){
            $("#bank,#auth").show();
    ...
    $("#cardCheckBtn").click(function(){
        ...
        if(cardCode==""){
            MCP.alert("카드사를 입력해 주세요.",function(){
                $("#cardCode").focus();
            });
            return false;
        }
    ...
    $("#blBankCode,#cycleDueDay,#chkAgree").change(function(){
        isChangeBtnDisable();
...
function initData(){
    ...
    $("#auth_credit").text("신용카드 인증").prop("disabled",true);
...
function niceAuthResult(prarObj){
    ...
    var authInfoJson= {cstmrName: cstmrName , cstmrNativeRrn: cstmrNativeRrn01 ,contractNum : contractNum};
    var isAuthDone = diAuthCallback(authInfoJson);

    if(isAuthDone){
        if ("C" == prarObj.authType) {
            $("._btnNiceAuthBut").eq(1).addClass("is-active ");
            $("._btnNiceAuthBut").eq(1).html('<i class="c-icon c-icon--card" aria-hidden="true"></i>신용카드<br/> 인증완료');
            $("._btnNiceAuth").eq(0).prop('disabled', 'true');
        } else {
            $("._btnNiceAuthBut").eq(0).addClass("is-active ");
            $("._btnNiceAuthBut").eq(0).html('<i class="c-icon  c-icon--phone" aria-hidden="true"></i>휴대폰<br/>인증완료');
