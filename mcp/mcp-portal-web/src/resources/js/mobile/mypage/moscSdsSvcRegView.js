...
function chgChk(obj){
    if($(obj).is(":checked")){
        ...
        if($(obj).data("isdlvry") == 'Y'){
            setInputFilter(document.getElementById("inpTel2"), function(value) {
...
function targetTermsAgree(){
    ...
    $("#btnSmsAuth").prop("disabled",false);
...
function agreeChk(obj){
    ...
    if(!$("#chkStepAgree1").prop("checked") || !$("#chkStepAgreeAll").prop("checked")){
        $("#btnSmsAuth").prop("disabled",true);
    }else{
        $("#btnSmsAuth").prop("disabled",false);
    }
...
function authSms(){
    ...
    if ("Y" ==  $("input[name='chkSelectBasicService']:checked").data("isdlvry") ) {
        ...
        if($("#inpOrdererName").val() == ''){
            ...
            MCP.alert('받으시는 분 이름을  입력해 주세요.', function (){
                $("#inpOrdererName").focus();
            });
            return false;

        }
        if($("#inpTel2").val() == '' || $("#inpTel2").val().length < 11){

            MCP.alert('배송 받을 휴대전화 번호를 입력해 주세요.', function (){
                $("#inpTel2").focus();
            });
            return false;
        }
        if($("#inpAdressNum").val() == ''){
            MCP.alert('배송지 우편번호를 입력해 주세요.', function (){
                $("#btnSearchAddrPost").focus();
            });
            return false;
        }
        if($("#inpAdress1").val() == ''){
            MCP.alert('배송지 주소를 설정 하시기 바랍니다.', function (){
                $("#btnSearchAddrPost").focus();
            });
            return false;
        }
...
function goComplete(){
    ...
    if ( "Y" ==  $("input[name='chkSelectBasicService']:checked").data("isdlvry") ) {
        ...
        if($("#inpTel2").val() == '' || $("#inpTel2").val() < 11){
            $("#inpTel2").focus();
            MCP.alert("배송 받을 휴대전화 번호를 입력해 주세요.");
            return;
        }
        if($("#inpAdressNum").val() == ''){
            $("#inpAdressNum").focus();
        ...
        var inpOrdererName = $("#inpOrdererName").val();
        var inpOrdererTel1 = $.trim($("#inpTel2").val().substring(0,3));
        var inpOrdererTel2 = $.trim($("#inpTel2").val().substring(3,7));
        var inpOrdererTel3 = $.trim($("#inpTel2").val().substring(7,11));
        var inpAdressNum = $("#inpAdressNum").val();
        var inpAdress1 = $("#inpAdress1").val();
        var inpAddress2 = $("#inpAddress2").val();
    }
        ...
        var varData = ajaxCommon.getSerializedData({
            ...
            , dlvryName:inpOrdererName
            , dlvryMobileFn:inpOrdererTel1
            , dlvryMobileMn:inpOrdererTel2
            , dlvryMobileRn:inpOrdererTel3
            , dlvryPost:inpAdressNum
            , dlvryAddr:inpAdress1
            , dlvryAddrDtl:inpAddress2
