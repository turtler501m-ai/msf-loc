...
function initAgreementHandlers() {
    //개별 동의 체크 시 스크롤 확인
    $('.acc-agree .c-checkbox._agree').on('change', function () {
        const checkbox = $(this);
        const modalId = checkbox.closest('.c-agree__item').find('[data-dialog-trigger]').data('dialog-trigger');
    ...
    $('#btnStplAllCheck').on('change', function () {
        ...
        if (isChecked) {
            ...
            $('.acc-agree .c-checkbox._agree').prop('checked', false).trigger('change');
        }
    });

    //개별 다 체크항목 다 체크 시
    $('.acc-agree .c-checkbox._agree').on('change', function () {
        const allChecked = $('.acc-agree .c-checkbox._agree').length === $('.acc-agree .c-checkbox._agree:checked').length;
...
function pop_nice() {
    ...
    openPage('outLink2', '/nice/popNiceAuth.do?sAuthType=M&mType=Mobile', { width: '500', height: '700' });
}

/** 신용카드 인증창 요청 */
function pop_credit() {
    pageObj.niceType = NICE_TYPE.CUST_AUTH;
    openPage('outLink2', '/nice/popNiceAuth.do?sAuthType=C&mType=Mobile', { width: '500', height: '700' });
}
...
function fnNiceCert(obj) {
    ...
    if (isAuthDone) {
        ...
        btns.eq(isCard ? 1 : 0).addClass("is-active")
            .html(`<i class="c-icon ${isCard ? 'c-icon--card' : 'c-icon--phone'}" aria-hidden="true"></i>${isCard ? '신용카드 인증완료' : '휴대폰<br>인증완료'}`);
