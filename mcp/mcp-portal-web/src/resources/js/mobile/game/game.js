...
function setup() {
  frameRate(60);
  var gameCanvas = createCanvas(100, 100);
  gameCanvas.parent('game_canvas');
  noLoop();
}

function preload() {
  ns.vars.resource.cloud = loadImage('../../resources/images/mobile/game/bg.jpg');
  ns.vars.resource.road = loadImage('../../resources/images/mobile/game/road.jpg');
  ns.vars.resource.playerIdle = loadImage('../../resources/images/mobile/game/player.png');
  ns.vars.resource.playerJump = loadImage('../../resources/images/mobile/game/jump.png');

  ns.vars.resource.obstacle1 = loadImage('../../resources/images/mobile/game/obstacle.jpg');
  ns.vars.resource.obstacle2 = loadImage('../../resources/images/mobile/game/obstacle2.jpg');
  ns.vars.resource.obstacle3 = loadImage('../../resources/images/mobile/game/obstacle3.jpg');
}
...
function windowResized() {
  if (ns.vars.el) {
    var rect = ns.vars.el.getBoundingClientRect();
    resizeCanvas(Math.min(400, rect.width), rect.height);
  }
}
...
ns.init = function (el) {
  ...
  ns.vars.el.addEventListener('touchstart', ns.startJump);
...
ns.start = function (event, delay) {
  ...
  ns.vars.obstacles = [];
  ns.vars.player.lvDesign(ns.vars.LV_DESIGN[lv].playerGravity, ns.vars.LV_DESIGN[lv].playerLift, ns.vars.LV_DESIGN[lv].playerVelocity);
...
ns.updateScore = function () {
  ...
  if (ns.vars.CURRENT_COUNT === ns.vars.LV_UP_COUNT) {
    ...
    ns.vars.player.lvDesign(ns.vars.LV_DESIGN[ns.vars.info.lv].playerGravity, ns.vars.LV_DESIGN[ns.vars.info.lv].playerLift, ns.vars.LV_DESIGN[ns.vars.info.lv].playerVelocity);
  }
  ns.printScore();
};
...
ns.endGame = function () {
  ...
  setTimeout(function () {
    ...
    var cache = gameOverPanel.offsetHeight;
...
ns.renderCloud = function () {
  image(ns.vars.resource.cloud, ns.vars.cloudX, 20, width, 80);
  image(ns.vars.resource.cloud, ns.vars.cloudX + width, 20, width, 80);
...
ns.renderRoad = function () {
  image(ns.vars.resource.road, ns.vars.roadX, 242, width, 21);
  image(ns.vars.resource.road, ns.vars.roadX + width, 242, width, 21);
...
ns.destroy = function () {
  ...
  if (ns.vars.el) {
    ns.vars.el.removeEventListener('touchstart', ns.startJump);
  }
