$(document).ready(function(){
    ...
    var setBannerTemplate = function(obj,targetObj,index) {
        ...
        arr.push(`<a class='rate-banner__content' href='/appForm/appFormDesignView.do?rateCd=${rateAdsvcCd}' title='${obj.rateAdsvcGdncBas.rateAdsvcNm} 페이지 이동'>\n`);
        ...
        if(giftPrmtFlag){
            ...
            if (giftPrmtList.length > 0) {
              ...
              giftPrmtList.forEach(item => {
                const popupUrl = item.popupUrl ? item.popupUrl.replace('/event/eventDetail.do', '/event/eventDetailAdminView.do') : '';
...
function goView(ntcartSeq, sbstCtg){
    ...
    var url = "/event/eventDetailAdminView.do?ntcartSeq=" + ntcartSeq + "&sbstCtg=" + sbstCtg + "&eventBranch=" + eventBranch + "&adminEventDate=" + adminEventDate;
    location.href = url;
}
