...
$(document).ready(function(){
    ...
    document.addEventListener('UILoaded', function() {
        KTM.swiperA11y('#outbox-banner', {
            // 모달 외부 배너는 width 값이 선언되어야 합니다.
            width : 620,
            spaceBetween : 20,
            autoplay : {
                delay : 5000,
    ...
    // [2022-03-16] 하단 bs가 있는 페이지는 ly-wrap 에 use-bs 클래스 추가
    ...
    $("input:checkbox[name=authCheckNone]").click(function(){
        ...
        if ($thisObj.is (':checked')) {
            ...
            if (validator.validate()) {
                ...
                var isConfirm = false;
                KTM.Confirm('전화상담가입 시, <br/> 가입 신청서가 우체국 계약등기로 발송되며<br/> 배송시까지 D+2일~3일 시간이 소요됩니다. <br/> (대면 수령 시 가입자 본인만 수령 가능)<br/><br/>※ 본인인증을 하지 않으실 경우 상담사에게<br/>신분증 사본 발송이 필요합니다.', function () {
                    ...
                    $("#btnNiceAuth").parent().hide();// 본인인증 버튼 감추기
                    $("#ulOnlineAuthDesc").parent().hide(); // 본인인증 설명 방법 감추기
                    $("#issuedBank").hide(); // 범용인증서 화면 감추기
    ...
    $(document).on('click', 'input:radio[name=dlvryType]', function (event, paramObj) {
        ...
        if (thisval == "02" ) {
            ...
            ajaxCommon.getItemNoLoading({
                ...
                ,function(jsonObj){
                    if (jsonObj && jsonObj.IS_NOW_DLVRY) {
                        ...
                        $("._isNowDlvry").show();
                        //setTimeout(function(){
                        $("input:radio[name=exitPw]:checked").trigger("click");
                        //}, 500);
            ...
            arr.push("<li class='c-text-list__item'>바로배송(당일 퀵)은 매일 오전 09시부터 오후 22시까지 이용 가능합니다. (무료배송/휴무일 없음)</li>\n");
            arr.push("<li class='c-text-list__item'>바로배송(당일 퀵)은 선결제 방식으로 미리 유심비를 결제하며, 결제 후 배송접수 실패 시 결제하신 내역은 영업일 기준 7일 이내 자동 환불 처리 됩니다.</li>\n");
            ...
            arr.push("<li>\n");
            //arr.push("    <a class='c-text c-text--underline u-ml--6 u-co-sub-4' href=\"javascript:openPage('eventPop','/event/eventDetailViewAjax.do?ntcartSeq=640','eventPopTitle=당일특급배송');\" >당일배송 자세히 보기<span class='c-hidden'>당일배송 자세히 보기 링크(새창)</span></a>\n");


            //https://www.ktmmobile.com/m/event/eventDetail.do?ntcartSeq=640
            arr.push("    <a class='c-text c-text--underline u-ml--6 u-co-sub-4' href=\"javascript:openPage('outLink','/event/eventDetail.do?ntcartSeq=640','eventPopTitle=당일특급배송');\" >당일배송 자세히 보기<span class='c-hidden'>당일배송 자세히 보기 링크(새창)</span></a>\n");

            arr.push("</li>\n");
            arr.push("<li class='c-text-list__item'>당일배송은 서울 전지역에 한해 서비스 가능합니다.</li>\n");
            arr.push("<li class='c-text-list__item'>당일배송은 당일 12시(정오)까지 가입신청 완료 시에만 당일 수령이 가능합니다.</li>\n");
            arr.push("<li class='c-text-list__item'>당일배송 신청 이후 유심 종류 및 요금제, 배송지 변경은 불가합니다. </li>\n");
            ...
            arr.push("<li class='c-text-list__item'>택배로 신청하실 경우 평일 13시까지 주문시 당일출고됩니다.</li>\n");

        }

        $("#ulDlvryTypeDesc").html(arr.join(''));
        isValidateNonEmptyStep2();
    });
    ...
    // 공동현관 출입방법
    $("input:radio[name=exitPw]").click(function(){
        $("#homePw").val("");
        var exitPw = $(this).val();
        if(exitPw=="1"){
            $("#divHomePw").show();
    ...
    var viewPhoneChargeInfo = function() {
        ...
        if ("01" == settlWayCd) {
            arr.push("<div class='c-charge__panel-left'>\n");
            arr.push("    <dl class='c-charge__item'>\n");
            arr.push("        <dt class='u-co-gray-9'>단말기 실구매가</dt>\n");
            arr.push("        <dd class='u-fw--medium'>"+numberWithCommas(pageObj.rateObj.instAmt)+" 원</dd>\n");
            arr.push("    </dl>\n");
            arr.push("    <dl class='c-charge__item'>\n");
            arr.push("        <dt class='u-co-gray-9'>단말기 잔여 금액</dt>\n");
            arr.push("        <dd class='u-fw--medium'>"+numberWithCommas(pageObj.rateObj.instAmt)+" 원</dd>\n");
            arr.push("    </dl>\n");
            arr.push("</div>\n");
            arr.push("<div class='c-charge__panel-right u-pa--type1'>\n");
            arr.push("    <dl class='c-charge__item'>\n");
            arr.push("        <dt>\n");
            arr.push("        <b class='u-fs-16'>일시납부 금액</b>\n");
            arr.push("        </dt>\n");
            arr.push("        <dd>\n");
            arr.push("        <b class='u-fs-26'>"+numberWithCommas(pageObj.rateObj.instAmt)+" </b>원\n");
            arr.push("        </dd>\n");
            arr.push("    </dl>\n");
            arr.push("</div>\n");
        } else {
            ...
            arr.push("    <dl class='c-charge__item'>\n");
            arr.push("        <dt class='u-co-gray-9'>단말기 잔여 금액</dt>\n");
            arr.push("        <dd class='u-fw--medium'>"+numberWithCommas(pageObj.rateObj.instAmt-pageObj.applDataForm.usePoint)+"원</dd>\n");
            arr.push("    </dl>\n");
            arr.push("    <dl class='c-charge__item'>\n");
            arr.push("        <dt class='u-co-gray-9'>할부기간</dt>\n");
            arr.push("        <dd class='u-fw--medium'>"+$("#modelMonthly").val()+"개월</dd>\n");
            arr.push("    </dl>\n");
            arr.push("</div>\n");
            arr.push("<div class='c-charge__panel-right'>\n");
            arr.push("    <dl class='c-charge__item'>\n");
            arr.push("        <dt>\n");
            arr.push("        <b class='u-fs-16'>단말기 할부 월 납부금액</b>\n");
            arr.push("        </dt>\n");
            arr.push("        <dd>\n");
            arr.push("        <b class='u-fs-26'>"+numberWithCommas(getInstMnthAmt())+" </b>원\n");
            arr.push("        </dd>\n");
            arr.push("    </dl>\n");
            arr.push("</div>\n");
        }
    ...
    var viewSelfPhoneChargeInfo = function() {
        ...
        //pageObj.applDataForm.usePoint
        arr.push("<div class='c-charge__panel-left'>\n");
        arr.push("    <dl class='c-charge__item'>\n");
        arr.push("        <dt class='u-co-gray-9'>단말기 실구매가</dt>\n");
        arr.push("        <dd class='u-fw--medium'  >"+numberWithCommas(pageObj.applDataForm.hndsetSalePrice)+ " 원</dd>\n");
        arr.push("    </dl>\n");

        if ("01" == userDivision) {
            arr.push("    <dl class='c-charge__item'>\n");
            arr.push("        <dt class='u-co-gray-9'>사용 포인트</dt>\n");
            arr.push("        <dd class='u-fw--medium'  >"+numberWithCommas(pageObj.applDataForm.usePoint)+ " P</dd>\n");
            arr.push("    </dl>\n");
        }
        arr.push("    <dl class='c-charge__item'>\n");
        arr.push("        <dt class='u-co-gray-9'>카드 즉시할인</dt>\n");
        if (0 < pageObj.applDataForm.cardDcAmt) {
            arr.push("        <dd class='u-fw--medium'  >"+numberWithCommas(getCardDcAmt())+ " 원</dd>\n");
        } else {
            arr.push("        <dd class='u-fw--medium'  >0 원</dd>\n");
        }
        arr.push("    </dl>\n");
        arr.push("    </div>\n");
        arr.push("    <div class='c-charge__panel-right'>\n");
        arr.push("    <dl class='c-charge__item'>\n");
        arr.push("        <dt>\n");
        arr.push("        <b class='u-fs-16'>단말기 잔여 금액</b>\n");
        arr.push("        </dt>\n");
        arr.push("        <dd>\n");
        arr.push("        <b class='u-fs-26' id='totalMonthlyPrice'>"+numberWithCommas(getPaymentHndsetPrice())+ "</b>원\n");
        arr.push("        </dd>\n");
        arr.push("    </dl>\n");
        arr.push("</div>\n");
    ...
    $('#btnUsimNumberCheck').click(function(){
        ...
        if (validator.validate()) {
            ...
            var varData = ajaxCommon.getSerializedData({
                ...
                //subscriberNo: ajaxCommon.isNullNvl(pageObj.niceResSmsObj.sMobileNo, "") //휴대폰인증 번호
                // 포탈에서만 휴대폰번호 넘기고있음.. 일단 그대로 유지
    ...
    $("._btnAddr").click(function() {
        ...
        if (addrFlag != "3") {
            ...
            openPage('pullPopup', '/dlvryInfo.do','nfcYn='+nfcYn,'1');
        }
    });
    ...
    var setStep = function(setStep) {
        ...
        if (setStep == 2) {
            ...
            if (stepInfo.isInitStep4) {
                ...
                $("#recommendFlagCd").trigger("change");  //4
                //임시 저장 부가서비스
                getAdditionTempList();
            ...
            ajaxCommon.getItem(
                {
                    ...
                    // if (isStolenIp && onOffType == "5" && operType == "NAC3") { //셀프개통 신규가입일 경우
    ...
    $("#inEqualCheck").click(function() {
        ...
        if($(this).is(':checked')) {
            ...
            if (dlvryType == "02" && $("#cstmrPost").val() != "") {
                ...
                var radUsimProd = $("input[name='radUsimProd']:checked").val();
                var nfcYn = radUsimProd == '05' ? 'N' : 'Y';
                openPage('pullPopupNoOpen', '/dlvryInfo.do','nfcYn='+nfcYn,'1');
    ...
    $("#btnChangeCancel").click(function(){
        ajaxCommon.createForm({
            method:"post"
            ,action:"/appForm/appFormDesignView.do"
        });
    ...
    $("#reqUsimSn ,#cstmrMobileFn,#cstmrMobileMn,#cstmrMobileRn,#cstmrMail,#selfIssuExprDt,#driverSelfIssuNum1,#driverSelfIssuNum2,#selfIssuNum,#dlvryName,#dlvryMobileFn,#dlvryMobileMn,#dlvryMobileRn,#homePw,#selfIssuNum").keyup(function(){
        isValidateNonEmptyStep2();
    });
        ...
        if(onOffType != "5"){
            if ($('#divDlvry').is(':visible') ) {
                ...
                if ("02" == dlvryType) {
                    ...
                    reqDlvryForm.cstmrNativeRrn = $.trim($("#cstmrNativeRrn01").val()) + $.trim($("#cstmrNativAeRrn02").val()) ;
    ...
    var fnSaveTempStep2 = function() {
        ...
        if(operType != OPER_TYPE.CHANGE && operType != OPER_TYPE.EXCHANGE ) {
            ...
            //기변
            paramObj.cstmrMobileFn = $.trim($("#changeCstmrMobileFn").val());
            paramObj.cstmrMobileMn = $.trim($("#changeCstmrMobileMn").val());
            paramObj.cstmrMobileRn = $.trim($("#changeCstmrMobileRn").val());
        }
        ...
        if(onOffType != "5"){
            ...
            if ($("#dlvryTelFn").val() != "") {
                paramObj.dlvryTelFn = $.trim($("#dlvryTelFn").val());
                paramObj.dlvryTelMn = $.trim($("#dlvryTelMn").val());
                paramObj.dlvryTelRn = $.trim($("#dlvryTelRn").val());

            }
            var dlvryMobile = $.trim($("#dlvryMobile").val());
            paramObj.dlvryMobileFn = $.trim($("#dlvryMobileFn").val());
            paramObj.dlvryMobileMn = $.trim($("#dlvryMobileMn").val());
            paramObj.dlvryMobileRn = $.trim($("#dlvryMobileRn").val());
    ...
    var fnSaveTempStep5 = function() {
        ...
        if ( reqPayType== "D") {
            ...
            //paramObj.reqCardCompany = $.trim($("#reqCardCompany").val());
    ...
    var fnSaveForm = function() {
        ...
        pageObj.applDataForm.cstmrAddr = $.trim($("#cstmrAddr").val());
        pageObj.applDataForm.cstmrAddrDtl = $.trim($("#cstmrAddrDtl").val());
        //pageObj.applDataForm.ktCounselAgree = $.trim($("#ktCounselAgree").val());
            ...
            ajaxCommon.getItemNoLoading({
                ...
                ,function(jsonObj){
                    ...
                    if (AJAX_RESULT_CODE.SUCCESS ==  jsonObj.RESULT_CODE || jsonObj.RESULT_CODE=="DONE") {
                        ...
                        KTM.LoadingSpinner.hide(true);
                        MCP.alert("바로배송 운영 시간이 아닙니다.\n다른 배송수단을 선택해 주세요. \n(바로배송 운영시간 : 10:00~22:00)");
                        $('._btnNext').show();
                        return ;
            ...
            ,function(jsonObj){
                if (AJAX_RESULT_CODE.SUCCESS ==  jsonObj.RESULT_CODE) {
                    ...
                    if("02" == dlvryType ) {
                        ...
                        var usimKindsCd = $('#usimKindsCd').val();
                        var actionURL = "/appForm/appFormComplete.do";

                        //eSIM
                        if ("09" == usimKindsCd) {
                            actionURL = "/eSimForm/appFormComplete.do";
                        }
    ...
    function goPay() {
        ...
        // 스마트로페이 결제요청
        // PC 연동일 경우에만 아래 smartropay.payment 함수를 구현합니다. (Mobile 연동은 STEP 3으로 이동)
        smartropay.payment({
            FormId : 'tranMgr',				// 폼ID
            Callback : function(res) {		// 인증/승인 분리일경우 콜백함수
                ...
                approvalForm.MallReserved.value = $('#MallReserved').val();
                approvalForm.action = $('#ReturnUrl').val();
                approvalForm.submit();
            }
        });
    };
    ...
    var fnReqPreMoveAut = function() {
        ...
        ajaxCommon.getItemNoLoading({
            ...
            if (AJAX_RESULT_CODE.SUCCESS ==  jsonObj.RESULT_CODE) {
                ...
                $("#isNpPreCheck").val("1");
                //ajaxCommon.setSelectBoxReadonly($("#moveCompany"));
        ...
        ajaxCommon.getItemNoLoading({
            ...
            if (AJAX_RESULT_CODE.SUCCESS ==  jsonObj.RESULT_CODE) {
                ...
                MCP.alert("상담사 개통으로만 신청 가능합니다.<br/>상담사 개통 신청으로 진행합니다.", function(){
                    location.href = "/appForm/appCounselorInfo.do";
                });
    ...
    var getListSale = function() {
        ...
        if (prodId != "" ) {
            ...
            ajaxCommon.getItem({
                ...
                ,function(phoneObj){
                    ...
                    if (pageObj.phoneObj.sesplsYn == "Y" && REQ_BUY_TYPE.USIM == reqBuyType) {
                        ...
                        var colorCd = thisPhoneSnty.atribValCd1;
                        var phoneProdImgDt = null;
                        for(var j = 0 ; j < pageObj.phoneObj.phoneProdImgDtoList.length ; j++) {
                            if (colorCd == pageObj.phoneObj.phoneProdImgDtoList[j].sntyColorCd) {
                                phoneProdImgDt = pageObj.phoneObj.phoneProdImgDtoList[j];
                                break;
                            }
                        }
                        ...
                        $('#phoneImg').attr("src",phoneProdImgDt.phoneProdImgDetailDtoList[0].imgPath).show();
            ...
            $('#bottomTitle').text(pageObj.prdtNm ) ;
            $('#phoneImg').hide();
        }
...
var isValidateNonEmptyStep1 = function() {
    //console.log ("isValidateNonEmptyStep1==>");
    var operType = $("#operType").val() ;
    var cstmrType = $("input:radio[name=cstmrType]:checked").val();
    var reqBuyType = $("#reqBuyType").val() ;
    var onOffType = $("#onOffType").val() ;
...
var isValidateNonEmptyStep2 = function() {
    //console.log ("isValidateNonEmptyStep2==>");
    var cstmrType = $("input:radio[name=cstmrType]:checked").val();
    var operType = $("#operType").val() ;
    ...
    if($("#onOffType").val() != "5"){
        $('#btnNext').attr('data-step','22');
        $('#btnNext2').attr('data-step','22');
    }else{
        $('#btnNext').attr('data-step','2');
        $('#btnNext2').attr('data-step','2');
    }
...
var isValidateNonEmptyStep3 = function() {
    //console.log ("isValidateNonEmptyStep3==>");

    var operType = $("#operType").val() ;
    var onOffType = $("#onOffType").val() ;

    if(onOffType != "5"){
        $('#btnNext').attr('data-step','23');
        $('#btnNext2').attr('data-step','23');
    }else{
        $('#btnNext').attr('data-step','3');
        $('#btnNext2').attr('data-step','3');
    }
...
var isValidateNonEmptyStep4 = function() {
    //console.log ("isValidateNonEmptyStep4==>");
    validator.config={};
    ...
    if($("#onOffType").val() != "5"){
        $('#btnNext').attr('data-step','24');
        $('#btnNext2').attr('data-step','24');
    }else{
        $('#btnNext').attr('data-step','4');
        $('#btnNext2').attr('data-step','4');
    }
...
var isValidateNonEmptyStep5 = function() {
    //console.log ("isValidateNonEmptyStep5==>");
    var cstmrType = $("input:radio[name=cstmrType]:checked").val();
    var reqPayType = $('input:radio[name=reqPayType]:checked').val();
    ...
    if($("#onOffType").val() != "5"){
        $('#btnNext').attr('data-step','25');
        $('#btnNext2').attr('data-step','25');
    }else{
        $('#btnNext').attr('data-step','5');
        $('#btnNext2').attr('data-step','5');
    }
...
//인터넷 가입 상담을 위한 개인정보 제3자 제공 동의
// function getKtCounselInfo() {
//     if ($("#ktCounselAgree").val() != null && $("#ktCounselAgree").val() == "Y" ) {
//         $("._ktCounselFlag").show();
//     }else {
//         $("._ktCounselFlag").hide();
//     }
...
function gtag_report_conversion(type,url) {
    ...
    if(type == '1'){
        //Event snippet for 이탈모수_셀프개통_PC_1
        gtag('event', 'conversion', {
            'send_to': 'AW-862149999/WPRMCPPe06YaEO-6jZsD',
            'value': 1.0,
            'currency': 'KRW',
            'event_callback': callback
        });
    }else  if(type == '2'){
        //Event snippet for 이탈모수_셀프개통_PC_2
        gtag('event', 'conversion', {
            'send_to': 'AW-862149999/fennCP_e06YaEO-6jZsD',
            'value': 1.0,
            'currency': 'KRW',
            'event_callback': callback
        });
    }else  if(type == '3'){
        //Event snippet for 이탈모수_셀프개통_PC_3
        gtag('event', 'conversion', {
            'send_to': 'AW-862149999/RkQnCIvf06YaEO-6jZsD',
            'value': 1.0,
            'currency': 'KRW',
            'event_callback': callback
        });
    }else  if(type == '4'){
        //Event snippet for 이탈모수_셀프개통_PC_4
        gtag('event', 'conversion', {
            'send_to': 'AW-862149999/7w-mCJff06YaEO-6jZsD',
            'value': 1.0,
            'currency': 'KRW',
            'event_callback': callback
        });
    }else  if(type == '21'){
        //Event snippet for 이탈모수_상담사개통_PC_1
        gtag('event', 'conversion', {
            'send_to': 'AW-862149999/B_BACPne06YaEO-6jZsD',
            'value': 1.0,
            'currency': 'KRW',
            'event_callback': callback
        });
    }else  if(type == '22'){
        //Event snippet for 이탈모수_상담사개통_PC_2
        gtag('event', 'conversion', {
            'send_to': 'AW-862149999/x0BSCIXf06YaEO-6jZsD',
            'value': 1.0,
            'currency': 'KRW',
            'event_callback': callback
        });
    }else  if(type == '23'){
        //Event snippet for 이탈모수_상담사개통_PC_3
        gtag('event', 'conversion', {
            'send_to': 'AW-862149999/H4isCJHf06YaEO-6jZsD',
            'value': 1.0,
            'currency': 'KRW',
            'event_callback': callback
        });
    }else  if(type == '24'){
        //Event snippet for 이탈모수_상담사개통_PC_4
        gtag('event', 'conversion', {
            'send_to': 'AW-862149999/oHcsCJ3f06YaEO-6jZsD',
            'value': 1.0,
            'currency': 'KRW',
            'event_callback': callback
        });
...
// var kakaoCancle = function() {
//     $('#custNm').val('');
//     $('#birthday').val('');
//     $('#inpPhone').val('');
//     $('#comfirmDiv').css('display', 'none');
//     $('#onlineAuthType11').attr('checked', false)
// }
...
//                 kakaoAuth(dataObejct.kakaoAuth.result);
//             }
//         }
//     });
// });
...
//                 $('#snsMobileNo').val($('#inpPhone').val());
//                 $('#snsBirthday').val($('#birthday').val());
//                 // niceAuthResult(dataObejct.niceLog);
//                 pageObj.niceHistSeq = dataObejct.niceLog.niceHistSeq;
//                 $('#frm').submit();
