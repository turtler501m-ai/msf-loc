...
var selfTimeChk = function() {
    ...
    if (!simpleApplyObj.IsMnpTime) {
        ...
        var el = document.querySelector('#join-info-modal');
...
var setSocCodeInfo = function(){
    ...
    var operNm = $('input[name=operTypeRadio]:checked').attr("operName");

    $('#bottomTitle2').html(operNm + ' / ' + rateNm);
    $("#bottomTitle").html(rateNm);
    $('#joinInfoDl3').show().find('#joinInfo3').text(rateNm);
...
var initDataTypeInfo = function() {
    ...
    $("#joinInfoDl2").show().find("#joinInfo2").text(dataType);
}

/** 가입유형 화면 세팅 */
var initOperTypeInfo = function() {

    var rateNm = $("#selSocCode option:selected").attr("rateNm");
    rateNm = ajaxCommon.isNullNvl(rateNm, "-");

    var operNm = $('input[name=operTypeRadio]:checked').attr("operName");

    $('#bottomTitle2').html(operNm + ' / ' + rateNm);
    $("#joinInfoDl1").show().find("#joinInfo1").text(operNm);
...
var fnSelfClick = function() {
    ...
    // 가입유형
    var operType = $('input[name=operTypeRadio]:checked').val();
    ...
    ajaxCommon.getItem({
        ...
        if(data.resultCd == '0000'){
            ...
            ajaxCommon.createForm({
                method : 'post'
                ,action : '/appForm/appAgentForm.do'
            });
