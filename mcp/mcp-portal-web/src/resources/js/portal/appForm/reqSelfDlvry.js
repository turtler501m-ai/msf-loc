...
$(document).ready(function(){
    ...
    $("#btnSmsAuth, #btnSmsMinorAgentAuth").click(function(){
        ...
        ,function(jsonObj){
            ...
            if(jsonObj.RESULT_CODE == "F_AUTH"){
              MCP.alert('<p class="u-mt--12"><strong class="self-opening__highlight">회원정보와 본인인증 정보가<br>일치하지 않습니다.</strong></p><p class="u-mt--12">다른 명의로 신청을 원하실 경우<br>로그아웃 후 이용 해 주시기 바랍니다.</p>');
              ...
              openPage('outLink2', jsonObj.pollingPageUrl, data);
            }
        });
    });
    ...
    $("input:radio[name=dlvryKind]").change(function(){
        ...
        if(dlvryType == '01'){
            if(!pageObj.isNowDlvry){
                if ("" == pageObj.strMsg) {
                    MCP.alert(' <div class="self-opening"> <span class="c-text-label c-text-label--type2 c-text-label--red rectangle">바로배송 가능시간</span> <p class="u-mt--12"> <strong class="self-opening__highlight">09:00 ~ 22:00</strong> </p><p class="u-mt--12">바로배송 가능 시간이 아닙니다 <br>택배를 이용해주세요.</p></div>');
                } else {
        ...
        $('#dlvryInfo,.dlvryType1,.dlvryType2').hide();

        if(dlvryType == "01"){ // 바로배송(당일 퀵)

            $(".dlvryKind01").show();
            $('#dlvryInfo,.dlvryType2').show();
            ...
            $('#radAccMeth0').trigger('click');

        }else { // 택배

            $(".dlvryKind02").show();
            $('#dlvryInfo,.dlvryType1').show();
    ...
    $("#dlvryChk").click(function() {
        ...
        if(nowHH < 9 || nowHH > 21){
            MCP.alert('바로배송(당일 퀵) 가능시간이 아닙니다.<br>택배를 이용해 주세요.', function (){
                $('#dlvryKind2').trigger('click');
            });
            return false;
        }
    ...
    // 공동현관 출입방법
    $('input[name=exitPw]').change(function(){
        $("#homePw").val("");
        if($(this).val() == "1"){
            $(".nowArea2Sub").show();
        } else {
    ...
    $("#saveBtn").click(function(){
        ...
        if(dlvryKind == "01"){
            ...
            // 공동현관 출입방법
            exitTitle = ajaxCommon.isNullNvl($('input:radio[name=exitPw]:checked').next().text(),"");
            exitPwVal = ajaxCommon.isNullNvl($('input:radio[name=exitPw]:checked').val(),"");
        ...
        ajaxCommon.getItem({
                ...
                ,cache:false
                ,url:"/appForm/saveSelfDlvryAjax.do"
                ,data: varData
                ,dataType:"json"
            }
            ,function(jsonObj){
...
var fnNiceCert = function(prarObj) {
    ...
    if(certResult.RESULT_CODE == AJAX_RESULT_CODE.SUCCESS){
        ...
        MCP.alert('<p class="u-mt--12"><strong class="self-opening__highlight">회원정보와 본인인증 정보가<br>일치하지 않습니다.</strong></p><p class="u-mt--12">다른 명의로 신청을 원하실 경우 로그아웃 후 이용 해 주시기 바랍니다.</p>');
    }else if(certResult.RESULT_CODE == "F_AUTH_AGNT"){
        MCP.alert('<p class="u-mt--12"><strong class="self-opening__highlight">본인인증 정보가 만 19세 미만입니다.</strong></p><p class="u-mt--12">법정대리인은 만 19세 이상 성인이어야 합니다.</p>');
    }else{
        MCP.alert('<p class="u-mt--12"><strong class="self-opening__highlight">본인인증 정보가 일치하지 않습니다.</strong></p><p class="u-mt--12">입력하신 정보를 확인 후 다시 시도해 주시기 바랍니다.</p>');
    }
...
function inputChk(){
    ...
    var exitType = $('input:radio[name=exitPw]:checked').val();
