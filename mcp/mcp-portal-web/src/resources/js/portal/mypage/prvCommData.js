...
function getDataList(){
    ...
    ajaxCommon.getItem({
        ...
        if(list == null || list == undefined || list.length== 0){ // 데이터가 없는 경우
            contentHtml= '<tr><td colspan="5">'+htmlContent+'</td></tr>';
        }else{ // 데이터가 존재하는 경우
            ...
            for(var i=0;i<list.length;i++){
                ...
                contentHtml+= 	'<td>';
                if (item.isInvstProc == "Y") {
                     contentHtml+= 		'<button class="c-button c-button--gray is-active" style="padding: 0.4rem 2.3rem;"onclick="showDetail('+item.apySeq+')">출력</button>';
                } else {
                     contentHtml+= 		'-';
                }
                contentHtml+= 	'</td>';
...
// 출력 팝업
function showDetail(apySeq){
    var parameterData = ajaxCommon.getSerializedData({
        ncn:$("#ctn").val(),
        seq : apySeq
    });

    openPage('pullPopupByPost','/mypage/prvCommDataPrintPop.do',parameterData);
}

function select(){
    ajaxCommon.createForm({
        method:"post"
        ,action:"/mypage/prvCommData.do"
    });
