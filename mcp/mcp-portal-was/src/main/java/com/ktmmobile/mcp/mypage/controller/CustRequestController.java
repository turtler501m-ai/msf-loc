...
public class CustRequestController {
    ...
    private static final Logger logger = LoggerFactory.getLogger(CustRequestController.class);
    ...
    /**
     * 안심보험 가입신청 저장 API — MSF TOBE 연동 전용.
     * custRequestAjax(reqType=IS) 와 동일한 DB 저장 로직(seq채번+MST+INSR)을 수행하되,
     * NICE 세션 검증 없이 JSON 요청으로 호출 가능하도록 분리.
     * X21 부가서비스 신청은 MSF 에서 직접 처리하므로 이 API 에는 포함하지 않음.
     *
     * 요청: CustRequestDto JSON { contractNum, cstmrName, mobileNo, cstmrNativeRrn,
     *        onlineAuthType, onlineAuthInfo, custId, cstmrType, insrProdCd, etcMobile }
     * 응답: { RESULT_CODE: SUCCESS/FAIL1/FAIL2/FAIL3, CUST_REQ_SEQ }
     */
    @RequestMapping(value = "/mypage/custRequestInsrSaveApi.do")
    @ResponseBody
    public Map<String, Object> custRequestInsrSaveApi(@RequestBody CustRequestDto custRequestDto) {
        HashMap<String, Object> rtnMap = new HashMap<String, Object>();

        custRequestDto.setReqType("IS");

        // 1. CUST_REQUEST_SEQ 채번
        long custReqSeq = 0;
        try {
            custReqSeq = custRequestService.getCustRequestSeq();
        } catch (Exception e) {
            logger.error("custRequestInsrSaveApi getCustRequestSeq error: {}", e.getMessage());
        }
        if (custReqSeq <= 0) {
            rtnMap.put("RESULT_CODE", "FAIL1");
            rtnMap.put("RESULT_MSG", "SEQ 채번 실패");
            return rtnMap;
        }
        custRequestDto.setCustReqSeq(custReqSeq);

        // 주민번호 암호화 (값이 있는 경우만)
        if (StringUtils.isNotBlank(custRequestDto.getCstmrNativeRrn())) {
            custRequestDto.setCstmrNativeRrn(EncryptUtil.ace256Enc(custRequestDto.getCstmrNativeRrn()));
        }

        // 2. NMCP_CUST_REQUEST_MST INSERT
        boolean isInsertReqMst = false;
        try {
            isInsertReqMst = custRequestService.insertCustRequestMst(custRequestDto);
        } catch (Exception e) {
            logger.error("custRequestInsrSaveApi insertCustRequestMst error: {}", e.getMessage());
        }
        if (!isInsertReqMst) {
            rtnMap.put("RESULT_CODE", "FAIL2");
            rtnMap.put("RESULT_MSG", "고객요청 마스터 저장 실패");
            return rtnMap;
        }

        // 3. NMCP_CUST_REQUEST_INSR INSERT
        boolean isInsertReqInsr = false;
        try {
            isInsertReqInsr = custRequestService.insertCustRequestInsr(custRequestDto);
        } catch (Exception e) {
            logger.error("custRequestInsrSaveApi insertCustRequestInsr error: {}", e.getMessage());
        }
        if (!isInsertReqInsr) {
            rtnMap.put("RESULT_CODE", "FAIL3");
            rtnMap.put("RESULT_MSG", "보험신청 세부 저장 실패");
            return rtnMap;
        }

        rtnMap.put("RESULT_CODE", "SUCCESS");
        rtnMap.put("CUST_REQ_SEQ", custReqSeq);
        return rtnMap;
    }
