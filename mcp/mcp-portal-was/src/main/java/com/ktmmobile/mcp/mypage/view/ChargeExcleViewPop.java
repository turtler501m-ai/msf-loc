...
import com.ktmmobile.mcp.common.mplatform.vo.MpFarPaymentInfoVO;
...
public class ChargeExcleViewPop extends org.springframework.web.servlet.view.document.AbstractJExcelView {

    private static final Logger logger = LoggerFactory.getLogger(ChargeExcleViewPop.class);

    private final String[] title = {"납부일자","납부방법","납부금액"};
    private final int [] width = {10,10,10,10};

    private final static String NAME = "납부상세내역";
    ...
    protected void buildExcelDocument(Map<String, Object> param, WritableWorkbook workbook, HttpServletRequest request, HttpServletResponse response) throws RowsExceededException, WriteException {
        ...
        MpFarPaymentInfoVO vo = (MpFarPaymentInfoVO) param.get("vo");
        String userName = (String) param.get("userName");
        String userCtn = (String) param.get("userCtn");
        List<ItemPay> list = vo.getItemPay();
        ...
        if( list != null){
            for(int i=0; i < list.size(); i++){
                ...
                String payMentMoney = StringUtil.addComma(list.get(i).getPayMentMoney());
                String payMentDate = StringUtil.strtoDateStr(list.get(i).getPayMentDate(),".");

                sheet.addCell(new jxl.write.Label(0,  row, payMentDate, format1));
                sheet.addCell(new jxl.write.Label(1,  row, list.get(i).getPayMentMethod(), format1)); // 1과2병합
                sheet.addCell(new jxl.write.Label(3,  row, payMentMoney, format1));
                sheet.mergeCells(1, row, 2, row); // 컬럼,로우,컬럼,로우
            }
        }
    }
    ...
    private WritableSheet createSheet(WritableWorkbook workbook, String name, int sheetNum,String userName,String userCtn) throws RowsExceededException, WriteException {
        ...
        sheet.addCell(new jxl.write.Label(1, 2, "납부방법", format1));
        sheet.addCell(new jxl.write.Label(3, 2, "납부금액", format1));
        sheet.mergeCells(1, 2, 2, 2); // 컬럼,로우,컬럼,로우 // 2열에1과2를병합
        return sheet;
    }
