...
public class BannerStatDayExcleView extends org.springframework.web.servlet.view.document.AbstractJExcelView {

    private static final Logger logger = LoggerFactory.getLogger(BannerStatDayExcleView.class);

    private final String[] title = {"번호","배너명","배너코드","배너등록일","신청완료","판매량(개통완료)"};
    private final int [] width = {10,40,40,40,20,20};

    private final static String NAME = "배너별판매량통계일별";
    ...
    protected void buildExcelDocument(Map<String, Object> param, WritableWorkbook workbook, HttpServletRequest request, HttpServletResponse response) throws RowsExceededException, WriteException {
        ...
        sheet.addCell(new jxl.write.Label(1,  row, "-", format1));
        sheet.addCell(new jxl.write.Label(2,  row, "-", format1));
        sheet.addCell(new jxl.write.Label(3,  row, "-", format1));
        sheet.addCell(new jxl.write.Label(4,  row, String.valueOf(total.getRequestStateCodeSum1()), format1));
        sheet.addCell(new jxl.write.Label(5,  row, String.valueOf(total.getRequestStateCodeSum2()), format1));
        if( list != null){
            for(int i=0; i < list.size(); i++){
                ...
                sheet.addCell(new jxl.write.Label(1,  row, data.getBannNm(), format1));
                sheet.addCell(new jxl.write.Label(2,  row, String.valueOf(data.getBannSeq()), format1));
                sheet.addCell(new jxl.write.Label(3,  row, data.getCretDt(), format1));
                sheet.addCell(new jxl.write.Label(4,  row, String.valueOf(data.getRequestStateCodeSum1()), format1));
                sheet.addCell(new jxl.write.Label(5,  row, String.valueOf(data.getRequestStateCodeSum2()), format1));
