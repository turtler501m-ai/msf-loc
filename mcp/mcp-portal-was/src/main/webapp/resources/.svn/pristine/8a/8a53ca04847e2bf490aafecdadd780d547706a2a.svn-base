...
$(document).ready(function (){
	...
	// 특정 URL이 있을 경우
	const lyTitle = $(".ly-page-sticky");
	const lyTitleTab = $(".c-tabs__list.sticky-header");
	const maskingBtn = 	$("<div class='masking-badge-wrap'><div class='masking-badge'><a class='masking-badge__button' href='/m/mypage/myinfoView.do' title='가려진(*) 정보보기 페이지 바로가기'><i class='masking-badge__icon' aria-hidden='true'></i><p>가려진(*) 정보보기</p></a></div></div>");
	const maskingPopBtn = $("<div class='masking-badge-wrap'><div class='masking-badge'><button id='osbinfoBtn' class='masking-badge__button' type='button' onclick='obsInfoView();'><i class='masking-badge__icon' aria-hidden='true'></i><p>가려진(*) 정보보기</p></button></div></div>");
	...
	if(myPage || myKtDcView){
	   $(lyTitle).after(maskingBtn);
		if(myView){
			...
			setTimeout(function(){
				const hasText = $(".ly-content").find(":contains('정회원 인증')").length > 0;
				if (hasText) {
					$(maskingBtn).remove();
				}
			},500);
	...
	// 탭구조 페이지일 경우
	if(myPage){
		const hasTab = $(lyTitleTab).next().hasClass("c-hr");
		const hasTab2 = $(lyTitleTab).hasClass("sticky-header");
		if(hasTab){
			$(lyTitleTab).next().after(maskingBtn);
			$(maskingBtn).css("top", "-0.5625rem");
		} else if(hasTab2){
			$(lyTitleTab).after(maskingBtn);
		}
	}
    ...
    $('.masking-badge').on("click", function(){
        ...
        $(".masking-badge a").click(function(){
            $(this).css("pointer-events", "none");
        });
        $(".masking-badge button").click(function(){
        	$(maskingPopBtn).remove();
        });
    });
    $(window).on('scroll', function(){
        $('.masking-badge').removeClass('is-active');
    });
});
