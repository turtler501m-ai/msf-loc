...
$(document).ready(function(){
  ...
  $("#roamingTab1").click(function(){
    goRoamingView("1");
  });

  // 신청 탭 (신청가능한 로밍 조회)
  $("#roamingTab2").click(function(){
    goRoamingView("2");
  });
  ...
  $(document).on("click", ".runtime-data-insert", function(){
    ...
    if(hasClass){
      ...
      // 상세정보 조회
      var isFreeSvc = $(this).closest("ul").hasClass("freeDataList");
      var type = isFreeSvc ? 1 : 2;
      var idx = isFreeSvc ? $(this).closest("li").attr("freeIdx") : $(this).closest("li").attr("nofreeIdx");
      rateCdDtl(type, idx);
...
var addRegSvcSearch = function(){
  ...
  ajaxCommon.getItem({
     id: 'myAddSvcListAjax'
    ,cache: false
    ,url: '/personal/myAddSvcListAjax.do'
    ,data: varData
    ,dataType: "json"
  },function(data){
    ...
    for(var i=0; i < svcList.length; i++){
      ...
      html += '<tr>';
      html += '<td>' + svcList[i].socDescription + '</td>';

      if(svcList[i].socRateValue == 'Free'){
        html += '<td>무료</td>';
      }else if(svcList[i].soc == 'PL2078760'){
        html += '<td>' + numberWithCommas(svcList[i].socRateVatValue) + ' 원 / 1일</td>';
      } else {
        html += '<td>월 ' + numberWithCommas(svcList[i].socRateVatValue) + ' 원</td>';
      }
      ...
      if(svcList[i].onlineCanYn == 'Y'){
        html += '<button class="c-button c-button--underline c-button--sm u-co-sub-4" type="button" onclick="btn_regCancel(\'' + svcList[i].socDescription + '\',\'' + svcList[i].soc + '\');">해지</button>';
...
var regSvcSearch = function(){
  ...
  ajaxCommon.getItem({
     id:'addSvcListAjax'
    ,cache:false
    ,url:'/personal/addSvcListAjax.do'
    ,data: varData
    ,dataType:"json"
  },function(data){
    ...
    if(freeData != null && 0 < freeData.length){
      for(var i=0; i < freeData.length; i++){
        if(freeData[i].useYn != 'Y'){ // 가입중인 부가서비는 제외
          ...
          freeHtml += '    </div>';
          freeHtml += '    <button class="runtime-data-insert c-accordion__button" id="acc_headerA' + i + '" type="button" aria-expanded="false" aria-controls="acc_contentA' + i + '">';
    ...
    if(noFreeData != null && 0 < noFreeData.length){
      for(var idx = 0; idx < noFreeData.length; idx++){
        if(noFreeData[idx].useYn != 'Y'){ // 가입중인 부가서비는 제외
          ...
          noFreeHtml += '    </div>';
          noFreeHtml += '    <button class="runtime-data-insert c-accordion__button" id="acc_headerB' + idx + '" type="button" aria-expanded="false" aria-controls="acc_contentB' + idx + '">';
...
var btn_regSvc = function(){
  ...
  openPage('pullPopup','/personal/addSvcViewPop.do', varData, 1);
}
...
var goRoamingView = function(tabType){
  ...
  ajaxCommon.createForm({
     method: "post"
    ,action: "/personal/roamingView.do"
  });
