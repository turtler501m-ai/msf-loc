...
var drawUseageData= function(useageData, useageVoice, useageSms){
    ...
    for(var i=0;i<drawDataList.length;i++){
        ...
        drawDataList[i].forEach(function(item){
            drawHtml+= drawUseageDataOne(item);
        });
    }
...
function btn_farReg(){
    ...
    if(chkRadion == 'S'){	//즉시변경
        ...
        if($("#isFarPriceThisMonth").val() == "Y"){
         ...
         var tSocAmnt = $("#tSocAmnt").val();
         var aSocAmnt1 = $("#aSocAmnt1").val();
         ...
         if(aSocAmnt1 != null){
            aSocAmnt1 = aSocAmnt1.replaceAll("원","");
         }
         varData = ajaxCommon.getSerializedData({
          ...
          var url = "/m/mypage/farPricePlanChgAjax.do";
          //페이지 호출
          $.ajax({
        ...
        KTM.Confirm("요금제 예약변경 하시겠습니까?", function (){
            ...
            var url = "/m/mypage/farPricePlanResChgAjax.do";
            //페이지 호출
             $.ajax({
