...
var pageInfo = {
    ...
    var operTypeTxtToggle = function (){
        if($('input[name=operType]:checked').val() == 'MNP3'){
            ...
            $('#newTxt').hide();
            $('#chgTxt').hide();
        } else if($('input[name=operType]:checked').val() == 'HDN3' || $('input[name=operType]:checked').val() == 'HCN3'){
            $('#chgTxt').show();
            $('#movTxt').hide();
            $('#newTxt').hide();
        } else {
            if (pageInfo.settlYn =="Y") {
...
//서식지로 이동
    ...
    var usimInfoAjax = function(isInit){
        ...
        ajaxCommon.getItemNoLoading({
            id:'getUsimInfo'
            ,cache:false
            ,url:'/usim/selectUsimBasShopListAllAjax.do'
            ,data: varData
            ,dataType:"json"
            ,async:false
        },function(jsonObj){
            if ('0001' ==  jsonObj.RESULT_CODE) {
                ...
                if(usimBasList){
                    ...
                    if ($("input[name=prdtId]:checked").length == 0) {
                        ...
                        if ($("input[name=prdtId]:checked").length == 0) {
                            ...
                            $('#bottomTitle1').text($("input[name=prdtId]:eq(0)").attr('prdtNm'));
                        }
                    }
                ...
                if(regiTypeList){
                    ...
                    for(var i=0; i<regiTypeList.length; i++){
                        ...
                        innerHTML += '<input class="c-radio c-radio--button operType'+i+'" id="operType'+i+'" type="radio" name="operType" operName="'+regiTypeList[i].operName+'" value="'+regiTypeList[i].operType+'" ' + checkedVal + ' onchange="operTypeTxtToggle();">';
                        innerHTML += '<label class="c-label operType'+i+'" for="operType'+i+'"><span class="app-form-view__lable">'+ regiTypeList[i].operName + '<br /><span class="u-fs-14">'+ (regiTypeList[i].operName === '번호이동' ? '(쓰던번호 그대로)' : '(새로운 번호로)')+'</span></span></label>';
                    ...
                    if ($(":radio[name=operType]:checked").length == 0) {
                        ...
                        } else if (onOffType == '5' && !pageInfo.simpleApplyObj.IsMnpTime ){
                            $('input[name=operType][operName="번호이동"]').attr('disabled','disabled');
                            setTimeout(function() {
                                $('input[name=operType][operName="신규가입"]').trigger('click');
                            }, 1000)
                    ...
                    if($('input[name=operType]:checked').val() == 'MNP3'){
                        if(onOffType == '5' && !pageInfo.simpleApplyObj.IsMnpTime ){
                            $('input[name=operType][operName="번호이동"]').attr('disabled','disabled');
                            setTimeout(function() {
                                $('input[name=operType][operName="신규가입"]').trigger('click');
                            }, 1000)
    ...
    var selectUsimBasJoinPriceAjax = function(obj , isInit){
        ...
        if($('#prodId').val() == '' || $('#prodCtgType').val() == '03'){
            $('#bottomTitle1').text($(":radio[name=prdtId]:checked").attr('prdtNm'));
            $('#bottomTitle2').text($('input[name=operType]:checked').attr('operName') + ' / ' + rateNm);
        } else {
        ...
        if(cprtDcAmt > 0){
            if(cprtDcAmt >= totalVatPrice){
                ...
                if($('#prodId').val() != '' && $('#prodCtgType').val() != '03'){
                    ...
                    $('#totalPrice2').html(numberWithCommas(payMnthChargeAmt));
                    $('#totalPriceBottom').html(numberWithCommas(payMnthChargeAmt - cprtDcAmt + Number(thisData.payMnthAmt)));
                }else{
                    $('#totalPrice').html(numberWithCommas(totalVatPrice - cprtDcAmt));
                    $('#totalPrice2').html(numberWithCommas(totalVatPrice));
                    $('#totalPriceBottom').html(numberWithCommas(totalVatPrice - cprtDcAmt));
                }
            }
        }
...
$(window).load(function (){
        ...
        ,function(jsonObj){
            ...
            if(jsonObj != undefined && jsonObj.length > 0){
                for (var i = 0; i < jsonObj.length; i++) {
                    if((i + 1) % 2 == 0){
                        innerHTML += '<div class="c-checktype-row c-col2 c-col2--w460" id="cprtCard' + i + '" style="display:none;">';
                    }
                    if(i == 0){
                        innerHTML += '<div class="c-checktype-row c-col2 c-col2--w460" id="cprtCardX" style="display:none;">';
                        innerHTML += '    <input class="c-radio c-radio--button c-radio--button--type1 c-radio--button--card" id="chkCardX" type="radio" name="chkCard" value="" checked onclick="cprtPriceSet();">';
                        innerHTML += '    <label class="c-label" for="chkCardX">';
                        innerHTML += '        <div class="icon-prefix" aria-hidden="true">';
                        innerHTML += '            <img src="/resources/images/portal/common/ico_card_no.svg" alt="">';
                        innerHTML += '        </div>';
                        innerHTML += '        <div class="c-label--title">';
                        innerHTML += '            <p class="c-text c-text--fs17 u-fw--medium">선택 안 함</p>';
                        innerHTML += '        </div>';
                        innerHTML += '    </label> ';
                    }
                    ...
                    innerHTML += '    <input class="c-radio c-radio--button c-radio--button--type1 c-radio--button--card" id="chkCard'+ i +'" type="radio" name="chkCard" value="' + i + '" onclick="cprtPriceSet();" style="display:none;"> ';
                    innerHTML += '    <label class="c-label" for="chkCard'+ i +'" style="display:none;">';
                    innerHTML += '        <div class="icon-prefix">';
                    innerHTML += '            <img src="' + jsonObj[i].cprtCardThumbImgNm + '" alt="' + jsonObj[i].cprtCardGdncNm + ' 실물 사진" onerror="this.src=\'/resources/images/portal/common/ico_card_no.svg\'">';
                    innerHTML += '        </div>';
                    innerHTML += '        <div class="c-label--title">';
                    innerHTML += '            <p class="c-text c-text--fs17 u-fw--medium">' + jsonObj[i].cprtCardGdncNm + '</p>';
                    if(jsonObj[i].cprtCardItemNm != ''){
                        innerHTML += '            <p class="c-text c-text--fs15">' + jsonObj[i].cprtCardItemNm + '</p>';
                    }
                    if(jsonObj[i].cprtCardItemDesc != ''){
                        innerHTML += '            <p class="c-text c-text--fs15">' + jsonObj[i].cprtCardItemDesc + '</p>';
                    }

                    innerHTML += '        </div>';
                    innerHTML += '    </label>';
                    if(i % 2 == 0 || i == (jsonObj.length - 1)){
                        innerHTML += '</div>'
                    }
                }
    ...
    $('#orderSearch').on('click', function (){
        getChargelist();
        $('.sortTxt').html($('input[name=orderEnum]:checked').next().text() + '<i class="c-icon c-icon--sort" aria-hidden="true"></i>');
    });
    ...
    $('input[name=instNom]').on('change', function (){
        ...
        $('#socCode').val('');
        $('#radPayTypeA').show();  //요금제를 선택해주세요.
        $('#radPayTypeB').hide();  //요금제 선택
        $('#radPayTypeC').hide();  //요금제 다시 선택하기
        $('#divGift').hide();
        initEvntCdPrmt();
        $('#choicePay').text('');  //선택 요금제 정보
        $('#choicePayTxt').text(''); //선택 요금제 정보
        $('.giftInfoList').empty().hide();   //????  없음...
        $('#bottomTitle2').text(''); //선택 요금제 정보
    ...
    var setQuestionTitle = function(type) {
        ...
        if (type == 1) {
            arr.push("내게 맞는 상품찾기\n");
            arr.push("<span class='u-fs-18 u-co-mint u-fw--bold u-block'>유심요금제&nbsp;\n");
            arr.push("    <span class='u-co-black u-mr--8'>고민이라면?</span>\n");
            arr.push("    <span class='u-fs-14 u-fw--regular u-co-black'>고객님 성향에 맞춘 추천 요금제를 확인해보세요</span>\n");
            arr.push("</span>\n");
        } else {
            arr.push("고객님께 딱 맞는\n");
            arr.push("<span class='u-fs-18 u-co-mint u-fw--bold u-block'>유심요금제&nbsp;\n");
            arr.push("    <span class='u-co-black u-mr--8'>추천합니다</span>\n");
            arr.push("    <span class='u-fs-14 u-fw--regular u-co-black'>고객님 성향에 맞춘 추천 요금제를 확인해보세요</span>\n");
            arr.push("</span>\n");
        }
                ...
                if(returnCode == "00") {
                    if(inqrBasList.length > 0) {
                        ...
                        $.each(inqrBasList, function(index, value) {
                            ...
                            arr.push("    <span class='u-fs-18 u-letter-spacing--0'>Q"+(index+1)+".</span>\n");
                            arr.push("    <span class='u-fs-16 u-co-black u-fw--normal' id='radio_header_"+index+"'>"+v_rateRecommendInqrSbst+"</span>\n");
                            arr.push("</div >\n");
                            ...
                            arr.push("            <div class='c-checktype-row c-col2'>\n");
                            if(v_ansSbst1 != "") {
                            ...
                            if(inqrBasLength == (index+1)) {
                                arr.push("			<button class='result c-button survey-button u-width--130 c-button--h48 c-button-round c-button--white u-mt--30'>결과 확인</button>");
                            } else {
                                arr.push("			<button class='c-button survey-button u-width--130 c-button--h48 c-button-round c-button--white u-mt--30'>다음</button>");
                            }
                            arr.push("		</div>");
    ...
    $(document).on('click', 'input:radio[id^=radQuestion_]', function () {
        var surveyBtn = $(this).parents('.survey__content').find('.c-button-wrap .c-button.survey-button.survey-button');
        $(surveyBtn).addClass('is-active');
    });

    //2번째 질문 유효성 검사
    $(document).on('click', '.survey__content .c-button-wrap .c-button.survey-button.result', function () {
    ...
    $(document).on('click', '.survey__content .c-button-wrap .c-button.survey-button', function () {
    ...
    if(document.referrer.indexOf('appSimpleInfo.do') > -1 || document.referrer.indexOf('openMarketInfo.do') > -1 || document.referrer.indexOf('storeInfo.do') > -1
             ...
             || document.referrer.indexOf('storyway.do') > -1	|| $('#initOnOffType').val() == '5') {
        $('#cstmrType1').trigger('click');

        if ( $('#initOnOffType').val() == '0' ) {
            $('#radOpenType2').trigger('click');
            $('#counselTxt').show();
        } else {
            if (pageInfo.simpleApplyObj.IsMacTime || pageInfo.simpleApplyObj.IsMnpTime) {
    ...
    /* 요금제 혜택 아코디언 */
    $(document).off('click', '.c-accordion__item > .c-accordion__head .c-accordion__button')
...
var setMoreBtn = function (){
    ...
    //$('#moreBtnTxt').text(pageInfo.cprtPageNo + '/' + totalPage);
    $('#moreBtnTxt1').html('<span class="c-hidden">현재 노출된 항목</span>' + numberWithCommas(dispCnt));
    $('#moreBtnTxt2').html('<span class="c-hidden">전체 항목</span>' + numberWithCommas(totalCnt));
...
document.addEventListener('UILoaded', function() {
    // [2022-02-10] 스와이퍼 옵션 추가
    KTM.swiperA11y('#swiperBanner .swiper-container', {
        width : 940,
        spaceBetween : 20,
        navigation : {
...
var getShareUrl = function (){
    ...
    if($('#prodId').val() == '' || $('#prodCtgType').val() == '03'){
        ...
        var onOffType			= $('input[name=radOpenType]:checked').attr('id') == 'radOpenType1' ? '5' : '0';						//구분코드 7 모바일(셀프개통) 3 모바일
        var prodId				= '';
        ...
        var onOffType		= '0';						//구분코드 7 모바일(셀프개통) 3 모바일
            ...
            if(bannerList.length > 0) {
                $.each(bannerList, function(index, value) {
                    ...
                    if($('#serverName').val() == 'LOCAL'){
                        bannerHtml += '		<img src="/resources/images/portal/content/img_sticky_banner.png" alt="' + v_bannNm + '">';
                    }else{
...
var getGiftList = function (buyType){
    ...
    ,function(jsonObj){
          ...
          innerHtml += `<hr class="c-hr c-hr--title u-mt--20 u-mb--20">\n`;
          innerHtml += `</div>\n`
...
var getChargelist = function(isInit) {
    ...
    var orderEnum = pageInfo.orderEnum ; //"RECOMM_ROW" ;//$('[name=orderEnum]:checked').val();
    ...
    if($('#prodId').val() == '' || $('#prodCtgType').val() == '03'){	//유심
        ...
        if(agrmTrm != undefined && agrmTrm != ''){
            ...
            ajaxCommon.getItem({
                id:'listCharge'
                ,cache:false
                ,url:'/usim/getUsimChargeListAjax.do'
                ,data: varData
                ,dataType:"json"
                ,async:false
            },function(data){
...
var getRowTemplateQuResult = function(rateObj ,cardCnt ){
    ...
    arr.push('    <input class="c-radio" id="cbResult_'+cardCnt+'" type="radio" name="selectRate" value="'+rateObj.rateCd +'" >\n');
    arr.push('        <label class="c-card__label" for="cbResult_'+cardCnt+'"></label>\n');
    arr.push('        <div class="rate-info__wrap">\n');
    arr.push('            <!--요금제 타이틀 영역 -->\n');
    arr.push('            <div class="rate-info__title">\n');
    arr.push('                <div class="rate-sticker-wrap">\n');
...
var charegListView = function () {
    ...
    if(rateList != null && rateList != '' && rateList.length > 0) {
        ...
        for (var i = 0; i < rateList.length; i++) {
            ...
            if (pageInfo.bestRateArr && pageInfo.bestRateArr.length > 0) {
                for(var j=0; j< pageInfo.bestRateArr.length; j++){
                    if(pageInfo.bestRateArr[j] == rateCdSp) {
                        isRateArr = true ;
                        break;
                    }
                }
...
var sliderInit = function (){
    var sliderEl;
    if($('#dataSlider3g').is(':visible')){
        sliderEl = document.querySelector('#dataSlider3g');
        sliderEl.noUiSlider.set([0, 100]);

        sliderEl = document.querySelector('#callSlider3g');
        sliderEl.noUiSlider.set([0, 100]);
    }else if($('#dataSliderLte').is(':visible')){
        sliderEl = document.querySelector('#dataSliderLte');
        sliderEl.noUiSlider.set([0, 100]);

        sliderEl = document.querySelector('#callSliderLte');
        sliderEl.noUiSlider.set([0, 100]);

    }else if($('#dataSlider5g').is(':visible')){
        sliderEl = document.querySelector('#dataSlider5g');
        sliderEl.noUiSlider.set([0, 100]);

        sliderEl = document.querySelector('#callSlider5g');
        sliderEl.noUiSlider.set([0, 100]);
