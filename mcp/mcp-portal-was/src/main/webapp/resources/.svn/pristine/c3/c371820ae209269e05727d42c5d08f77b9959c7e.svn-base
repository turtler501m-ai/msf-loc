...
$(document).ready(function(){
	...
	$("#nextTab").click(function(){
		location.href="/m/cs/csInquiryBookingHist.do";
	});

	// 문의 유형 변경 이벤트
	$("#qnaCtg").change(function(){

		var etc2 = $("#qnaCtg option:selected").val();
	...
	$("#saveBtn").click(function(){
		...
		KTM.Confirm('예약하시겠습니까?' ,function () {
			...
			ajaxCommon.getItem({
				...
				,cache:false
				,url:'/m/cs/insertInquiryBooking.do'
				,data: varData
				,dataType:"json"
				,async:false
				,errorCall : function () {
				...
				if(resultCd == '0003'){ // 정회원 인증 페이지로 이동
					MCP.alert(resultMsg, function (){location.href = '/m/mypage/updateForm.do';});
					return false;
				}
...
function bookingTime(){
	...
	var today = new Date();
	var startDate= new Date(today.getFullYear(), today.getMonth(), today.getDate()); // 예약 가능 시작일
	var endDate = new Date(today.getFullYear(), today.getMonth()+2, 0); // 예약 종료일
	...
	if(bookingDate==null || bookingDate== '') { // 날짜 미선택
		...
		return false;
	}else if(bookingDt > endDate){   // 익월 이후 날짜 선택
	...
	ajaxCommon.getItem({
			...
			,cache:false
			,url:'/m/cs/checkBookingAvailableByDt.do'
			,data: varData
			,dataType:"json"
			,async:false
		}
		,function(jsonObj){
			...
			var resultCd= (!jsonObj || !jsonObj.remainYn) ? "F" :jsonObj.remainYn;
			...
			// 해당 일시에 예약가능 인원 수 존재
			openPage('pullPopupByPost','/m/bookingTimePop.do',varData);
		});
}