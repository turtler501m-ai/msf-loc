...
var getRwdRegStatus= function(contractNum){
    ...
    switch (viewCd){
        case "IMPOSSIBLE":
            notAcceptReason= "고객님께서는 자급제 보상 서비스 가입 대상이 아닙니다.<br/>자세한 내용은 고객센터(114/무료)로 문의 바랍니다.";
            break;
        case "ING":
...
var applyRwd= function(){
    ...
    if(priceVal.match(/[^0-9]/gi)){ // 숫자 이외의 값을 포함하는 경우
        ...
        ajaxCommon.getItemFileWithAsync({
             ...
             ,errorCall : function (e) {
                ...
                if(resultCd != AJAX_RESULT_CODE.SUCCESS){
                 ...
                 // 신청 최종 성공 (메인 화면으로 이동)
                MCP.alert(resultMsg, function(){location.href="/m/main.do";});
            });
    },function() { // 신청 취소버튼 클릭
        applyRwdStatusChg(true);
        return;
