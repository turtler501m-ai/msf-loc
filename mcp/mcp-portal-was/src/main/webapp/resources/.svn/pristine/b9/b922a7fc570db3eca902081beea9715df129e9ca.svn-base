...
var returnUrl = "/request/marketRequest.do?a="   + $("#cntpntShopId").val() +
    "&o=" + $("#operType").val() +
    "&s=" + $("#modelSalePolicyCode").val() +
    "&p=" + $("#reqModelName").val() +
    "&g=" + $("#sprtTp").val() +
    "&r=" + $("#socCode").val() +
    "&y=" + $("#enggMnthCnt").val() +
    "&u=" + $("#openMarketReferer").val() +
    "&f=N"; //안면인증 대상아님으로 처리
...
$(document).ready(function(){
            ...
            ajaxCommon.getItem(
                {
                    ...
                    if (isStolenIp) {
                        MCP.alert("접속하신 IP는 부정사용 개통 신고되어 있는<br> " +
                            ...
                            "IP 접속제한 해제를 희망하실 경우<br>" +
                            "고객센터(114/무료)로 연락주시면<br>" +
                            "본인인증 후 제한 해제 가능합니다.");
    ...
    simpleAuthvalidation= function(){
        ...
        if(operType == OPER_TYPE.NEW){
            validator.config={};
            validator.config['isSmsAuth'] = 'isNonEmpty';   // 휴대폰인증
            if(!validator.validate()){
                var errId = validator.getErrorId();
                MCP.alert(validator.getErrorMsg(),function (){
                    $selectObj = $("#"+errId);
    ...
    $("._agree").click(function(){
        handleCommonAgreeClick();  // 개별 약관 동의 시 전체약관 선택 버튼 처리
    });
    ...
    $("#cstmrName, #cstmrNativeRrn01, #cstmrNativeRrn02").keyup(function(){
        isValidateNonEmptyStep1();
    });
    ...
    $("#moveMobileFn, #moveMobileMn, #moveMobileRn").keyup(function(){
        isValidateNonEmptyStep3();
    });
...
var fnNiceCert = function(prarObj) {
    ...
    if (pageObj.niceType == NICE_TYPE.CUST_AUTH) { // 간편본인인증

        var cstmrNativeRrn = $.trim($("#cstmrNativeRrn01").val());
        var cstmrName = $.trim($("#cstmrName").val());
        var strMsg = '<p class="u-mt--12"><strong class="u-fw--bold">본인인증 정보가 일치하지 않습니다.</strong></p><p class="u-mt--12">입력하신 정보를 확인 후 다시 시도해 주시기 바랍니다.</p>';
        ...
        var authInfoJson= {cstmrName: cstmrName , cstmrNativeRrn: cstmrNativeRrn, authType:prarObj.authType,
            operType: operType , onOffType: onOffType, ncType: ""};
    ...
    } else if (pageObj.niceType == NICE_TYPE.SMS_AUTH) {  // 휴대폰인증
    ...
    } else if (pageObj.niceType == NICE_TYPE.INSR_PROD) {  // 안심보험 휴대폰 인증

        var authInfoJson= {cstmrName: "insrProdNm", cstmrNativeRrn: "insrProdRrn"
            ,authType:prarObj.authType , isCommonPrd:false, ncType: "" };
...
var setStep = function(setStep) {
    ...
    var operType = $("#operType").val();
    if (setStep == 1) {
        $("#divBtnGroup1").show();      // 다음버튼
        $("#divBtnGroup2").hide();      // 이전 및 다음버튼
    } else {
        if( operType == OPER_TYPE.NEW && setStep == 3){
            $("#divBtnGroup1").show();
            $("#divBtnGroup2").hide();
        } else if( operType == OPER_TYPE.MOVE_NUM && setStep == 4 ){
            $("#divBtnGroup1").show();
            $("#divBtnGroup2").hide();
        }else  {
            $("#divBtnGroup1").hide();
    ...
    ajaxCommon.getItemNoLoading({
        ...
        if (AJAX_RESULT_CODE.SUCCESS ==  jsonObj.RESULT_CODE) {
            ...
            ajaxCommon.createForm({
                method:"post"
                ,action:"/appForm/appAgentFormComplete.do"
            });
