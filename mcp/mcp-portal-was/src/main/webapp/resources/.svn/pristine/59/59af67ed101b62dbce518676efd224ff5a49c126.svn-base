...
var btn_tab = function(type){
  if(type == "1"){
    addRegSvcSearch(); // 조회 및 변경 탭 (이용중인 로밍 조회)
  }else if(type == "2"){
    regSvcSearch(); // 신청 탭 (신청가능한 로밍 조회)
  }
}
...
var addRegSvcSearch = function(){
  ...
  ajaxCommon.getItem({
    ...
    ,cache:false
    ,url:'/m/personal/myRoamJoinListAjax.do'
    ,data: varData
    ,dataType:"json"
  },function(data){
    ...
    for(var i=0; i < svcList.length; i++){
      ...
      if(dateType == "1"){
        html += '<br/>';
        html += '<span class="roaming-info__sub">(' + getRoamingDateFormat(strtDt, "yyyy.mm.dd") + ' ~</span>';
        html += '<br/>';
        html += '<span class="roaming-info__sub">' + getRoamingDateFormat(endDt, "yyyy.mm.dd") + ')</span>';
      }else if(dateType == "2"){
        html += '<br/>';
        html += '<span class="roaming-info__sub">(' + getRoamingDateFormat(strtDt, "yyyy.mm.dd hh:mi") + ' ~)</span>';
      }else if(dateType == "3" || dateType == "4"){
        html += '<br/>';
        html += '<span class="roaming-info__sub">(' + getRoamingDateFormat(strtDt, "yyyy.mm.dd hh:mi") + ' ~</span>';
        html += '<br/>';
        html += '<span class="roaming-info__sub">' + getRoamingDateFormat(endDt, "yyyy.mm.dd hh:mi") + ')</span>';
      }
      ...
      if(lineType == "M"){
        ...
        if(subCtnList != null &&  0 < subCtnList.length){
          for(var j = 0; j < subCtnList.length; j++) {
            html += '<br/>';
            html += '<span class="roaming-info__sub">서브회선' + (j + 1) + ' : ';
            html += subCtnList[j];
            html += '</span>';
          }
        }
        ...
        html += '<br/>';
        html += '<span class="roaming-info__sub">대표회선 : ';
      ...
      if(svcList[i].updateFlag == 'Y' && 0 < infoList[i].rateAdsvcGdncSeq){
        ...
        html += '<a class="c-button c-button--underline c-button--sm u-co-mint" href="javascript:void(0);"  onclick="btn_regChg(\'' + infoList[i].rateAdsvcGdncSeq + '\',\'Y\',\'' + svcList[i].prodHstSeq + '\');">변경</a>';
        html += '</div>';
      }

      if(svcList[i].onlineCanYn == 'Y'){
        html += '<div>';
        html += '<a class="c-button c-button--underline c-button--sm u-co-mint" href="javascript:void(0);" onclick="btn_regCancel(\'' + svcList[i].socDescription + '\',\'' + svcList[i].soc + '\',\'' + svcList[i].prodHstSeq + '\');">해지</a>';
...
var btn_regChg = function(rateAdsvcGdncSeq, flag, prodHstSeq){
  ...
  openPage('pullPopupByPost','/m/personal/roamingViewPop.do', varData);
}
...
var regSvcSearch = function(){
  ...
  ajaxCommon.getItem({
     id:'regRoamListAjax'
    ,cache:false
    ,url:'/m/personal/regRoamListAjax.do'
    ,data: varData
    ,dataType:"json"
  },function(data){
    ...
    if(list != null && 0 < list.length){
      for(var i = 0; i < list.length; i++){
        if(list[i].selfYn == 'Y'){
          ...
          // 로밍 유형에 따라 가격 조회
          html += '        <span class="c-text c-text--type4" id="baseVatAmt_' + i + '" style="white-space: nowrap;">'
...
var getRoamingPriceFormat = function(freeYn, price, dateType, usePrd, socRateValue){
  ...
  if(freeYn == "Y"){
    ...
    priceView = numberWithCommas(priceNumber) + '원<br> / ' + usePrd + '일';
  }else{
...
var goRegServiceView = function(tabType){
  ...
  ajaxCommon.createForm({
     method: "post"
    ,action: "/m/personal/regServiceView.do"
  });
