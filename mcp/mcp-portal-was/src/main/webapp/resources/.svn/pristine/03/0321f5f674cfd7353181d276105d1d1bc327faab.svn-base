...
/* 휴대폰 인증 성공 후 객체값 post방식으로 넘김 / 다음 페이지 이동 */
//function fnNiceCert(obj) {
//location.href = "${pageContext.request.contextPath}/ma/join/sndPage.do";
//	$('#frm').submit();
//}
function fnNiceCert(obj) {
	niceAuthResult(obj);
...
function pop_nice() {
	...
	//var openNewWindow = window.open("/nice/popNiceAuth.do?sAuthType=M&sCheck=U&mType=Mobile", "link_ipin", 'resizable=yes, height=550, width=445');
}
...
function pop_ipin() {
	...
	//var openNewWindow = window.open("/nice/popNiceIpinAuth.do", "link_ipin", 'resizable=no, height=550, width=445');
}

function pop_credit() {
	pageObj.niceType = NICE_TYPE.CUST_AUTH ;
	openPage('outLink','/nice/popNiceAuth.do?sAuthType=C&mType=Mobile','');
	//var openNewWindow = window.open("/nice/popNiceAuth.do?sAuthType=C&sCheck=U&mType=Mobile", "link_ipin", 'resizable=yes, height=550, width=445');
}
