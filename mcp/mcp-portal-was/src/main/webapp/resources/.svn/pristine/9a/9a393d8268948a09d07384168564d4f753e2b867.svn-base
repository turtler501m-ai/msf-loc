    ...
    return new Promise((resolve) => {
        ...
        ajaxCommon.getItem({
            ...
            if (!mMonthpaymentdto) {
                ...
                periodSpan = `<span>${mMonthpaymentdto.billStartDate}~${mMonthpaymentdto.billEndDate}</span>`;

                infoHtml += `<div class="bill-tit">`;
                infoHtml += `    <p>${mMonthpaymentdto.billMonth}월 청구요금</p>`;
                infoHtml += `    <div class="bill-btn-wrap">`;
                infoHtml += `        <a href="javascript:;" url="/m/mypage/childChargeView.do" onclick="openNewDetailPopup(this);">내역보기</a>`;
                infoHtml += `    </div>`;
                infoHtml += `</div>`;
                infoHtml += `<div class="bill-price u-co-mint">`;
                infoHtml += `    <b>${mMonthpaymentdto.totalDueAmt}</b><span>원</span>`;
                infoHtml += `</div>`;

                $("#billInfoPeriod").append(periodSpan);
...
function initRealTimePayInfo(childNcn) {
    return new Promise((resolve) => {
        ...
        ajaxCommon.getItem({
            ...
            infoHtml += `    <p>실시간 요금</p>`;
            infoHtml += `    <div class="bill-btn-wrap">`;
            infoHtml += `        <a href="javascript:;" url="/m/mypage/childRealTimeRateView.do" onclick="openNewDetailPopup(this);">내역보기</a>`;
            infoHtml += `    </div>`;
            infoHtml += `</div>`;
            infoHtml += `<div class="bill-price">`;
            infoHtml += `    <b>${sumAmt}</b><span>원</span>`;
            infoHtml += `</div>`;
    ...
    return new Promise((resolve) => {
        ...
        ajaxCommon.getItem({
            ...
            if ($("#payAddSvcList tr").length == 0) {
                $("#payAddSvcHeader").prop('disabled', true);
            }
            if ($("#freeAddSvcList tr").length == 0) {
                $("#freeAddSvcHeader").prop('disabled', true);
            }
            resolve();
        });
    });
...
function selectMain() {
    ajaxCommon.createForm({
        method:"post"
        ,action:"/m/mypage/childInfoView.do"
    });
...
function getPopupChildNcn() {
    return $("#pullModalContents").find('#ctn').val();
}