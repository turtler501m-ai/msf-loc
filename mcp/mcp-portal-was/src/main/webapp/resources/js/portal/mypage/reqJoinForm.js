...
VALIDATE_FIX_MSG.fax1 = "팩스번호 앞자리는 두자리 이상입니다.";
VALIDATE_FIX_MSG.fax2 = "팩스번호 중간자리는 세자리 이상입니다.";
VALIDATE_FIX_MSG.fax3 = "팩스번호 뒷자리는 네자리 이상입니다.";
VALIDATE_COMPARE_MSG.fax1 = "팩스번호 형식이 일치하지 않습니다.";
...
history.pushState(null, null,"/mypage/reqJoinForm.do");
window.onpopstate = function (event){   	   				
	history.go("/mypage/reqJoinForm.do");
}
...
$(document).ready(function (){
	...
	$("#btnRequest").click(function(){
		...
		if($("input:radio[name=rdRecvType]:checked").val() == "FX") {			
	        ...
	        validator.config["fax1"] = "isNumBetterFixN2";
            validator.config["fax2"] = "isNumBetterFixN3";
            validator.config["fax3"] = "isNumFix4";
            validator.config["fax1&fax2&fax3"] = "isTelNumberCheck";
		}else {
		...
		if(validator.validate()){
			...
			pageObj.applDataForm["faxNo"] = $.trim($("#fax1").val()) + "" + $.trim($("#fax2").val()) + "" + $.trim($("#fax3").val());
