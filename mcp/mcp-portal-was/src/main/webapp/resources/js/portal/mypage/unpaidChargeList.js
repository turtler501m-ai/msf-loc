var confirm = "자동납부를 이용하시는 경우, \n 당월 청구금액 납부 시 이중 납부가 될 수 있으니"+
	"\n납부일 및  계좌/카드 거래내역 확인 후 이용하시기 바랍니다.\n요금을 납부하시겠습니까?";
var pageObj = {
	niceType:""
	, authObj:{}
	, niceHistSeq:0
...
function select(){
	ajaxCommon.createForm({
		method:"post"
		,action:"/mypage/unpaidChargeList.do"
	});
...
function searchChg(ncn){
	...
	ajaxCommon.getItem({
			...
			,cache:false
			,url:'/mypage/unpaidChargeListAjax.do'
			,data: varData
			,dataType:"json"
		}
		,function(jsonObj){
...
function pop_ipin() {
	...
	var openNewWindow = window.open("about:blank", "link_ipin", 'resizable=no, height=550, width=445');
	openNewWindow.location.href = "/nice/popNiceIpinAuth.do";
	openNewWindow.focus();
}

function pop_credit() {
	pageObj.niceType = NICE_TYPE.CUST_AUTH ;
	window.open('/nice/popNiceAuth.do?sAuthType=C', 'popupChk', 'width=500, height=550, top=100, left=100, fullscreen=no, menubar=no, status=no, toolbar=no, titlebar=yes, location=no, scrollbar=no');

}

function pop_nice() {
	pageObj.niceType = NICE_TYPE.CUST_AUTH ;
	window.open('/nice/popNiceAuth.do?sAuthType=M', 'popupChk', 'width=500, height=550, top=100, left=100, fullscreen=no, menubar=no, status=no, toolbar=no, titlebar=yes, location=no, scrollbar=no');
}
	...
	ajaxCommon.getItem({
		...
		,function(jsonObj) {
			...
			if(jsonObj.RESULT_CODE == 'S'){
				...
				ajaxCommon.createForm({
					method:"post"
					,action:"/mypage/realTimePaymentView.do"
				});
