...
$(document).ready(function(){
    ...
    var setUlRecommendRateList = function(obj,targetObj,index) {
        ...
        if (obj.rateGdncBannerList.length > 0) {
            ...
            arr.push(`				   <div class='rate-banner__label-wrap'></div>\n`);
        }
        ...
        arr.push(`                        <div class="rate-banner__detail-info">\n`);
        if (obj.rateAdsvcBnfitGdncMap.RATEBE01?.rateAdsvcItemDesc) {
            arr.push(`                        <p class="rate-banner__detail-desc">${obj.rateAdsvcBnfitGdncMap.RATEBE01.rateAdsvcItemDesc}</p>`);
        }

        if (obj.rateAdsvcBnfitGdncMap.RATEBE31?.rateAdsvcItemDesc) {
            arr.push(`                        <p class="rate-banner__detail-sub">${obj.rateAdsvcBnfitGdncMap.RATEBE31.rateAdsvcItemDesc}</p>`);
        } else {
        	arr.push(`					      <p class="rate-banner__detail-sub"></p>\n`);
        }
    ...
    //상단 친구가 추천해준 요금제 높이값
    function equalizeHeights(selectors) {
	...
	window.addEventListener("load", function () {
	  equalizeHeights([
	    ".friend-all-banner__rate .rate-banner__label-wrap",
	    ".friend-all-banner__rate .rate-banner__title-wrap",
	    ".friend-all-banner__rate .rate-banner__detail-desc",
	    ".friend-all-banner__rate .rate-banner__detail-sub",
	  ]);
	});
