...
$(document).ready(function (){
    ...
    $(".paySelect").on("change",function(){
        var paySel =$(".paySelect:checked").val();
        if(paySel=='C'){
            $(".payCard").show();
            $(".payKakao").hide();
            $(".payPayco").hide();
    ...
    function btn_payReg(payType){
        ...
        if(payType == "C"){
            ...
            KTM.Confirm(confirm, function (){
                ...
                ajaxCommon.getItem({
                    ...
                    if(jsonObj.RESULT_CODE == 'S') {
                        ajaxCommon.createForm({
                            method:"post"
                            ,action:"/mypage/realTimePaymentCompleteView.do"
                        });
            ...
            if(jsonObj.RESULT_CODE == 'S'){
                ...
                //if(jsonObj.url != ""){
                //    window.open(jsonObj.url, 'payPop', 'width=500, height=550, top=100, left=100, fullscreen=no, menubar=no, status=no, toolbar=no, titlebar=yes, location=no, scrollbar=no');
                //}
