...
$(document).ready(function (){
    ...
    $("#iccid").keyup(function() {
        var iccid = $("#iccid").val().length;
        if(iccid != 19){
            $(".iccidChk").addClass("has-error");
            $("#msgA1").show();
        }else {
            $(".iccidChk").removeClass("has-error");
            $("#msgA1").hide();
        }
    });
...
function fnNiceCert(prarObj) {
    ...
    function sharing_reg(){
        ...
        var minorAgentName = ($("#minorAgentName").val() == null) ? "" : $.trim($("#minorAgentName").val());
        var minorAgentRrn   = ($("#minorAgentRrn01").val() == null) ? "" : $.trim($("#minorAgentRrn01").val())  + $.trim($("#minorAgentRrn02").val());
        var minorAgentTelFn  = ($("#minorAgentTelFn").val() == null) ? "" : $.trim($("#minorAgentTelFn").val());
        var minorAgentTelMn  = ($("#minorAgentTelMn").val() == null) ? "" : $.trim($("#minorAgentTelMn").val());
        var minorAgentTelRn  =  ($("#minorAgentTelRn").val() == null) ? "" : $.trim($("#minorAgentTelRn").val());
        var minorAgentRelation   =  ($("#minorAgentRelation").val() == null) ? "" : $.trim($("#minorAgentRelation").val());
        var cstmrMobileFn = "";
        var cstmrMobileMn= "";
        var cstmrMobileRn= "";

        if ($("#cstmrMobileFn").length) {
            cstmrMobileFn = $.trim($("#cstmrMobileFn").val());
            cstmrMobileMn= $.trim($("#cstmrMobileMn").val());
            cstmrMobileRn= $.trim($("#cstmrMobileRn").val());
        }
            ...
            ,function(jsonObj){
                if (AJAX_RESULT_CODE.SUCCESS ==  jsonObj.RESULT_CODE) {
                    ...
                    ajaxCommon.createForm({
                        method:"post"
                        ,action:"/content/dataSharingStep4.do"
                    });
    ...
    var fnSelfPreOpenCallBack = function() {
        ...
        ,function(jsonObj){
            if (AJAX_RESULT_CODE.SUCCESS ==  jsonObj.RESULT_CODE) {
                ...
                $("#selfCertSection1, #selfCertSection2, #selfCertSection3").show();
    ...
    function sharingYnChg(){
         ...
         ajaxCommon.getItem({
            ...
            ,cache:false
            ,url:'/content/insertOpenRequestAjax.do'
            ,data: varData
            ,dataType:"json"
         }
         ,function(jsonObj){
