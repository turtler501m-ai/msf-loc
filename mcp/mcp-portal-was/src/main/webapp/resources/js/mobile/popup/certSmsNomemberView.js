function authCfrm(){
    ...
    location.href = '/m/order/orderTempList.do';
    //searchOrderTempList();
...
function searchOrderTempList(selPageNo) {
    ...
    ajaxCommon.getItem({
        ...
        ,cache:false
        ,url:'/m/order/orderTempPageGroupListAjax.do'
        ,data: varData
        ,dataType:"json"
    },function(data){
    ...
    $(listData).each(function(index) {
        ...
        if(index != 0){
            ...
            if(this.tempType == 'SELF'){
                loopHeadHtml += '<span class="u-fw--medium">'+emptyToTyphoon(this.selfProdNm)+'</span>';
                loopHeadHtml += '<span class="u-co-gray-7">자급제</span>';
            }else if(this.reqBuyType == 'UU'){
                if ("09" == this.usimKindsCd) {
                    loopHeadHtml += '<span class="u-fw--medium">eSIM</span>';
                } else {
                ...
                if(this.onOffType == '5' || this.onOffType == '7' || this.onOffType == '9'){
                    ...
                    loopHeadHtml += '<span class="u-co-gray-7">온라인서식지</span>';
                }
        ...
        if(this.tempType == 'SELF'){
            ...
            loopBodyHtml += '<span class="u-ml--8">'+emptyToTyphoon(this.selfProdNm)+' / '+emptyToTyphoon(this.atribValNmOne)+' / '+emptyToTyphoon(this.atribValNmTwo)+'</span>';
            loopBodyHtml += '</p>';
            loopBodyHtml += '<p>';
            ...
            loopBodyHtml += '<p>';
            loopBodyHtml += '<i class="c-icon c-icon--data" aria-hidden="true"></i>';
            loopBodyHtml += '<span class="u-ml--8">'+emptyToTyphoon(this.prodNm)+' / '+emptyToTyphoon(this.atribValNmOne)+' / '+emptyToTyphoon(this.atribValNmTwo)+'</span>';
            loopBodyHtml += '</p>';
            loopBodyHtml += '<p>';
            loopBodyHtml += '<i class="c-icon c-icon--call-gray" aria-hidden="true"></i>';
               loopBodyHtml += '<span class="u-ml--8">'+this.rateNm+'</span>';
            ...
            loopFooterHtml += '</div>';
            //if(stepNo > 0){
                loopFooterHtml += '<a id="join_'+this.requestKey+'" data-buytype="'+this.reqBuyType+'" data-saleyn="'+this.phoneSaleYn+'" class="c-button c-button--sm c-button--white" href="javascript:go_join_page(\''+this.requestKey+'\')"  >이어하기</a>';
            //}
            loopFooterHtml += '</div>';
            loopFooterHtml += '</span>';
            loopIndex ++;
...
function makeOrderTempList_group(listData){
    ...
    $(listData).each(function(index) {
        ...
        if(preReqKey != this.requestKey){
            ...
            loopFooterHtml += '</div>';
            loopFooterHtml += '<a id="join_'+this.requestKey+'" data-buytype="'+this.reqBuyType+'" data-saleyn="'+this.phoneSaleYn+'" class="c-button c-button--sm c-button--white" href="#" onClick="go_join_page(\''+this.requestKey+'\')" >이어하기</a>';
            loopFooterHtml += '</div>';
            loopFooterHtml += '</span>';
            loopIndex ++;
        }
