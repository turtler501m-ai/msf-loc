...
VALIDATE_COMPARE_MSG.cstmrReceiveTel = "연락가능 연락처가 형식에 맞지 않습니다.";
...
history.pushState(null, null,"/m/mypage/reqCallList.do");
	window.onpopstate = function (event){
	history.go("/m/mypage/reqCallList.do");
}
...
$(document).ready(function() {
    ...
    $('#btnReqCall').click(function(){
		...
		if($.trim($("#startDate").val()) != "" && $.trim($("#endDate").val()) != ""){
			var reqStartDate = $.trim($("#startDate").val().replace(/\./gi,""));
			reqStartDate = reqStartDate.replace(/-/gi,"");
			var reqEndDate = $.trim($("#endDate").val().replace(/\./gi,""));
			reqEndDate = reqEndDate.replace(/-/gi,"");

			var diff = fn_getDateDiff(reqStartDate + "-" + reqEndDate);
			...
			pageObj.startDate = reqStartDate;
			pageObj.endDate = reqEndDate;
		}
        ...
        if(validator.validate()){
			...
			//수신방법
			if($("input:radio[name=rdRecvType]:checked").val() == "FX"){
				pageObj.recvText = $.trim($("#faxNum").val());
			}else {
			...
			pageObj.applDataForm['startDate'] = pageObj.startDate;
			pageObj.applDataForm['endDate'] = pageObj.endDate;
			...
			pageObj.applDataForm['callNum'] = $.trim($("#cstmrReceiveTel").val());
