/*
    ...
    <jsp:include page="/WEB-INF/views/mobile/popup/simpleAuthForm.jsp">
    ...
    첫번째 본인인증 위치: <jsp:include page="/WEB-INF/views/mobile/popup/simpleAuthForm.jsp">
                         <jsp:param name="controlYn" value="Y"/>
                       </jsp:include>
    두번째 본인인증 위치: <jsp:include page="/WEB-INF/views/mobile/popup/simpleAuthForm.jsp">
                         <jsp:param name="controlYn" value="Y"/>
...
var simpleAuthvalidation = null;  // 본인인증 전 유효성 검사 function
    ...
    if($("#isAuth").length > 0){
        ...
        $("input:radio[name=onlineAuthType]").click(function(){
            ...
            if(checkAuthValidation()){
                ...
                $("._onlineAuthType").each(function(){
                    if($(this).data("value") == selAuthType) $(this).show();
                    else $(this).hide();
                });
            }else{
                $(this).prop("checked", "");
        ...
        $("._btnNiceAuth").click(function(){
            ...
            if(selAuthType == "C" || selAuthType == "M") creditCardAuth(selAuthType);
    ...
    if($("#isAuth2").length > 0){
        ...
        $("input:radio[name=onlineAuthType2]").click(function(){
            ...
            if(checkAuthValidation('Y')){
                var selAuthType = $(this).val();
                $("._onlineAuthType2").each(function(){
                    if($(this).data("value") == selAuthType) $(this).show();
                    else $(this).hide();
                });
            }else{
                $(this).prop("checked", "");
                $("._onlineAuthType2").hide();
            }
        });

        // 간편본인인증 진행버튼 클릭 이벤트
        $("._btnNiceAuth2").click(function(){
            ...
            if(selAuthType == "C" || selAuthType == "M") creditCardAuth(selAuthType, "Y");
...
function creditCardAuth(authType, isSecond){
    ...
    if(checkAuthValidation(isSecond)){
        ...
        var prcType= $("#onOffTypeInit").val() == "7" ? "COMMON" : "PARTIAL";
        openPage('outLink',authReqObj.pollingPageUrl+'&mType=Mobile&prcType='+prcType,'');
    }
}
...
function passAuth(isSecond){
    ...
    if(checkAuthValidation(isSecond)){
        ...
        var prcType= $("#onOffTypeInit").val() == "7" ? "COMMON" : "PARTIAL";
        openPage('outLink', authReqObj.pollingPageUrl+'?prcType='+prcType, '');
    }
}
...
function kakaoAuth(isSecond){
    ...
    if(checkAuthValidation(isSecond)){
        ...
        var prcType= $("#onOffTypeInit").val() == "7" ? "COMMON" : "PARTIAL";
        openPage('outLink2', authReqObj.pollingPageUrl+'?prcType='+prcType, '');
    }
}
...
function getTxId(successCallback, failCallback) {
    ...
    var varData = ajaxCommon.getSerializedData({
       ...
       ,prcType: $("#onOffTypeInit").val() == "7" ? "COMMON" : "PARTIAL"
    });
...
function vdlCheckByCstmrType(){
    ...
    var cstmrRadioCnt = $("input:radio[name=cstmrType]").length;
    var cstmrSelectCnt = $("#cstmrType").length;  // appOutForm 호환

    if(cstmrRadioCnt == 0 && cstmrSelectCnt == 0) return true;  // 고객선택유형 미존재
    ...
    if(0 < cstmrRadioCnt){
        ...
        cstmrType= $("#cstmrType").val();
        validator.config["cstmrType"] = "isNonEmpty";
    }
...
function commonPrdProcess(authType, isSecond){
    ...
    var authDesTag= "_onlineAuthType";
    var authButtonTag= "onlineAuthType";
    var $authBtn= $("#btnNiceAuth"+authType);

    if(isSecond == "Y"){ // 두번째 본인인증 태그
        $("#isAuth2").val("1");
        authDesTag= "_onlineAuthType2";
        authButtonTag= "onlineAuthType2";
        $authBtn= $("#btnNiceAuth"+authType+"2");
    }else{
        $("#isAuth").val("1");
    }

    $authBtn.addClass('is-complete').html($authBtn.html() + " 완료");

    $("."+authDesTag).each(function(){
        if(authType == $(this).data("value")) $(this).show();
        else $(this).hide();
    });

    $("input:radio[name="+authButtonTag+"]").each(function(){
        if(authType == $(this).val()) $(this).prop("checked", true);
...
function getNicePinCi(){
    ...
    var cstmrType = $("input:radio[name=cstmrType]:checked").val() ? $("input:radio[name=cstmrType]:checked").val() : $("#cstmrType").val();
    if(usimKindsCd == "09" && prntsContractNo != ""){ // esimWatch
        ...
        // 모집경로 모바일(3), 모바일셀프개통(7), 모바일해피콜(8)
        if (onOffType == "3" || onOffType == "7" || onOffType == "8") {
            // 셀프개통 신규는 pass
            if (onOffType == "7" && OPER_TYPE.NEW == operType) return true;
...
/** 메뉴 타입에따른 간편 본인인증 검사 */
function authCallbackWithMenuType(infoObj){

    var result = {};

    if(infoObj.name == undefined) infoObj.name= "";
    if(infoObj.sMobileNo == undefined) infoObj.sMobileNo= "";

    var varData = ajaxCommon.getSerializedData({
         name: infoObj.name
        ,sMobileNo: infoObj.sMobileNo
        ,reqSeq: pageObj.niceResLogObj.reqSeq
        ,resSeq: pageObj.niceResLogObj.resSeq
    ...
    ajaxCommon.getItemNoLoading({
          id: 'checkAuthCallbackWithMenuType'
          ,cache: false
          ,async: false
          ,url: '/auth/checkAuthCallbackWithMenuType.do'
          ,data: varData
          ,dataType: "json"
      }
      ,function(obj){
    ...
    if(result.RESULT_CODE == "00000") commonPrdProcess(infoObj.authType, infoObj.isSecond);
    return result;
}