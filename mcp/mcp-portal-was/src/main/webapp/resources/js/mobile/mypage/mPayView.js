...
var pageObj= {
	drawMpayHistory: null // 결제내역 팝업 표출을 위한 객체
...
// myCommCtn.jsp에서 회선 > onchage 이벤트로 select 함수를 호출한다 
function select(){
    ...
    ,function(data){
		...
		if(typeof date != "undefined" && date != null){
			...
			for(var month=0;month<dateList.length;month++){
				...
				datePeriodHtml+= '<span class="c-text c-text--type4 u-co-gray" name="firstLastDay" data-month="'+dateList[month].monthTitle+'" style="display:none">';
...
function searchFilterHistory(){
	var type= $("#mPayStatus option:selected").val(); // 검색 조건
	var drawList= filterMpayHistory(pageObj.allMpayHistory, type);
...
function filterMpayHistory(mPayList, type){
	...
	if(type=="ALL"){ // "ALL"인 경우 필터 필요 없음
		pageObj.drawMpayHistory= mPayList;
		return mPayList;
	}
	...
	pageObj.drawMpayHistory= drawList;
	return drawList;
...
function drawMPayList(drawList){
	...
	if(drawList== null || drawList.length== 0){ // 데이터가 없는 경우
		contentHtml= '<tr><td colspan="4">내역이 존재하지 않습니다.</td></tr>';
	}else{ // 데이터가 존재하는 경우
		...
		for(var i=0;i<drawList.length;i++){
            ...
            contentHtml+=   '<td>'+item.settlDtFmt+'</td>';
        	contentHtml+=   '<td>'+numberWithCommas(item.amt)+'원</td>';
         	contentHtml+=   '<td>';
         	contentHtml+=   	'<button class="c-button c-button--underline u-co-sub-4 fs-13" onclick="showDetailPay('+i+');" data-dialog-trigger="#micropay-detail_dialog">상세</button>';
         	contentHtml+=   '</td>';
         	contentHtml+=  '</tr>';
		}
	}
...
// 소액결제내역 상세화면
function showDetailPay(index){
	
	var target= pageObj.drawMpayHistory[index];
	$("#onePtnrCpNm").html(target.ptnrCpNm);
	$("#onePtnrSvcNm").html(target.ptnrSvcNm);
	$("#oneSettlNm").html(target.settlNm);
	$("#onePtnrPgNm").html(target.ptnrPgNm);
	$("#oneSettlDtFmt").html(target.settlDtFmt);
	$("#oneAmt").html(numberWithCommas(target.amt)+"원");
	$("#onePayStatusNm").html(target.payStatusNm);
