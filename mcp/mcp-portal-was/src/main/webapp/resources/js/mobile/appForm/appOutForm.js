...
$(document).ready(function(){
    ...
    $("input:radio[name=cstmrType]").click(function(){
        ...
        pageObj.applDataForm.cstmrType = thisVal;
    });
    ...
    $("input:radio[name=operTypeRadio]").click(function(){
        ...
        if (operType == OPER_TYPE.MOVE_NUM) {
            $("#pOperTypeDesc").html("현재 사용하고 있는 번호 유지하고 kt M 에 가입합니다.");
        } else {
    ...
    var chgBottomTitle = function() {
        ...
        var dataType = $("#dataTypeRadio").val() ? $("#dataTypeRadio").val() : $('#prdtSctnCd').val();
        $('#bottomTitle').text(dataType + prdtNmTemp + operTypeNm + rateNm);
    }
    ...
    $("#selSocCode").change(function(){
        ...
        $("#paySumMnth").html(numberWithCommas(itemObj.payMnthChargeAmt));
        $("#totalPrice2").html(numberWithCommas(itemObj.payMnthChargeAmt));
        $("#totalPrice").html(numberWithCommas(itemObj.payMnthChargeAmt));
        $("#baseAmt").html(numberWithCommas(itemObj.baseVatAmt));
        $("#addDcAmt").html(numberWithCommas(itemObj.addDcVatAmt));
        $("#promotionDcAmtTxt").html(numberWithCommas(itemObj.promotionDcAmt));
            ...
            ajaxCommon.getItemNoLoading({
                ...
                ,function(jsonObj){
                    if (AJAX_RESULT_CODE.SUCCESS ==  jsonObj.RESULT_CODE) {
                        ...
                        MCP.alert("입력하신 계좌 정보로 인증이 불가합니다.<br> 입력 정보를 확인 후 다시 시도해 주시기 바랍니다.");
                    }
                });
    ...
    $('#btnSetAddition').click(function(){
        ...
        var operType = $("#operType").val();
        if (operType == OPER_TYPE.NEW || operType == OPER_TYPE.MOVE_NUM) {
            ...
            ajaxCommon.getItemNoLoading({
                ...
                ,function(jsonObj){
                    if (AJAX_RESULT_CODE.SUCCESS ==  jsonObj.RESULT_CODE) {
                        ...
                        $("#btnCheckAccount").html("계좌번호 체크 완료");
    ...
    $("._btnAddr").click(function() {
        ...
        if (addrFlag != "3") {
            ...
            openPage('pullPopup', '/m/dlvryInfo.do');
        }
    });
    ...
    var isValidateStep1 = function() {
        ...
        pageObj.applDataForm.cstmrType = cstmrType;
            ...
            ,function(jsonObj){
                if (AJAX_RESULT_CODE.SUCCESS ==  jsonObj.RESULT_CODE) {
                    ...
                    ajaxCommon.createForm({
                        method:"post"
                        ,action:"/m/appForm/appOutFormComplete.do"
                    });
    ...
    var step4Show = function() {
        ...
        //임시 저장 안심보험
        var insrProdCdTemp = $("#insrProdCdTemp").val() ;
        if (insrProdCdTemp !="" && pageObj.insrProdList.length < 1) {
            getInsrProdInfo();
        }

        // 자급제 보상 서비스 추가 (초기 선택값 세팅)
        var rwdProdCdTemp = $("#rwdProdCdTemp").val();
        if (rwdProdCdTemp !="" && pageObj.rwdProdList.length < 1) {
    ...
    var getListSale = function() {
                ...
                if (objList.length > 0) {
                    ...
                    for (var i=0 ; i < objList.length ; i++) {
                        ...
                        if ($("#reqBuyType").val() == "MM" && $("#prdtSctnCd").val() == "LTE5G") {
                            if ($("#dataTypeRadio").val() != objList[i].mspRateMstDto.dataType) {
                                continue;
                            }
                        }
                        var arr =[];
        ...
        if (modelSalePolicyCode != "" && pageObj.salePlcyObj == null) {
            ajaxCommon.getItemNoLoading({
                    id:'getSalePlcyAjax'
                    ,cache:false
    ...
    var showJoinUsimPrice = function() {
        ...
        if (joinPriceIsPay == "Y") {
            ...
            $("#joinPriceTxt").html('<span class="c-text c-text--strike">'+numberWithCommas(joinPrice)+' 원</span>(무료)');
        }

        if (usimPriceIsPay == "Y") {
            $("#usimPriceTxt").html(numberWithCommas(usimPrice)+" 원");
        } else {
            $("#usimPriceTxt").html('<span class="c-text c-text--strike">'+numberWithCommas(usimPrice)+' 원</span>(무료)');
        }
    }
...
var fnNiceCert = function(prarObj) {
    ...
    if (pageObj.niceType == NICE_TYPE.CUST_AUTH) {
        ...
        if (obj.RESULT_CODE == "00000") {
            ...
            //console.log("pageObj.niceHistInsrProdSeq===>" + pageObj.niceHistInsrProdSeq);
            isValidateNonEmptyStep4();
            return null;
...
var isValidateNonEmptyStep4 = function() {
    validator.config={};
    //console.log("isValidateNonEmptyStep4 => Call11111" );
    ...
    if (validator.validate(true)) {
        //  console.log("isValidateNonEmptyStep4 => Call" );
