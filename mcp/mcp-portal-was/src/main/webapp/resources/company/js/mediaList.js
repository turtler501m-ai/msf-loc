...
function loadMoreData(pageNo) {
    $.ajax({
        url: '/company/moreMediaList.do',
        method: 'POST',
        data: {
            pageNo: pageNo
        },
        success: function (response) {
            const newsDataList = response.newsDataList;
            ...
            newsDataList.forEach(news => {
                const newsItemHtml = `
                    <li class="board-item">
                        <a href="javascript:void(0);" class="news-link" data-seq="${news.newsDatSeq}" data-target="${news.linkTarget}" onclick="goDetail(${news.newsDatSeq}, ${currentPage});">
                ...
                `;
                $('.board-list').append(newsItemHtml);
            });
