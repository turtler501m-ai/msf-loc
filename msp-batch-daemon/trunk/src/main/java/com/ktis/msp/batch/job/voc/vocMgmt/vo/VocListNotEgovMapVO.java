package com.ktis.msp.batch.job.voc.vocMgmt.vo;

import com.ktis.msp.base.BaseVo;

public class VocListNotEgovMapVO extends BaseVo {
	
	private String rcpId;
	private String rcpIdStr;
	private String pppo;
	private String telNum;
	private String custNm;
	private String customerId;
	private String contractNum;
	private String custYn;
	private String custYnNm;
	private String custRel;
	private String custRelNm;
	private String reqCustNm;
	private String reqTelNum;
	private String reqOrgnId;
	private String reqOrgnNm;
	private String reqrId;
	private String reqrNm;
	private String reqDttm;
	private String cnslMstCd;
	private String cnslMstNm;
	private String cnslMidCd;
	private String cnslMidNm;
	private String cnslDtlCd;
	private String cnslDtlNm;
	private String cnslStat;
	private String cnslStatNm;
	private String procOrgnId;
	private String procOrgnNm;
	private String procrId;
	private String procrNm;
	private String procDttm;
	private String cmplOrgnId;
	private String cmplOrgnNm;
	private String cmplrId;
	private String cmplrNm;
	private String cmplDttm;
	private String procMm;
	private String prodTypeNm;
	private String reqCntt;
	private String resCntt;
	private String procCntt;
	private String ssn;
	private String custAge;
	private String cstmrType;
	
	private String lstRateNm;	/*현재요금제*/
	private String orgnNm;		/*개통대리점*/
	
	// 2017-07-24, 잔여시간 추가
	private String remainMm;
	
	public String getLstRateNm() {
		return lstRateNm;
	}
	public void setLstRateNm(String lstRateNm) {
		this.lstRateNm = lstRateNm;
	}
	public String getOrgnNm() {
		return orgnNm;
	}
	public void setOrgnNm(String orgnNm) {
		this.orgnNm = orgnNm;
	}	
	public String getCstmrType() {
		return cstmrType;
	}
	public void setCstmrType(String cstmrType) {
		this.cstmrType = cstmrType;
	}
	public String getSsn() {
		return ssn;
	}
	public void setSsn(String ssn) {
		this.ssn = ssn;
	}
	public String getCustAge() {
		return custAge;
	}
	public void setCustAge(String custAge) {
		this.custAge = custAge;
	}
	public String getRcpId() {
		return rcpId;
	}
	public void setRcpId(String rcpId) {
		this.rcpId = rcpId;
	}
	public String getRcpIdStr() {
		return rcpIdStr;
	}
	public void setRcpIdStr(String rcpIdStr) {
		this.rcpIdStr = rcpIdStr;
	}
	public String getPppo() {
		return pppo;
	}
	public void setPppo(String pppo) {
		this.pppo = pppo;
	}
	public String getTelNum() {
		return telNum;
	}
	public void setTelNum(String telNum) {
		this.telNum = telNum;
	}
	public String getCustNm() {
		return custNm;
	}
	public void setCustNm(String custNm) {
		this.custNm = custNm;
	}
	public String getCustomerId() {
		return customerId;
	}
	public void setCustomerId(String customerId) {
		this.customerId = customerId;
	}
	public String getContractNum() {
		return contractNum;
	}
	public void setContractNum(String contractNum) {
		this.contractNum = contractNum;
	}
	public String getCustYn() {
		return custYn;
	}
	public void setCustYn(String custYn) {
		this.custYn = custYn;
	}
	public String getCustYnNm() {
		return custYnNm;
	}
	public void setCustYnNm(String custYnNm) {
		this.custYnNm = custYnNm;
	}
	public String getCustRel() {
		return custRel;
	}
	public void setCustRel(String custRel) {
		this.custRel = custRel;
	}
	public String getCustRelNm() {
		return custRelNm;
	}
	public void setCustRelNm(String custRelNm) {
		this.custRelNm = custRelNm;
	}
	public String getReqCustNm() {
		return reqCustNm;
	}
	public void setReqCustNm(String reqCustNm) {
		this.reqCustNm = reqCustNm;
	}
	public String getReqTelNum() {
		return reqTelNum;
	}
	public void setReqTelNum(String reqTelNum) {
		this.reqTelNum = reqTelNum;
	}
	public String getReqOrgnId() {
		return reqOrgnId;
	}
	public void setReqOrgnId(String reqOrgnId) {
		this.reqOrgnId = reqOrgnId;
	}
	public String getReqOrgnNm() {
		return reqOrgnNm;
	}
	public void setReqOrgnNm(String reqOrgnNm) {
		this.reqOrgnNm = reqOrgnNm;
	}
	public String getReqrId() {
		return reqrId;
	}
	public void setReqrId(String reqrId) {
		this.reqrId = reqrId;
	}
	public String getReqrNm() {
		return reqrNm;
	}
	public void setReqrNm(String reqrNm) {
		this.reqrNm = reqrNm;
	}
	public String getReqDttm() {
		return reqDttm;
	}
	public void setReqDttm(String reqDttm) {
		this.reqDttm = reqDttm;
	}
	public String getCnslMstCd() {
		return cnslMstCd;
	}
	public void setCnslMstCd(String cnslMstCd) {
		this.cnslMstCd = cnslMstCd;
	}
	public String getCnslMstNm() {
		return cnslMstNm;
	}
	public void setCnslMstNm(String cnslMstNm) {
		this.cnslMstNm = cnslMstNm;
	}
	public String getCnslMidCd() {
		return cnslMidCd;
	}
	public void setCnslMidCd(String cnslMidCd) {
		this.cnslMidCd = cnslMidCd;
	}
	public String getCnslMidNm() {
		return cnslMidNm;
	}
	public void setCnslMidNm(String cnslMidNm) {
		this.cnslMidNm = cnslMidNm;
	}
	public String getCnslDtlCd() {
		return cnslDtlCd;
	}
	public void setCnslDtlCd(String cnslDtlCd) {
		this.cnslDtlCd = cnslDtlCd;
	}
	public String getCnslDtlNm() {
		return cnslDtlNm;
	}
	public void setCnslDtlNm(String cnslDtlNm) {
		this.cnslDtlNm = cnslDtlNm;
	}
	public String getCnslStat() {
		return cnslStat;
	}
	public void setCnslStat(String cnslStat) {
		this.cnslStat = cnslStat;
	}
	public String getCnslStatNm() {
		return cnslStatNm;
	}
	public void setCnslStatNm(String cnslStatNm) {
		this.cnslStatNm = cnslStatNm;
	}
	public String getProcOrgnId() {
		return procOrgnId;
	}
	public void setProcOrgnId(String procOrgnId) {
		this.procOrgnId = procOrgnId;
	}
	public String getProcOrgnNm() {
		return procOrgnNm;
	}
	public void setProcOrgnNm(String procOrgnNm) {
		this.procOrgnNm = procOrgnNm;
	}
	public String getProcrId() {
		return procrId;
	}
	public void setProcrId(String procrId) {
		this.procrId = procrId;
	}
	public String getProcrNm() {
		return procrNm;
	}
	public void setProcrNm(String procrNm) {
		this.procrNm = procrNm;
	}
	public String getProcDttm() {
		return procDttm;
	}
	public void setProcDttm(String procDttm) {
		this.procDttm = procDttm;
	}
	public String getCmplOrgnId() {
		return cmplOrgnId;
	}
	public void setCmplOrgnId(String cmplOrgnId) {
		this.cmplOrgnId = cmplOrgnId;
	}
	public String getCmplOrgnNm() {
		return cmplOrgnNm;
	}
	public void setCmplOrgnNm(String cmplOrgnNm) {
		this.cmplOrgnNm = cmplOrgnNm;
	}
	public String getCmplrId() {
		return cmplrId;
	}
	public void setCmplrId(String cmplrId) {
		this.cmplrId = cmplrId;
	}
	public String getCmplrNm() {
		return cmplrNm;
	}
	public void setCmplrNm(String cmplrNm) {
		this.cmplrNm = cmplrNm;
	}
	public String getCmplDttm() {
		return cmplDttm;
	}
	public void setCmplDttm(String cmplDttm) {
		this.cmplDttm = cmplDttm;
	}
	public String getProcMm() {
		return procMm;
	}
	public void setProcMm(String procMm) {
		this.procMm = procMm;
	}
	public String getProdTypeNm() {
		return prodTypeNm;
	}
	public void setProdTypeNm(String prodTypeNm) {
		this.prodTypeNm = prodTypeNm;
	}
	public String getReqCntt() {
		return reqCntt;
	}
	public void setReqCntt(String reqCntt) {
		this.reqCntt = reqCntt;
	}
	public String getResCntt() {
		return resCntt;
	}
	public void setResCntt(String resCntt) {
		this.resCntt = resCntt;
	}
	public String getProcCntt() {
		return procCntt;
	}
	public void setProcCntt(String procCntt) {
		this.procCntt = procCntt;
	}
	public String getRemainMm() {
		return remainMm;
	}
	public void setRemainMm(String remainMm) {
		this.remainMm = remainMm;
	}
	
}
