...
//				2. SELECT INSERT QRY 수행
				...
				LOGGER.debug("개통고객 LMS 전송(단말할인) : 발송 데이터 추출 갯수 => {}", iSetKDCnt);
				
				// 요금할인
				int iSetPMCnt = OpenCstmrLmsSelectInsert(lSeq, "PM");
				LOGGER.debug("개통고객 LMS 전송(요금할인) : 발송 데이터 추출 갯수 => {}", iSetPMCnt);
				
				// 심플할인
				int iSetSMCnt = OpenCstmrLmsSelectInsert(lSeq, "SM");
				LOGGER.debug("개통고객 LMS 전송(심플할인) : 발송 데이터 추출 갯수 => {}", iSetSMCnt);

				if(iSetKDCnt > 0 || iSetPMCnt > 0 || iSetSMCnt > 0) {
//					3. LMS 발송 고객 조회
					List<Map<String, Object>> list = OpenMapper.getOpenCstmrLmsList(lSeq);
					LOGGER.debug("개통고객 LMS 전송(단말할인, 요금할인, 심플할인) : 발송 데이터 조회 갯수 => {}", list.size());
...
//						4. 필수값 체크
						if(valid(strRateCd, strReqBuyType, strModelMonthly, strContractNum, strSubscriberNo)) {
							LOGGER.debug("개통고객 LMS 전송 : Validation 성공.");
						}
...
//						if(rateCan != null && rateCan.size() > 0) {
						...
						LOGGER.debug("요금제코드({}), 지원금유형({}), 할부기간({})의 위약금은 12개월 {}원, 18개월 {}원 입니다.", strRateCd, strReqBuyType, strModelMonthly, strCcAmt12, strCcAmt18);
...
//							6(1/2). LMS 발송 INSERT : 발송조건 만족
						...
						LOGGER.debug("개통고객 LMS 전송 내용 : {}", smsVO.getMessage());
						
						int iLmsSendCnt = 0;
						
						smsVO.setReserved01("BATCH");
						smsVO.setReserved02(templateId);
						smsVO.setReserved03("OpenCstmrLmsSchedule");
						smsVO.setTemplateId(templateId);
						smsVO.setRcptData(strSubscriberNo);
						// SMS 발송 등록
...
//							2(2/2). 수행 결과 UPDATE
						if(iLmsSendCnt > 0) {
							...
							iReturn++;
							OpenCstmrLmsUpdate(strContractNum, s0000, "LMS 발송 성공.", smsVO.getMessage());
						} else {
							LOGGER.error("개통고객 LMS 전송 : LMS 발송 실패.");
							OpenCstmrLmsUpdate(strContractNum, s9993, "LMS 발송 실패.", smsVO.getMessage());
							continue;
						}
//						} else {
