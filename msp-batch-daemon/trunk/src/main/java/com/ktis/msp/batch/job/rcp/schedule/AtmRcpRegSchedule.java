 ...
 */
@Component
public class AtmRcpRegSchedule extends BaseSchedule {
	
	@Autowired
	private AtmMgmtService atmMgmtService;
	 ...
	 * @throws Exception
	 */
	@Scheduled(cron="${atmRcpRegSchedule}")
	...
	public String execute(BatchCommonVO batchCommonVO) throws MvnoServiceException {
		...
		try {
			...
			List<RcpDetailVO> list = atmMgmtService.selectNonAtmRcpList();
			...
			for(RcpDetailVO rcpDetailVO : list) {
				...
				/*신청서 생성*/
				int returnCnt = atmMgmtService.updateRcpNewDetail(rcpDetailVO);
				...
				if(returnCnt == 0) {
					...
					atmMgmtService.prodSendScan(scanVo, rcpDetailVO);
				}
