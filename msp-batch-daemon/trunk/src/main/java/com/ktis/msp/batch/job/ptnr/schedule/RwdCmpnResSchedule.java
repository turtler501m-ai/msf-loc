...
/**
 * 권리실행 접수현황 정보 연동 받은 후 생성 스케줄러
 * 매일 3시 45분 생성
 */

@Component
public class RwdCmpnResSchedule extends BaseSchedule {
	 ...
	 * @throws Exception
	 */
	@Scheduled(cron="${rwdCmpnResSchedule}")
	...
	public String execute(BatchCommonVO batchCommonVO) throws MvnoServiceException {
		...
		try {
			...
			// ========== BATCH MAIN ==================
			LOGGER.info("--- 권리실행 접수 파일 다운로드 시작 ---");
			PointFileVO pointFileVO = rwdService.regRwdFileInfo(PointConstants.STR_WINIA, PointConstants.WINIA_005, "res");
			
			ftpService.downloadPointFile(PointConstants.STR_WINIA, pointFileVO.getFileNm());
			
			// 권리실행 접수현황 결과 파일 읽기
			rwdService.rwdStaReadFile(pointFileVO.getFileNm(), "3");
			LOGGER.info("--- 권리실행 접수 파일 다운로드 종료 ---");
			// ========== BATCH MAIN ==================
