...
import com.ktis.msp.batch.job.rcp.rcpmgmt.mapper.OsstMgmtMapper;
...
@Service
public class OsstMgmtService extends BaseService {

	@Autowired
	private OsstMgmtMapper osstMgmtMapper;
	...
	public List<RcpDetailVO> selectNonOsstRcpList(String jobCd) throws MvnoServiceException {

		LOGGER.info("신청정보 미생성 OSST 개통정보 조회 START");

		// OSST 개통정보 조회
		List<RcpDetailVO> list = osstMgmtMapper.selectNonOsstRcpList(jobCd);

		LOGGER.info("신청정보 미생성 OSST 개통정보 조회 End 건수 = [{}]", list.size());
	 ...
	 */
	@Transactional(rollbackFor=Exception.class)
	public List<RcpDetailVO> selectNonKmvRcpList() throws MvnoServiceException {

		LOGGER.info("KMV FTP 연동누락 개통정보 조회 START");

		// OSST 개통정보 조회
		List<RcpDetailVO> list = osstMgmtMapper.selectNonKmvRcpList();

		LOGGER.info("KMV FTP 연동누락 개통정보 조회 End 건수 = [{}]" , list.size());
	...
	public int updateRcpNewDetail(RcpDetailVO vo) throws MvnoServiceException {
		...
		if(StringUtils.isEmpty(vo.getRequestKey())){
			/*isNew = true;*/
			RcpDetailVO keyMap = osstMgmtMapper.maxRequestkey();
...
*/			vo.setManagerCode("M0001");
		...
		/** 2022.11.24.wooki 주민번호를 이용하여 cstmrType set start */
		if(StringUtil.isBlank(vo.getCstmrType())){
			//고객타입(NA,FN,NM)
	    	String customerType = "NA";
	    	
			if(StringUtil.isNotBlank(vo.getCstmrNativeRrn())) {
				
				String unUserSsn = "";
				//암호화된 주민번호 복호화
				try {
					unUserSsn = EncryptUtil.ace256Dec(vo.getCstmrNativeRrn());
				} catch (CryptoException e) {
					throw new MvnoServiceException("cstmrNativeRrn복호화error", e);
				}
			...
			vo.setCstmrType(customerType);
		}
		/** 주민번호를 이용하여 cstmrType set end */
			
		updateRcpRequestCnt = osstMgmtMapper.insertRcpRequest(vo);
		
		osstMgmtMapper.insertRcpCustomer(vo);
		osstMgmtMapper.insertRcpSale(vo);
		osstMgmtMapper.insertRcpDelivery(vo);
		osstMgmtMapper.insertRcpReq(vo);
		osstMgmtMapper.insertRcpMove(vo);
		osstMgmtMapper.insertRcpPay(vo);
		osstMgmtMapper.insertRcpAgent(vo);
		
		/*20240904 MSP 데이터 추가 개발 (이승국)*/
		osstMgmtMapper.insertMspRequest(vo);
		osstMgmtMapper.insertMspCustomer(vo);
		osstMgmtMapper.insertMspSale(vo);
		osstMgmtMapper.insertMspDelivery(vo);
		osstMgmtMapper.insertMspReq(vo);
		osstMgmtMapper.insertMspMove(vo);
		osstMgmtMapper.insertMspPay(vo);
		osstMgmtMapper.insertMspAgent(vo);
		
		//시퀸스 선 조회 후 값..
		int stateKey = osstMgmtMapper.getStateKey(vo);
		vo.setStatusKey(stateKey);
		
		osstMgmtMapper.insertRcpState(vo);
		osstMgmtMapper.insertRcpChange(vo);
		osstMgmtMapper.insertRcpDvcChg(vo);
		
		osstMgmtMapper.insertMspState(vo);
		osstMgmtMapper.insertMspChange(vo);
		osstMgmtMapper.insertMspDvcChg(vo);
		
		//20221124 wooki operType 확인해봐야 함.
		if("NAC3".equals(vo.getOperType()) || "MNP3".equals(vo.getOperType())){
			osstMgmtMapper.deleteRcpAddition(vo);
			osstMgmtMapper.deleteMspAddition(vo);
			
			if( vo.getAdditionKey() != null && !"".equals(vo.getAdditionKey()) ){ 
				osstMgmtMapper.insertRcpAddition(vo);
				osstMgmtMapper.insertMspAddition(vo);
			}
			
		}
		
		// 서식지가 생성된 OSST 연동정보 수정
		osstMgmtMapper.updateOsstRcpCrYn(vo);
		
		//2022.11.24.wooki
		osstMgmtMapper.updateMspRequestDtl(vo);

		vo = osstMgmtMapper.selectMcpRequestDtl(vo.getRequestKey());

		osstMgmtMapper.insertMcpRequestDtl(vo);
	...
	public void prodSendScan(ScanVO vo, RcpDetailVO rcpDetailVO) throws MvnoServiceException {
		...
		//1. 서식지 정보를 조회
		Map<String, String> requestData = osstMgmtMapper.getAppFormData(vo);
		...
		osstMgmtMapper.updateAppFormXmlYn(vo);
		
		String scanId = osstMgmtMapper.selectScanId(vo);
		
		rcpDetailVO.setScanId(scanId);
		
		//8. ScanId 값 UPDATE 처리
		osstMgmtMapper.updateOsstJuoSubInfo(rcpDetailVO);
		osstMgmtMapper.updateOsstRequestDtl(rcpDetailVO);
...
//		StringBuffer xml = new StringBuffer(InitializaionXml(vo));
		...
		//5-1. 서식지 위치정보 가져오기
		List<Map<String, String>> appPointInfoList = osstMgmtMapper.getAppFormPointGroupList(groupCode);
...
//			String clauseRentalService = requestData.containsKey("CLAUSE_RENTAL_SERVICE") && requestData.get("CLAUSE_RENTAL_SERVICE") != null ? requestData.get("CLAUSE_RENTAL_SERVICE") : "";
		...
		//금융제휴 요금제 체크
		List<Map<String, Object>> list = osstMgmtMapper.getMcpCodeList("ClauseFinanceRatesCD");
	...
	public String InitializaionXml(ScanVO vo, int totalPage){
		...
		Map<String,String> requestData = osstMgmtMapper.getAppFormUserData(vo);
