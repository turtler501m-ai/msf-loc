...
/**
 * 보상처리현황 정보 연동 받은 후 생성 스케줄러
 * 매일 4시 45분 생성
 */

@Component
public class RwdCmpnProcSchedule extends BaseSchedule {
	 ...
	 * @throws Exception
	 */
	@Scheduled(cron="${rwdCmpnProcSchedule}")
	...
	/**
	 * 보상처리현황 접수현황 정보 연동 받은 후 생성 스케줄러
	 */
	@Override
	public String execute(BatchCommonVO batchCommonVO) throws MvnoServiceException {
		...
		try {
			...
			// ========== BATCH MAIN ==================
			LOGGER.info("--- 보상처리현황 접수 파일 다운로드 시작 ---");
			PointFileVO pointFileVO = rwdService.regRwdFileInfo(PointConstants.STR_WINIA, PointConstants.WINIA_006, "res");
			
			ftpService.downloadPointFile(PointConstants.STR_WINIA, pointFileVO.getFileNm());
			
			// 보상처리현황 결과 파일 읽기
			rwdService.rwdCmpReadFile(pointFileVO.getFileNm(), "3");
			LOGGER.info("--- 보상처리현황 파일 다운로드 종료 ---");
			// ========== BATCH MAIN ==================
