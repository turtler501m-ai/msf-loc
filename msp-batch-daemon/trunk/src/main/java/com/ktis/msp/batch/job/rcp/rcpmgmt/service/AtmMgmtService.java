...
import com.ktis.msp.batch.job.rcp.rcpmgmt.mapper.AtmMgmtMapper;
...
@Service
public class AtmMgmtService extends BaseService {

	@Autowired
	private AtmMgmtMapper atmMgmtMapper;
	
	@Resource(name = "propertiesService")
	protected EgovPropertyService propertiesService;
	
	/**
	 * 신청정보 미생성 ATM 개통정보 조회
	 * @return
	 * @throws MvnoServiceException
	 */
	@Transactional(rollbackFor=Exception.class)
	public List<RcpDetailVO> selectNonAtmRcpList() throws MvnoServiceException {

		LOGGER.info("신청정보 미생성 ATM 개통정보 조회 START");

		// 해지고객 TEMP 이관대상 조회
		List<RcpDetailVO> list = atmMgmtMapper.selectNonAtmRcpList();
	...
	public int updateRcpNewDetail(RcpDetailVO vo) throws MvnoServiceException {
		...
		if(StringUtils.isEmpty(vo.getRequestKey())){
			/*isNew = true;*/
			RcpDetailVO keyMap = atmMgmtMapper.maxRequestkey();
...
*/			vo.setManagerCode("M0001");
		...
		updateRcpRequestCnt = atmMgmtMapper.insertRcpRequest(vo);
		
		atmMgmtMapper.insertRcpCustomer(vo);
		atmMgmtMapper.insertRcpSale(vo);
		atmMgmtMapper.insertRcpDelivery(vo);
		atmMgmtMapper.insertRcpReq(vo);
		atmMgmtMapper.insertRcpMove(vo);
		atmMgmtMapper.insertRcpPay(vo);
		atmMgmtMapper.insertRcpAgent(vo);
		
		//시퀸스 선 조회 후 값..
		int stateKey = atmMgmtMapper.getStateKey(vo);
		vo.setStatusKey(stateKey);
		
		atmMgmtMapper.insertRcpState(vo);
		atmMgmtMapper.insertRcpChange(vo);
		atmMgmtMapper.insertRcpDvcChg(vo);
		
		if("NAC3".equals(vo.getOperType()) || "MNP3".equals(vo.getOperType())){
			atmMgmtMapper.deleteRcpAddition(vo);
			
			if( vo.getAdditionKey() != null && !"".equals(vo.getAdditionKey()) ){ 
				atmMgmtMapper.insertRcpAddition(vo);
			}
			
		}
		
		// 서식지가 생성된 ATM 연동정보 수정
		atmMgmtMapper.updateAtmRcpCrYn(vo);
	...
	public void prodSendScan(ScanVO vo, RcpDetailVO rcpDetailVO) throws MvnoServiceException {
		...
		//1. 서식지 정보를 조회
		Map<String, String> requestData = atmMgmtMapper.getAppFormData(vo);
		...
		atmMgmtMapper.updateAppFormXmlYn(vo);
		
		String scanId = atmMgmtMapper.selectScanId(vo);
		
		rcpDetailVO.setScanId(scanId);
		
		//8. ScanId 값 UPDATE 처리
		atmMgmtMapper.updateAtmJueSubInfo(rcpDetailVO);
		atmMgmtMapper.updateAtmRequestDtl(rcpDetailVO);
...
//		StringBuffer xml = new StringBuffer(InitializaionXml(vo));
		...
		//5-1. 서식지 위치정보 가져오기
		List<Map<String, String>> appPointInfoList = atmMgmtMapper.getAppFormPointGroupList(groupCode);
...
//			String clauseRentalService = requestData.containsKey("CLAUSE_RENTAL_SERVICE") && requestData.get("CLAUSE_RENTAL_SERVICE") != null ? requestData.get("CLAUSE_RENTAL_SERVICE") : "";
		...
		//금융제휴 요금제 체크
		List<Map<String, Object>> list = atmMgmtMapper.getMcpCodeList("ClauseFinanceRatesCD");
	...
	public String InitializaionXml(ScanVO vo, int totalPage){
		...
		Map<String,String> requestData = atmMgmtMapper.getAppFormUserData(vo);
